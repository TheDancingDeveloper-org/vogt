"""Reading the PR ↔ work-item edge a forge already records.
...
Observed-first, additive, forward-only: the edge is *read*, never
...
#: `branch_patterns` lands. Read off the fixture manifest's
#: `branch_patterns` (`gh-<n>-<slug>`, `feature/gh-<n>-<slug>`,
#: `wi-<n>/<slug>`): a known prefix, a separator, then the issue number.
# TODO: source the prefixes/shapes from `config.branch_patterns` once that
# exists, and pass the compiled pattern in rather than defaulting here.
