"""The Forgejo/Gitea implementation of `ForgeProvider` (D2).
...
collectors turn the declaration into a `not_supported` receipt.
...
installation (D8). The estate's is `forge.example.net`; a test's is whatever
its config says. That is why `parse` here takes its host list from the
provider's own client rather than a module literal.

Subject keys are host-qualified — `forge:{host}/{owner}/{repo}#{n}` —
because two Forgejo installations can each hold an `acme/widgets`
...
class ForgejoClient:
    ...
    The *shape* — token from a file, `Transport` seam for tests,
    ...
    def send(self, path: str, payload: dict[str, Any], *, method: str = "POST") -> Any:
        ...
        greppable, and there is no DELETE anywhere in the product.
        """
...
#: What a forge permits in an owner or repository name; matches the
...
def parse_repo_url(repo_url: str | None, hosts: tuple[str, ...]) -> RepoRef | None:
    ...
    # arbitrary path/query. Reject it — "not mine", the ordinary None.
...
class ForgejoProvider:
    ...
    def capabilities(self) -> ForgeCapabilities:
        """Declared per what `/api/v1` actually offers, never probed.
        ...
        - `supports_notifications` — `/repos/{o}/{r}/notifications` exists
          and is repo-scoped, so the scope rule holds the same way.
    ...
    def list_repos(self) -> Iterable[ForgeRepo]:
        ...
        . Enumeration, never a crawl: the token is the scope."""
    ...
    def checks(self, ref: RepoRef) -> Iterable[ForgeCheck]:
        ...
        # `None` — an honest empty, not an error.
        payloads = self._client.get(
    ...
    def posture(self, ref: RepoRef) -> ForgePosture:
        ...
        "could not tell", which is not the same answer as "off".
        """
        return ForgePosture(
    ...
    def notifications(self, ref: RepoRef) -> Iterable[ForgeNotification]:
        # Per-repository, never the account-wide inbox. `all=true`
        ...
        for item in _as_list(payloads):
            ...
            yield ForgeNotification(
                ...
                # guess (the not-collected rule applied to fields rather than
                # kinds).
                reason=None,
    ...
    def update_issue_body(
        ...
        """Re-render an issue body's managed region.
        ...
        no human text is removed and the no-deletion rule's spirit holds."""
        return self._post(
            ref,
            endpoint=f"/repos/{ref.owner}/{ref.repo}/issues/{number}",
            payload={"body": body},
    ...
    def create_repo(
        ...
        """`POST /user/repos` — a new repository under this credential.
