"""Incremental all-state forge sync (D3).
...
projection forever. This reads `state=all` incrementally instead: each
...
class _ForgeSyncCollector:
    ...
    def collect(self, ctx: CollectorContext, project: Project) -> Iterable[Finding]:
        ...
        if provider is None or ref is None:
            ...
            # so a zero here is never read as "there is nothing".
            yield self._receipt(
    ...
    def _receipt(
        ...
        *,
        ...
        # The config names the Forgejo hosts, so the reason can tell
    ...
    def _finding(
        ...
        return finding(
            ...
            payload={
                ...
                # against the overlay rather than silently overwrite it.
                "body": item.body,
    ...
    def _finding(
        ...
        return finding(
            ...
            payload={
                ...
                "base": item.base,
                # Reviewability rollups where the provider exposes them (GitHub
                # first); `None` is an honest "the forge did not say".
                ...
                # Empty when the PR names no work.
...
def _implements(
    ...
    `ParsedEdge`s, then keys each against the forge so a same-repo `#3`
