...
class ForgeCapabilities:
    ...
    silently collecting nothing. `supports_since` is the one Phase 2
...
class ForgePull:
    ...
    the tip SHA) — the half the PR↔work-item edge reads. `review_state`,
...
@dataclass(frozen=True)
class ForgeRelease:
    """One release, which is where an observed version comes from."""
...
@dataclass(frozen=True)
class ForgePosture:
    """Update-automation posture, three independent facts.
...
class ForgeNotification:
    """One notification thread for a registered repository."""
...
@dataclass(frozen=True)
class ForgeCheck:
    """One check on one revision. Actions is a producer, not the model."""
