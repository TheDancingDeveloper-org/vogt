"""The forge read collectors, behind the provider (D2).
...
an empty success, the same way the sync collectors do.
...
class ForgeReleasesCollector(_ForgeReadCollector):
    """Releases, where an observed version comes from. Replaces
    `gh-releases`."""
...
class ForgeLabelsCollector(_ForgeReadCollector):
    """Repository labels, now on the read surface (v1 ceiling)."""
...
class ForgeNotificationsCollector(_ForgeReadCollector):
    """Per-repository notifications. Replaces `gh-notifications`."""
