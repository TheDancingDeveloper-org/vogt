...
GitHub was the only implementation in v1; Forgejo/Gitea (`forgejo.py`)
...
1. **There is no destructive verb.** The write surface is `comment`,
  ...
  . "No deletion, no force, ever" is guaranteed here by
...
2. **Capabilities are declared, never discovered.** A caller reads
   ...
   not. A forge that cannot answer "what changed since X" forces a
...
- **Gitea / Forgejo** — implemented (`forgejo.py`), and the paper
  ...
  `supports_posture` is `False` and the gap reports itself. Subject
  keys are host-qualified: `forge:{host}/{o}/{r}#{n}`.
"""
...
class ForgeProvider(Protocol):
    ...
    def list_repos(self) -> Iterable[ForgeRepo]:
        """Repositories the credential can see, for the import picker.
        ...
        rule is unbroken because the credential is the scope. A provider
    ...
    def posture(self, ref: RepoRef) -> ForgePosture:
        """Update-automation posture. Only meaningful when
    ...
    def update_issue_body(
        ...
        """Re-render an issue *body* — the one edit verb, deliberately bounded.
        ...
        for. It stays inside the no-deletion rule's spirit by construction of
        its only caller —
    ...
    def create_repo(
        ...
        """Create a new, empty repository under the credential's account.
        ...
        additive under the no-deletion rule — the verb set gains a create and no
