"""Health, version, and connection info — plain HTTP, always.

`DEPLOYMENT.md` exist because of a specific failure in an earlier product:
...
class Readiness(BaseModel):
    ...
    Readiness gates traffic until migration completes (`DEPLOYMENT.md`),
    which is why this reports schema versions rather
...
class ConnectionInfo(BaseModel):
    ...
    #: this is the one connection fact it cannot infer. Reported as
    ...
    *,
    ...
    async def ready(response: Response) -> Readiness:
        ...
        try:
            ...
            # at all. Log it with a correlation ref; return only the ref.
        ...
        # Named per store and with both numbers, because "not ready"
    ...
    async def connection_info(request: Request) -> ConnectionInfo:
        ...
        Unauthenticated by design, and therefore the one place a
