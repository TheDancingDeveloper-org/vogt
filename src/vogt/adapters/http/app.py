...
which is what makes "nothing is client-only" structurally true rather
...
values into lists, and emits a complete OpenAPI document — rather
than the registry having to describe itself twice.

`serve` and the health endpoints live beside this adapter, which is the
application they serve. The core serves the API and
...
def build_app(
    *,
    ...
    caller already has the data directory (`DEPLOYMENT.md`). When it is
    present, every route resolves a principal and checks its scope before
    the handler runs, and the decision is recorded either way.
    """
    ...
    app = FastAPI(
        title="Vogt",
        version=__version__,
        # `serve` passes one, to run the collector schedule for
...
def _with_public_identity(ctx: AppContext, request: Request) -> AppContext:
    """Let a front door say where this request arrived.
...
def _endpoint_for(
    ...
    *,
    ...
    # Offload the synchronous handler off the event loop. FastAPI runs
