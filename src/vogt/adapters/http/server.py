...
`DEPLOYMENT.md`: the GUI, `/api`, `/mcp` and the health endpoints all
answer on the same listener. That is a deliberate inversion of the earlier
...
TLS can terminate **in-process** from an operator-owned certificate, so a
private-network listener holding its own certificate gains nothing from being
fronted; a public deployment puts a reverse proxy in front instead.
"""
...
class ServeOptions:
    ...
    examples. They encode exposure: a wrong default here is the
    `:18081` incident. The compose file supplies them, and the compose file
    is allowed to know the answer for its deployment.
    """
    ...
    #: Collect in the background while serving. The interval itself
    ...
    def validate(self) -> None:
        if not self.host.strip():
            msg = "serve needs a listen address; there is no default"
...
def build_server(
    ...
    *,
    ...
    def resolve(request: Request) -> tuple[AppContext, Authenticated]:
        """Derive the principal from authentication only.
    ...
    # First-run install mode: unauthenticated, self-closing, and
