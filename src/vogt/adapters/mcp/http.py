...
The remote transport. It reuses the stdio server's message handling
verbatim — one dispatcher, two transports — because the alternative is the
duplication an earlier product ended up with, where the same twenty tool signatures
were hand-mirrored across a server, a bridge and a registry.

Scope filtering happens here: `tools/list` returns exactly what this
...
def add_mcp_route(
    ...
    *,
    ...
    async def mcp(request: Request) -> JSONResponse:
        ...
        # is the slow part, so only it is offloaded.
...
def _call(
    ...
    *,
    ...
    # LOCAL_ONLY operations (restore/backup/serve/import/init/
    ...
    try:
        # The second gate, and it is recorded either way.
        authorize(
            ctx,
            caller,
    ...
    try:
        ...
        # Don't return the raw exception text to a remote caller: it
