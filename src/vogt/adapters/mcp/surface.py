...
MCP is the primary surface (ARCHITECTURE.md): agents are the expected first
users. What lives here is the part that matters for correctness — the tool
list and the dispatch, derived from the registry so no tool signature is ever
hand-mirrored. The stdio transport, the streamable-HTTP transport and the
`vogt-mcp-remote` bridge all sit on top of this surface rather than beside
it.
...
- **Identity is never a tool argument.** `call_tool` takes arguments and a
  ...
  caller could use to claim to be someone else.
...
#: call, and MCP clients send it on connect and sometimes repeatedly.
...
class McpSurface:
    ...
    def list_tools(self) -> list[McpTool]:
        ...
        — is not applied here: at this layer there is no authentication to
        filter by, and pretending otherwise would be a gate that gates
        nothing.

        Memoized per registry: the registry is static per process, so
