"""Write-back: additive, forward-only, and opt-in.
...
`create_repo` sits outside the per-project policy table because the
...
Comments flow **outbound only**. A comment authored here posts
...
class ForgeWriter:
    ...
    def update_issue_body(
        ...
        one the initiative projection needs: it replaces an issue *body*, never
        a title, never a
        ...
        not overwritten. "No deletion, no force" still holds: nothing is
