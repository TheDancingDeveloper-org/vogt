"""Cloning a repository, without ever writing the token down.
...
command line, no configuration file, and no stored URL. Secrets travel by
file reference rather than argv, and this is that rule applied at
...
class CloneOutcome:
    ...
    #: — re-running an import must not be destructive.
    reused: bool = False
...
def clone_repository(request: CloneRequest) -> CloneOutcome:
    ...
    if existing is not None:
        ...
        # is gated. A new folder never reaches
...
# -- publish: the read-only gate, and the one push --------------------
...
class PushRequest:
    ...
    The token travels the same `GIT_ASKPASS` road the clone's does:
...
def inspect_publish_source(root: Path) -> PublishSource:
    ...
    The same posture as `_enforce_import_parity` at the opposite
...
def push_branch(request: PushRequest) -> PushOutcome:
    ...
    "No force, ever" is guaranteed here the way the provider surface
...
def _reuse_existing(destination: Path, remote: str) -> CloneOutcome | None:
    """Decide what an occupied destination means.
...
def _enforce_import_parity(
    ...
    """Refuse a re-import onto a checkout that is not clean and at parity.
...
def _origin_head(
    ...
    the credential ever touching a command line.
    """
