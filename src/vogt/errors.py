...
class InstallClosed(Conflict):
    ...
    Install mode is active exactly while the token store holds no
...
class ForgeAccountsNotConfigured(InvalidRequest):
    ...
    an honest "not configured" is the only correct answer.
    """
...
class ImportParityRefused(Conflict):
    ...
    parity with origin, and Vogt performs no merge on the user's behalf.
...
class ImportWorkingTreeDirty(ImportParityRefused):
    """The destination's working tree has uncommitted changes.
...
class ImportBranchDiverged(ImportParityRefused):
    """The local default branch has diverged from origin.
...
class PublishRefused(Conflict):
    """`forge.publish` found a precondition missing, and names it.
...
class PublishSourceInvalid(PublishRefused):
    """The project root is not a publishable git checkout.
...
class PublishWorkingTreeDirty(PublishRefused):
    """The project's working tree has uncommitted changes.

    The same rule as the import parity gate, mirrored at the other
...
class PublishNonFastForward(PublishRefused):
    """The push was rejected as non-fast-forward, and was not forced.

    "No force, ever" applied to the one place a force flag would be
...
class RemoteRepoExists(PublishRefused):
    """The forge already has a repository by that name.
...
class NotLinked(Conflict):
    """A write verb needs a forge-linked project, and this one is not.

    Decision 10 of the upstream-truth pivot: on an unlinked project the work
    write
    ...
    publish it (`forge.publish`). Items with no project at all are
...
class LinkRefused(InvalidRequest):
    """`forge.link` found a precondition missing, and names it.
    ...
    credential (the acting actor's linked PAT, or the instance file token).
...
class UpstreamWriteRefused(Conflict):
    """Write-through was refused by policy before anything was sent.
...
class UpstreamWriteFailed(VogtError):
    """The forge rejected or failed a write-through, so nothing changed.
...
class MissingReason(InvalidRequest):
    ...
    `reason` is the only caller-supplied audit field (ARCHITECTURE.md); a write
