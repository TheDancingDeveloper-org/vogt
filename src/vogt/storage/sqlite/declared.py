...
is what keeps a Postgres backend a swap rather than a rewrite.
"""
...
#: (the audit log's project filter).
#:
...
class SqliteDeclaredStore:
    ...
    def publish_event(
        ...
        *,
        ...
        """Append an observed-side event (`SCHEMA.md` §2.5).
        ...
        keep their promise never to write the declared store.
        """
    ...
    def write(self) -> Iterator[SqliteWriteTxn]:
        """Open one atomic declared write.
...
class SqliteReadView:
    ...
    def __init__(self, conn: sqlite3.Connection) -> None:
        ...
        #: identical workflow_defs queries + JSON parses per render.
    ...
    def drift_evidence_ids(self) -> frozenset[str]:
        """Observation ids any proposal references, of any status."""
        rows = self._conn.execute(
...
class SqliteWriteTxn(SqliteReadView):
    ...
    def upsert_workflow(self, workflow: Workflow, *, at: datetime) -> None:
        ...
        # Keep the per-view workflow cache honest if this same txn
    ...
    def upsert_forge_account(
        ...
        *,
        ...
        # because SQLite-only semantics are forbidden above the backend.
        cursor = self._conn.execute(
...
def _work_where(work_filter: WorkFilter) -> tuple[str, list[object]]:
    ...
    if not work_filter.include_superseded:
        # A native row that migrated upstream is retired — the
    ...
    if work_filter.exclude_unlinked_native:
        # The forge-less surface withdrawal: a native row on an unlinked project is
