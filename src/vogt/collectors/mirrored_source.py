...
The shape this exists for (ARCHITECTURE.md): **rustnzb** vendors
...
diff them, and never says one is behind — all three are forbidden, because
answering "have these diverged" needs resolved versions, and the subsystem
that would resolve them was deliberately deleted. The declared versions on
each side are
...
free. The alternative — hashing trees — is the content comparison this
collector rules out. A name that two registered projects both claim is
dropped rather than guessed at: an ambiguous match is not a finding, and
reporting one would put Vogt in the business of asserting which copy is real.

**Why a collector of its own rather than more of `dep-refs`.** Coverage. A
sweep records which collector looked at what, and folding this into
`dep-refs` would leave no way to tell "no mirrors here" from "nothing looked
for mirrors" — the distinction the observation layer exists to keep.
...
#: `dep-refs` parses, for the same reason (ARCHITECTURE.md).
...
class MirroredSourceCollector:
    """Path members that are also somebody else's published source."""
        ...
        for path in walk_project(root, exclusions=exclusions):
            ...
            yield finding(
                ...
                payload={
                    ...
                    # is owed them; never compared here.
    ...
    def _published(self) -> dict[str, _Published]:
        ...
        which copy is the real one — exactly what this collector refuses to
        make.
        """
