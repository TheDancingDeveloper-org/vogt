"""`session-outcomes` — what a run left behind.
...
more honest than two that say it twice.
...
`provisional: true` — the rule written into the evidence rather than
...
class SessionRecord:
    ...
    , so the application layer reads the row, resolves the work-item
...
class SessionOutcomeCollector:
    ...
    def __init__(self, engine: EngineClient, sessions: DeclaredSessions) -> None:
        ...
        # stale copy of somebody else's running state that must never be
        # written.
    ...
    def requires_network(self) -> bool:
        ...
        # still another process over HTTP, and the forge-less layer
    ...
    def _outcome_of(self, project: Project, record: SessionRecord) -> Finding:
        ...
        if running:
            ...
            # A claim backed by a still-running session is
    ...
    def _binds(self, task: EngineAgentTask, project: Project) -> bool:
        ...
        # every work item that can host a session has one, and
...
def _delta(
    ...
    **A closed window** — the session ended and something recorded when —
    ...
    to how often we look rather than to what changed. What a
