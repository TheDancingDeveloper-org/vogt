...
One of the core collectors, with one difference from the
others: **it runs on demand only.** Nothing re-checks compliance on a timer
(deliberately deferred). The contract is evaluated when somebody asks, and the
    ...
    def collect(self, ctx: CollectorContext, project: Project) -> Iterable[Finding]:
        ...
        yield finding(
            ...
            payload={
                ...
                # The rules *evaluated*, not only the ones that failed.
