...
The coverage record is the point. A sweep row says which collector
looked at which projects, when it started and finished, and how it went — and
that is what makes "absent" different from "not collected". Most of
an earlier product's "missing" drift turned out to be collector-coverage artefact; this
...
eleven, and it never touches declared data.
"""
