...
class GitLocalCollector:
    ...
    def collect(self, ctx: CollectorContext, project: Project) -> Iterable[Finding]:
        ...
        if describe:
            ...
            # drift; this collector only reports.
            yield finding(
                kind=KIND_TAG,
        ...
        # Where work is in git, reported never driven: one
        ...
        # observed ones are compared elsewhere and disagree as drift;
...
def _branch_findings(
    ...
    stable, so re-observing an unchanged branch writes nothing, and
...
def tracked_names(root: Path) -> frozenset[str] | None:
    ...
    filesystem rather than failing every criterion at once.
