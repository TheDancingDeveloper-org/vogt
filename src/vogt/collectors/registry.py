...
def core_collectors() -> list[Collector]:
    ...
    `sweep` does not run it: nothing re-checks compliance on a timer
    (deliberately deferred). Naming it explicitly runs it.
    """
    return [
        GitLocalCollector(),
...
class CollectorRegistry:
    ...
    def select(self, names: tuple[str, ...], *, offline_only: bool) -> list[Collector]:
        ...
        without it, the scheduler would quietly reintroduce the continuous
        re-checking that was deliberately deleted.
        """
        if names:
            return [
                collector
