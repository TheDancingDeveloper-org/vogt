...
Two rules shape everything here (ARCHITECTURE.md):
...
**A proposal must never outlive its evidence**. Every proposal
...
#: Local drift kinds. The forge kinds — `forge_state_mismatch`,
#: `vanished_upstream`, `ci_red_vs_healthy`, `update_automation_gap` —
#: belong to the forge module.
VERSION_MISMATCH = "version_mismatch"
UNRESOLVED_DEPENDENCY = "unresolved_dependency"
BROKEN_PATH_DEPENDENCY = "broken_path_dependency"

#: The shipped default policy: low-risk auto-accept (ARCHITECTURE.md).
...
class DriftFinding:
    ...
    def auto_acceptable(self) -> bool:
        ...
        . The old one-direction-only rule was about evidence, not risk:
...
def version_mismatch(
    *,
    ...
    """Declared version vs the newest tag or release actually seen."""
...
def unresolved_dependency(
    *,
    ...
    Reported rather than resolved. The reference keeps its raw
...
def _last_observed_open(
    ...
    Once the incremental sync reads `state=all`, an open observation is
...
#: and no longer matches what the member's workflow state says. The tick
...
def forge_state_mismatch(
    *,
    ...
    """A linked issue's state disagrees with the item's (ARCHITECTURE.md)."""
    return DriftFinding(
...
def initiative_checkbox_drift(
    *,
    ...
    """A tracking-issue checkbox disagrees with the member's state.
...
def vanished_upstream(
    *,
    ...
    Coverage-gated on purpose. Absence outside a completed sweep is
    "not collected", and most of an earlier product's "missing" drift turned out to be
...
def issue_references(text: str) -> list[str]:
    """Forge subject keys a work item's own text names.
...
def referenced_issue_state_mismatch(
    *,
    ...
    """A work item and the issue its own text names disagree.
...
def update_automation_gap(
    *,
    ...
    """One of the three automation facts is off.
