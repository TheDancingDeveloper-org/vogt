...
This exists first so that `project create` can scaffold a compliant
skeleton: it has to know what compliant *looks* like. Evaluating the
contract and recording a compliance status lands here too — as
configuration carrying a version identifier, so a recorded status can name
...
barrier you pass (ARCHITECTURE.md, §5).
"""
...
class Contract:
    """What a compliant project looks like (ARCHITECTURE.md)."""
...
def contract_from_settings(
    *,
    ...
    """Build the contract this instance evaluates against.
    ...
    **The version is derived when it would otherwise lie.** The contract is
    sourced from configuration *and* carries a version identifier a
...
class CriterionResult:
    ...
    A contract check returns the specific rules evaluated and the
    ...
    there is the case this field exists for: present on one disk,
...
#: honest than a silently refreshed `compliant` (ARCHITECTURE.md).
COMPLIANT = "compliant"
NON_COMPLIANT = "non_compliant"
NOT_CHECKED = "not_checked"

#: The fourth answer, and the only one that is never stored on a project row
#: This project never adopted the contract, so there is nothing to
...
def _exempted(
    ...
    """Re-read a criterion somebody declared inapplicable here.
...
def evaluate(
    ...
    *,
    ...
    """Check a folder or repository against a contract.
     ...
     Works on any path, registered or not, and blocks nothing
    . The result is a value to read.
     ...
     the criterion cannot apply here. Those criteria are still
...
def recommendations(
    ...
    """What would close each failing criterion of an evaluation.

    Advisory output. Nothing here applies anything, and nothing that consumes
    it may treat it as authority: the rule that compliance is never a
