"""Rendering an initiative as a forge tracking issue with a task list.

An initiative is vogt-local by decision (no milestones, no Projects v2),
which makes an epic invisible to anyone with only the forge open. The
...
def render_managed_region(
    *,
    ...
    `siblings` is the cross-project link set: the other
...
def parse_checkbox_states(body: str) -> dict[int, bool]:
    ...
    rendered, and surface the disagreement as drift.
    """
