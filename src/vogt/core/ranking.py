...
ARCHITECTURE.md: ordering is computed from constant, documented
...
#: Points for an item with an open pull request against it. Work in
...
#: Points for a branch committed to recently, decaying to nothing over a
...
#: Penalty for items whose declaration nothing has confirmed. Zero until
#: sweeps give trust states something to be computed from, and negative-only:
#: trust
...
class RankingInputs:
    ...
    #: Whether an open pull request implements this item. Derived from
    #: the observed `implemented_by` edge, never declared.
    open_pr: bool = False
    #: Age of the most recent commit on a branch bound to this item, in
    #: seconds. `None` where no branch was observed — a fact absent,
...
#: Inputs documented in ARCHITECTURE.md that cannot fire yet, recorded so that
