...
Two things here are easy to get half-right: transitions
...
#: The shipped default machine (ARCHITECTURE.md): open -> in_progress -> review
...
def check_completion_allowed(*, to_state: str, blockers: list[tuple[str, str]]) -> None:
    """`depends_on` blocks completion (ARCHITECTURE.md).

    Not an enforcement of compliance, trust or drift — those may never gate an
    operation. This is the declared meaning of the edge the user
