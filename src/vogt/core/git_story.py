...
branches are bound to it and which pull request implements it.
...
Everything here is **derived and read-only**: a phase is computed from
...
Task-run conclusion is a deliberate omission: it is an engine-side
...
def derive_drift(
    *, item_terminal: bool, pr_state: PrState | None, has_active_branch: bool
    ...
    """The contradictions between the item and its git evidence.
