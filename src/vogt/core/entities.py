...
#: never adopted the contract *reports*. The three stored values are
...
#: Whether a project's work model is the forge's (decision 1). Set by
#: an explicit act — `project.import`'s clone+consolidate, `forge.link`, and
#: `forge.publish` — never inferred per call from what tokens
...
#: — the forge sync reads it from a PR's closing keywords and branch
...
#: it is a fact the forge reports, never one typed in.
...
class Actor(Entity):
    """A human or an agent. Both are first-class."""
...
class Project(Entity):
    """One explicitly registered repository or folder."""
    ...
    #: When this project opted into the contract. Null is the
    ...
    #: Persisted, and set only by an explicit act — import's
...
class Label(Entity):
    """A free-form tag, shared per instance and GitHub-label aligned."""
...
class Initiative(Entity):
    """A cross-project epic, carrying weight that feeds ranking."""
...
class Relation(Entity):
    """One typed edge out of a work item."""
...
class Comment(Entity):
    """A remark on a work item, attributed to an actor."""
...
class WorkItem(Entity):
    ...
    #: Set when the native item migrated upstream on link/publish: the
...
class Suppression(Entity):
    """An audited decision that an observed subject is not work.
...
class ContractExemption(Entity):
    ...
    A criterion a project *has not met* and one it *cannot meet by
...
class WorkOverlay(Entity):
    """The vogt-local half of an upstream-truth work item (decision 2).
    ...
    #: The *declared* half of the branch binding — additive and
    #: forward-only: recording one never touches git, and it is kept
    #: separate from the `git.branch` observations a sweep collects so the two
    #: can disagree as drift rather than one silently overwriting the
    #: other.
    branches: list[str] = []
    created_at: datetime
    updated_at: datetime
...
class CodingSession(Entity):
    """A terminal the engine runs, and what Vogt started it for.
    ...
    other half of the registry-owned `cwd` rule: every session has a project
    and a `cwd` the registry recorded, and only some have an item.
    """
    ...
    #: What was *asked for*, not what the agent is using now. A
    #: session whose operator typed `/model` at the prompt has moved on and
    #: this field will not know — the same line drawn around activity: live
    #: state is the engine's, never a cached copy here.
    model: str | None = None
...
class Token(Entity):
    """A scoped credential bound to an actor.
...
class WriteBackRecord(Entity):
    """One thing Vogt said to a forge, and what came back.
...
class ForgeAccount(Entity):
    ...
    cleartext columns a status read needs without touching the key.
    """
...
class DriftProposal(Entity):
    """A machine-raised question, resolved by a human or an agent."""
   ...
   , and still carries the snapshot it was raised with. It says only
...
class Sweep(Entity):
    ...
    The thing that makes "absent" different from "not collected".
    Absence may only be asserted inside a scope a completed sweep
    provably covered.
    """
...
class Observation(Entity):
    """One immutable thing a collector found."""
...
class DepRef(Entity):
    ...
    version is resolved (r2 decision, ARCHITECTURE.md).
    """
...
class Event(Entity):
    """One row of the ordered notification feed.
