...
The acting principal is derived from *authentication only* and is
never a caller-suppliable field. The simplest authentication path is the
local one — the OS user running the process — which is exactly what
`DEPLOYMENT.md` prescribes for the loopback topology. Token-derived
principals plug in here too; nothing downstream needs to change,
...
def local_principal() -> Principal:
    ...
    identities that authenticate with their own credential, so that
