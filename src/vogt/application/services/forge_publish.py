"""`forge.publish` — create the remote, push, link, migrate.

The first verb in the product that *creates upstream state and pushes
commits* — everything before it was the deliberately non-destructive
write-back set. The bounded shape:
...
- **Requires clean, explicit local state.** `inspect_publish_source` is the
  read-only gate (the import gate's posture at the other boundary): a git
  repository at
...
- **The token is never written down.** The push authenticates through the
  same `GIT_ASKPASS` road the clone uses: the remote URL is the
...
name, never silently re-pointed. After the link, the native migration publishes
...
#: repo are placeholders, but they must still parse as valid names (`repo_of`
#: rejects non-name owner/repo), so `probe/probe` rather
...
def publish_project(ctx: AppContext, params: ForgePublishParams) -> ForgePublishResult:
    """Create a remote repository from a local project and link it."""
    ...
    if provider is None:
        raise PublishRefused(
            ...
            "link your own forge account (`forge account link`) or "
            "configure the instance token file, then retry"
        )
    # Same rule as `forge.link`: if open native items must migrate,
    ...
    # argv, or the checkout's configuration. Never --force: a
