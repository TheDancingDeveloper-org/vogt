...
def _snapshot(observation: Observation) -> EvidenceSnapshot:
    """Copy the evidence as it stands now.
...
def _referenced_issue_findings(
    ...
    """Issues a work item's own text names, against what was observed.
    ...
    - **Only what was observed.** No observation is "not collected", and
      raising drift from that is the mistake the not-collected rule exists
      to prevent.
    """
    findings: list[DriftFinding] = []
...
def _forge_findings(ctx: AppContext) -> list[DriftFinding]:
    """The forge-derived kinds: state mismatch, vanished upstream, red CI,
    posture."""
    findings: list[DriftFinding] = []
    with ctx.declared.read() as view:
    ...
    for item in items:
        for subject_key in links.get(item.id, {}):
            ...
            if not seen:
                ...
                # Without a completed forge sweep this is "not
                # collected", and raising it would be the exact mistake that
                # made most of an earlier product's "missing" drift an artefact.
    ...
    for project in projects.values():
        ...
        if posture:
            ...
            # Three independent facts, named individually. `None`
...
def _initiative_findings(ctx: AppContext) -> list[DriftFinding]:
    """A tracking-issue checkbox ticked upstream out of step with its member.
...
def _raise_proposal(
    ...
    """Insert one proposal as the declared write it is.
    ...
    def body(txn: WriteTxn, actor: Actor) -> WriteOutcome[DriftProposal]:
        ...
        # `project` is additive: it lets the engine's agent-task trigger
...
def _reconcile_open_proposals(
    ...
    """Re-validate the board against the evidence this run just read.
    ...
    **Not auto-close, deliberately.** Resolution stays with a human or
    an authorised agent, and the GUI refuses even bulk *accept*
    ...
    **Coverage-gated, deliberately.** Silence is only meaningful inside a
    sweep that provably covered the subject. A proposal is marked
    ...
    for proposal in board:
        ...
        # or its retirement path is dead (D4).
...
def detect_drift(ctx: AppContext, params: DriftDetectParams) -> DriftDetectResult:
    """Compare declared state against observation and raise proposals.
    ...
    # the confusion the not-collected rule exists to prevent.
...
def _projects_without_evidence(ctx: AppContext) -> list[str]:
    """Registered projects no collector has ever swept.
...
def resolve_drift(ctx: AppContext, params: DriftResolveParams) -> DriftResult:
    """Accept, reject, or leave contested — always audited.
