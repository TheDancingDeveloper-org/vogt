...
One rule shapes this whole module: **no component may consume
...
And nothing re-checks on a timer. `contract check` runs when somebody
...
def contract_check(ctx: AppContext, params: ContractCheckParams) -> ContractCheckResult:
    ...
    # against it. The answer is still an answer — it says which
...
def compliance(ctx: AppContext, params: ComplianceParams) -> ComplianceResult:
    ...
    # The fourth answer, and the one that is never stored. A project
...
# -- adoption, and the declarations around it ------
...
def contract_adopt(ctx: AppContext, params: ContractAdoptParams) -> ContractAdoptResult:
    """Opt a project into the contract.
...
def contract_decline(
    ...
    """Opt a project back out of the contract.
