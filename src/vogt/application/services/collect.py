...
registered project, and publishing the sweep-completion event into
the declared feed on the collectors' behalf.
"""
...
def collector_registry(ctx: AppContext) -> CollectorRegistry:
    ...
    are none". `session-outcomes` follows the same rule
...
class _RegisteredProjects:
    ...
    is: collectors never read declared data. The collector is handed
...
class _DeclaredSessions:
    ...
    data (`collectors/__init__.py`). The collector is handed flat
...
def sweep(ctx: AppContext, params: SweepParams) -> SweepResult:
    ...
    Scope is always the registered project list — optionally
    ...
    try:
        if total_new == 0:
            ...
            # schedule, was ~2M redundant row writes/day at scale.
        ...
        # Overwrite it to `failed` before the exception
...
def _resolve_target(
    ...
    *,
    ...
    reported as `unresolved_dependency` drift — it is usually a project
...
def observations(ctx: AppContext, params: ObservationsParams) -> ObservationsResult:
    ...
    decision hides them from views, it does not delete evidence (ARCHITECTURE.md).
    """
    if not ctx.observed.has_evidence_tables():
...
def deps(ctx: AppContext, params: DepsParams) -> DepsResult:
    """References out of a project, and — reversed — into it.
