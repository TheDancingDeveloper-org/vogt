"""Connecting a client, and the document `DEPLOYMENT.md` wanted.

The gap this closes is narrow and was invisible for a long time. `/health`,
`/version` and `/connection-info` all exist and are tested, and
...
value and therefore carries no default, and when it is unset this
...
def _http_config(mcp_url: str | None, fmt: str, versions: list[str]) -> str:
    ...
    is no version to skew (the protocol-version negotiation exists because the
    other path has one) and
    ...
    if fmt == "markdown":
        return "\n".join(
            [
                ...
                "URL, which ends up in logs, proxies and history.",
                "",
                "```console",
...
def _bridge_config(url: str | None, fmt: str) -> str:
    ...
    protocol versions from `/connection-info` at startup, so the only
    ...
    if fmt == "markdown":
        return "\n".join(
            [
                ...
                "- Token: `VOGT_TOKEN_FILE`, a file path — never argv, never a URL.",
            ]
        )
    return json.dumps(
        {
