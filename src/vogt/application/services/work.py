...
The write plane has two shapes, split by the project's persisted link state
(decision 1):
...
def _write_through_gaps(ctx: AppContext, project: Project, action: str) -> list[str]:
    """Every unmet precondition for a write-through, named.
...
def _require_permitted(
    ...
    *,
    ...
    the first, so following the advice cannot still fail on a gate the
...
def _upstream_writer(
    ...
    trip never runs while a write lock is held. Reuses the forge-account
    identity selection: the acting actor's linked PAT when there is one, else
    the instance file token.
    """
    with ctx.declared.read() as view:
        actor = view.actor_by_identity(ctx.principal.identity_ref)
...
def _upstream_record(
    ...
    *,
    ...
    """The write-back ledger row for one successful write-through."""
...
def create_work(ctx: AppContext, params: CreateWorkParams) -> WorkResult:
    """Create a work item — upstream on a linked project.

    Split by the project named and by `local_only`:
    ...
    - a project, `local_only` → a native declared item *in* that project,
      ...
      migrates upstream. Reachable on linked and unlinked projects
...
def _create_native(
    ...
    """A native declared item, optionally scoped to a project.

    `project is None` is the plain no-project create. `project` set is the
    ...
    `work.adopt` and the native-migration population already speak.
    """
...
def _create_upstream(
    ...
    the issue like anything else; the reverse window — a local item
...
def get_work(ctx: AppContext, params: GetWorkParams) -> WorkResult:
    """One work item, with what is running for it.
    ...
    A subject key resolves here like a `WI-n` ref does (design §6):
    ...
    with ctx.declared.read() as view:
        ...
        # Where this item is worked on in git: declared branches from
        ...
        # The derived git story: the branches above joined to the
        # observed PR edge into a phase shown beside the workflow state,
...
def list_work(ctx: AppContext, params: ListWorkParams) -> WorkListResult:
    """Work items in scope: declared rows, plus upstream-truth items.
    ...
    An **unlinked** project scope answers with the CTA marker: empty items
...
def _reproject_membership(
    ...
    """Re-render the task list of any initiative this update joined or left.
...
def _update_upstream(
    ...
    Title/body are upstream truth vogt does not edit (the write-back verb set
    has no edit), and an item cannot move project. Priority, effort, assignee and
    ...
    if params.title is not None or params.body is not None:
        raise InvalidRequest(
            ...
            "or bodies on a linked project (the write-back verb set has no "
            "edit) — edit it on the forge"
        )
    ...
    if params.remove_labels:
        raise InvalidRequest(
            ...
            "append-only: labels can be added upstream, never "
...
def _transition_native(ctx: AppContext, params: TransitionWorkParams) -> WorkResult:
    def body(txn: WriteTxn, actor: Actor) -> WriteOutcome[WorkResult]:
        ...
        # other. Reaching a terminal state closes upstream; leaving
...
def relate_work(ctx: AppContext, params: RelateWorkParams) -> WorkResult:
    """Add a typed edge between two work items, cross-project.
    ...
    of v1. An upstream-truth ref is refused with the reason named rather
    ...
    def body(txn: WriteTxn, actor: Actor) -> WriteOutcome[WorkResult]:
        ...
        if params.kind not in DECLARABLE_RELATION_KINDS:
            # `implemented_by` is observed, never declared: the forge
    ...
    try:
        ...
        if upstream.resolve_upstream(ctx, view, ref) is not None:
            raise InvalidRequest(
                ...
                "upstream issues are not supported in v1 of the upstream-truth "
                "model — relate them on the forge"
            ) from None
        raise


# -- the declared branch binding ------------------------------------
...
def bind_branch(ctx: AppContext, params: BindBranchParams) -> WorkResult:
    ...
    declared field. Additive and forward-only — Vogt never creates,
    renames or deletes a branch; it only records the convention so a
...
def comment_work(ctx: AppContext, params: CommentParams) -> CommentResult:
    """Add a comment attributed to the acting actor.
    ...
    On an upstream-truth item the comment *is* the upstream comment:
...
def _comment_native(ctx: AppContext, params: CommentParams) -> CommentResult:
    def body(txn: WriteTxn, actor: Actor) -> WriteOutcome[CommentResult]:
        ...
        # Outbound only: a comment authored here posts upstream; a
...
def _comment_upstream(
    ...
    def body(txn: WriteTxn, actor: Actor) -> WriteOutcome[CommentResult]:
        ...
        # comment's home, and the mirror is how it is read back.
        comment = Comment(
            id=ctx.id_factory("cmt"),
...
def _transition_summary(
    ...
    """The `work.transitioned` event summary.
