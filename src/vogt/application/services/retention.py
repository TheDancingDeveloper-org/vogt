"""Retention over the observed store.
...
1. **The newest observation per subject is kept indefinitely.** Digest dedup
   means a stable subject's newest row can be far older than the
   history window — a repository nobody has touched in a year still has a
   current answer, and age alone must never prune it.
2. **Anything a drift proposal references is kept**. The exemption is wired
   into retention itself, because retention that runs before the exemption
   exists is how evidence goes missing.
...
def _protected_observation_ids(ctx: AppContext) -> frozenset[str]:
    """Observations pinned by a drift proposal.
...
def prune(ctx: AppContext, params: PruneParams) -> PruneResult:
    ...
    # Retention also caps the auth-decision telemetry, which otherwise
    ...
    # a row retention has just removed.
