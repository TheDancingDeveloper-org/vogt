"""Native-item migration on link/publish (design §4, decision 7).
...
the write-back ledger, audit rows keyed by its id — and deleting it
...
`work_links` row is written for the new subject: the upstream-truth dedup reads
...
def open_native_items(view: ReadView, project: Project) -> list[WorkItem]:
    ...
    *object* are excluded too: they are the pre-upstream-truth bridge —
    upstream identity exists, write-back already speaks to it, and the dedup
...
def require_migratable(project: Project, pending: list[WorkItem]) -> None:
    ...
    Migration creates issues upstream, so it is governed by the same
    write-back policy gate `work.create` honours. Checking before the project links —
...
def migrate_open_native_items(
    ...
    *,
    ...
    """Publish each open native item upstream and re-key it.
