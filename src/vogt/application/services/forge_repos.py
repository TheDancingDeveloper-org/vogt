...
After an actor links their forge PAT, the
...
the "nothing discovers" rule: a person still names what they import, and
...
: the acting actor's linked PAT for the host when they have one — which
works even with no instance file token — and the file token otherwise.
...
def _reader_provider(
    ...
    Mirrors the write path's attribution choice so the picker and the
