"""Upstream-truth work items: the observed mirror joined to the overlay.

On a **linked** project (decision 1) the work items *are* the
project's observed forge issues: the mirror the sync collectors maintain
...
def is_linked(project: Project | None) -> bool:
    """Whether this project's work model is the forge's.
...
def build_item(
    ...
    *,
    ...
    prose lives, and vogt does not edit bodies on linked projects.
    """
...
def upstream_items(
    ...
    *,
    ...
    is the item, and emitting both would be the double-count the upstream-truth
    model exists to end. Callers filter with `matches`; ordering follows the
    declared list's `(created_at, ref)` so merged pages page stably.
    """
