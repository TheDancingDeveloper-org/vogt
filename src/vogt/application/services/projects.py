...
def guard_remote_root_path(ctx: AppContext, root_path: str) -> None:
    """Constrain a caller-supplied `root_path` on the remote transports.
     ...
     which is the operator-owned allowlist.

     The principal is authentication-derived and never caller-suppliable
    , so the `local:` scheme reliably marks the loopback/CLI surface.
    """
...
def _new_project(
    ...
    *,
    ...
    `adopts_contract` is false for a project Vogt is *handed*: the
    ...
    return Project(
        ...
        # Registration never links: `linked` is set only by the acts
        # that establish upstream truth — import's clone+consolidate here via
        # this parameter, `forge.link`, and `forge.publish`.
...
def register_project(ctx: AppContext, params: RegisterProjectParams) -> ProjectResult:
    ...
    Registration is never refused on contract grounds — the contract
...
def record_registration(
    ...
    *,
    ...
    lose the only record that a clone happened.
    """
...
def _lay_scaffold(
    ...
    """Write a scaffold into a directory, never overwriting.
...
def create_project(ctx: AppContext, params: CreateProjectParams) -> CreateProjectResult:
    """Scaffold a contract-compliant skeleton, then register it.
    ...
    registered = record_registration(
        ...
        # contract; a project handed to `project register` is not.
...
def brief_project(ctx: AppContext, params: ProjectBriefParams) -> ProjectBriefResult:
    ...
    answers, and a shape that changes once collectors run is a shape every
    ...
    return ProjectBriefResult(
        ...
        # No view may imply a fault in a project that never adopted
...
def _observed_version(ctx: AppContext, project: Project) -> str | None:
    """The newest tag or release a collector has seen.

    Reported next to the declared version rather than instead of it. The two
    disagreeing is `version_mismatch` drift, which `drift detect` turns into a
    proposal; here it is simply a fact the brief states.
    """
...
def _ci_summary(ctx: AppContext, project: Project) -> CiSummary:
    """The CI story for the newest observed revision.
...
def transition_project(
    ...
    """Move a project through its lifecycle, validating the edge."""
