...
Every write-back action is audited locally and re-observed on the
...
#: One consolidation run, attributed to whoever asked for it.
...
def writer_for(ctx: AppContext) -> ForgeProvider | None:
    ...
    provider's append-only write surface now, not a GitHub-specific
...
def attempt(
    ...
    *,
    ...
    # The whole write goes through the provider surface now: it owns the
    # key scheme (`number_of`), the append-only verbs, and — being resolved
    # from the project's own `repo_url` — which forge to speak to.
    #
    # Attribution: when the acting actor has linked their own PAT for
    # this host, speak upstream as *them*; otherwise the instance file token
    # is the fallback, which is what every sweep and unlinked actor
...
def _writer_provider(
    ...
    `actor` may be `None` for the write-through path, which resolves
...
def _detail_with_identity(detail: str | None, writer_identity: str) -> str:
    ...
    write went up as an actor or as the file token. Skipped attempts
...
def set_write_back(ctx: AppContext, params: SetWriteBackParams) -> object:
    """Set a project's write-back policy.
...
def onboard(ctx: AppContext, params: OnboardParams) -> OnboardResult:
    """Consolidate a repository's existing state, read-only.
    ...
    # Onboarding is "reset the watermark and sync now": the same
    ...
    # every collector here only ever reads.
    now = ctx.clock()
