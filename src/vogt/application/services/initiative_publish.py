"""`initiative.publish` — an initiative as a forge tracking issue.

An initiative is vogt-local by decision, so an epic is invisible
...
def reproject_initiative(ctx: AppContext, initiative_id: str, *, reason: str) -> None:
    """Re-render existing tracking issues after a membership change.
...
def _members_in(
    ...
    Read the upstream-truth way: the members are the observed issues
