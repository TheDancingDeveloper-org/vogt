...
Two collected facts meet here: the branches bound to the item and the
pull request the `implemented_by` edge points at. Read back together
...
Everything here is derived and read-only: nothing is written onto the
...
class GitSignals:
    """The git-activity inputs the ranker reads, computed once per scope.
...
def git_story_for(
    ...
    Reuses the branch views already assembled for the item rather than
    re-reading them, finds the pull request that implements it, derives
    ...
    return WorkItemGitStory(
        ...
        pull_request=pull,
        drift=drift,
        # The task-run conclusion is an engine-side record not surfaced
...
def _pull_request_for(
    ...
    """The pull request that implements `item`, if one was observed.
...
def _edge_provenance(
    ...
    Reads the observed `implements` edges: a subject-key match binds
