"""`forge.link` — the explicit act that makes a project upstream-truth.
...
clone+consolidate, or through `forge.publish`. What this validates is
...
Linking also **migrates**: every open native work item the
...
not a toggle, and it is out of scope by decision.
"""
...
def link_project(ctx: AppContext, params: ForgeLinkParams) -> ForgeLinkResult:
    ...
    # exactly when `[forge_token_files]` names it.
    ...
    if provider is None:
        msg = (
            ...
            "(`forge account link`) or configure the instance "
            "token file, then retry."
        )
        raise LinkRefused(msg)
    # Before the link commits, not after: a policy that will refuse the
    # migration's creates must refuse the whole act, or the project
