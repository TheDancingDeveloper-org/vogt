...
#: the scale envelope this is the query the benchmark fixture watches.
...
def freshness_of(ctx: AppContext) -> Freshness:
    """How old the evidence behind an aggregating answer is.
...
def trust_for(
    ...
    *,
    ...
    """Compute a trust state; never hand-set (SCHEMA §4).
    ...
    Confirmation is "last seen", not "last changed": a forge
...
class _Candidate:
    ...
    #: Git-activity ranking inputs: an open PR and a recently-committed
...
def _observed_candidates(
    ...
    *,
    ...
    `refined` carries the upstream-truth join: for a subject on a
...
class _Gathered(NamedTuple):
    ...
    #: the withdrawal of the forge-less work layer, counted rather than
    ...
    *,
    ...
    with ctx.declared.read() as view:
        ...
        work_filter = WorkFilter(
            ...
            # The forge-less withdrawal: an unlinked project's native rows are not
        ...
        # Git-activity ranking inputs, gathered once for the scope: a
        ...
        if include_observed and ctx.observed.has_evidence_tables():
            ...
            # outstanding, and the closed filter now runs in SQL. Before
            ...
            # PRs stay in the backlog by default; excluding them is a
            ...
            # twice would double-count the work it represents. The declared
            # half of an *unlinked* project is withdrawn from the
            ...
            # The upstream-truth join: on a linked project a forge
            ...
            # Backlog collapse: a PR observed to *implement* a work item
...
def candidate_population(
    ...
    *,
    ...
    """Backlog-candidate counts for a Board scope.
...
def _unlinked_scope(ctx: AppContext, project: str) -> BacklogResult | None:
    """The link-or-publish CTA answer for an unlinked project scope, or `None`.
...
def backlog(ctx: AppContext, params: BacklogParams) -> BacklogResult:
    ...
    Paged with `limit`/`offset` against `total_considered`. Ranking is
    ...
    An unlinked project scope answers with the CTA marker instead of a
...
def bugs(ctx: AppContext, params: BugsParams) -> BacklogResult:
    ...
    Paged like `backlog`, and for the same reason — including the
    CTA marker for an unlinked project scope and the exclusion count.
    """
    if params.project is not None:
        unlinked = _unlinked_scope(ctx, params.project)
        if unlinked is not None:
...
def why(ctx: AppContext, params: WhyParams) -> WhyResult:
    """Per-input score contributions for one ranked entry.
