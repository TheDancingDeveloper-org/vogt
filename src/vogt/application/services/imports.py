"""Importing a repository that lives on a configured forge.
...
#: `forge.import` lands the same clone+register+consolidate as
...
#: under my credential" are different provenance answers.
...
def import_project(ctx: AppContext, params: ImportProjectParams) -> ImportProjectResult:
    ...
    token when one is configured. Public repositories can clone
    without a token. The clone, register
    and consolidate themselves are `import_from_ref`, which `forge.import`
    shares — the only difference between the two verbs is how the
...
def import_from_ref(
    ...
    *,
    ...
    """
    # A remote caller may not clone into an arbitrary server path;
    ...
    registered = record_registration(
        ...
        # that make a project upstream-truth: its work items are its
