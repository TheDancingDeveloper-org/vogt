...
`observations prune`. Both carry a `reason`, and both used to
...
Collectors still never write the declared store. What is recorded here
...
the fixed reason "scheduled sweep" and no person to name. Auditing it
...
what a collector's record should be. `test_a_sweep_writes_no_audit_rows`
holds that line.
"""
...
def audited_action(
    ...
    *,
    ...
    guaranteed for declared writes holds here too.
    """
    cleaned_reason = validate_reason(reason)
    with ctx.declared.write() as txn:
        actor = ensure_actor(ctx, txn)
