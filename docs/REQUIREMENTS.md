...
| **FR-U25** | S | Measured content-sized windows serve Board and Backlog, Board cells read bounded snapshot-stable server pages, and a clipped Board card now expands its title and body in place under the same measurement. Backlog rows still clip to fixed single-line columns with no in-place expansion, and a large-estate browser proof remains. | A ranked row whose title and `why` factors are unreadable cannot be judged from the list, and a DOM window over an unbounded read still fails the scale contract. |
