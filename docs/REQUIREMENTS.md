...
2. **Cadastre and MCP tooling become opt-in everywhere.** The engine image
   ...
   when `CADASTRE_MCP_ENABLED=1`, and report the disabled state as
...
| FR-E9 | *(r9)* The engine shall remain bootable with vogt-core absent, degrading to plain sessions — absence of the core costs Vogt features, never session availability (an optional upstream's degrade-rather-than-fail rule, applied inward). | S | MERGE §11.2 |
...
| FR-T14 | *(r18)* Every assistant interaction shall be recorded to a durable log in **both directions**, surviving restart and attributable to the actor who drove it: the recognised utterance and, where FR-T13's repair pass changed it, the repaired form alongside the raw one; the composed request; the model's reply; every tool call with its arguments; every tool result; and every FR-T2 pending action with its outcome — **approved, denied, or expired**. The log shall complement FR-T3 rather than replace it: FR-T3 audits the write an assistant caused, and this records the conversation that did or did not cause one, a distinction that today leaves every refusal and every un-acted question unrecorded. **Audio shall not be stored** — FR-T12's rule stands and this log is text and structure. External content in a logged entry stays delimited as untrusted data per FR-T4, including when the log is later read back to a model. Reading the log shall be scope-gated (FR-S3) and its retention horizon configurable. **Delivered:** an engine-local append-only SQLite log at `state_dir/assistant-log.db` (`engine/server/src/assistant_log.rs`), written by the tool loop — utterance with its repaired form, request, reply, every tool call and result, and every pending action's proposal and outcome (`approved`/`denied`/`expired`), each attributed to the front-door actor who drove it and durable across restart. Reads go through the scope-gated `GET /api/assistant/log` (`assistant` capability, `auth::required_capability`), returning structured entries with external content still delimited. Engine-local so an absent core costs it nothing (FR-E9). Retention is `assistant_log_retention_days` (default 30), enforced on a daily sweep. Evidence in §6.1. | M | r18; FR-T3, FR-T4, FR-T12, FR-S6 |
...
| **FR-T14** | The interaction is durably logged in both directions, survives a restart with actor attribution, and a denied *and* an expired pending action both appear with their outcome; a repaired utterance carries its raw and repaired forms; reading the log is scope-gated | `engine/server/src/assistant_log.rs` (`records_and_reads_back_newest_first`, `a_repaired_utterance_carries_both_forms`, `survives_a_reopen_of_the_same_directory`, `filters_by_actor`); `assistant.rs::the_interaction_log_survives_a_restart_with_actor_attribution` (drops the runtime mid-conversation and reads it back from a fresh handle on the same dir), `::a_denied_pending_action_is_logged_with_its_outcome` and `::an_expired_pending_action_is_logged_with_its_outcome` (each asserts `["proposed", <outcome>]`, so dropping either the proposal or the outcome turns it red), `::a_repaired_utterance_logs_raw_and_repaired_through_the_runtime`, `::a_typed_turn_logs_a_request_and_no_utterance`; `auth.rs::maps_mutating_routes_to_capabilities` asserts `GET /api/assistant/log` takes the `assistant` capability while the in-memory `history` read stays ungated. Engine-local, so `vogt_core.rs`'s coreless suite records and serves it with no core at all (FR-E9) |
...
| **FR-G16** | The contract is opt-in: a project Vogt was handed reports `not_applicable` and never a fault, a check against it records nothing, and adoption and withdrawal are audited declarations. Creating a project *is* the opting in | `tests/test_contract_adoption.py` (six). The row worth keeping is `test_a_project_that_declined_is_never_refused_anything`: FR-G16 must not become the gate FR-G13 forbids, and a rule about who is measured is one refactor away from being a rule about who is served |
| **FR-G17** | The scaffold `project create` writes is reachable against an already-registered project, never overwrites, and reports created and skipped exactly as creation does | `tests/test_contract_adoption.py` (three). *Skipped means untouched* is asserted on the file's bytes, not on the word: that is the whole guarantee that makes offering the scaffold to somebody else's repository safe |
| **FR-G18** | Every failing criterion carries what would close it, and the two kinds of remedy are distinguished: what `project scaffold` performs, and what a person or an agent has to decide | `tests/test_contract_adoption.py` (two). One of them asserts that evaluating a folder writes nothing into it — a recommendation that applied itself would be the gate again, wearing the costume r2 removed |
| **FR-G19** | A criterion declared unmeetable is reported with its reason and its author, is not counted as failing, is absent from the recommendations, and can be withdrawn | `tests/test_contract_adoption.py` (four). The last of them covers the stored answer: a verdict recorded before the declaration must stop being read as a failure afterwards |
...
| **NFR-D12** | `dev` builds `:dev` images, `main` builds `sha-<commit>` and `prod` builds `prod-<sha>`, for the core-only image **and** the merged stack, with no alias any of them can move | `tests/test_deploy.py::test_the_three_streams_are_kept_apart`, parametrized over both jobs — a rule kept by the core image and dropped by the merged one would leave the artefact the merge exists for as the unlabelled stream. It also refuses a negated ref condition outright: `sha-` was once "anything that is not dev", which was correct for two streams and silently put prod's images into main's the moment a third existed. `test_every_building_branch_has_a_tag_rule` pins the other half — a branch that builds with no rule of its own |
...
pass asserted: `test_the_three_streams_are_kept_apart` is parametrized over the
...
| **The restructure conformance demo** — Inbox cursor continuation while new facts arrive, stable places and migration, measured Board/Backlog content, and the full route matrix | M11 | A browser with CSS and a fixture estate | Partial. The browser suite now covers the complete Stage 3 route matrix on a phone, a bounded Board cell continuing from its server cursor, measured Board cards and Backlog rows, a crowded rail's Files reachability, phone target and font floors, first-viewport composition and a waiting session's terminal input. Inbox high-water continuation while new facts arrive, and a large measured estate rather than a fixture, still need evidence. |
