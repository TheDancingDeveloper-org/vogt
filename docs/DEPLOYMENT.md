...
| Integration | Purpose | Optional | When absent | Configured by |
|---|---|---|---|---|
| **Engine token and bind** | The engine's own bearer token (≥16 chars) and listen address | token required | The engine refuses to start | `--token`, `--bind` (default `127.0.0.1:8910`) and `--config` flags, or the config file. These three are the exception to the prefix rule: their environment forms are still read only under the legacy names `MYDEVENV2_TOKEN`, `MYDEVENV2_BIND`, `MYDEVENV2_CONFIG`. Scoped extra tokens: `ENGINE_EXTRA_TOKENS_JSON`; write-rate cap: `ENGINE_MUTATING_REQUEST_LIMIT_PER_MINUTE` |
