...
`ghcr.io/thedancingdeveloper-org/vogt:0.6.0`, is also published at every
...
| `VOGT_STACK_IMAGE` | no | `ghcr.io/thedancingdeveloper-org/vogt-stack:0.6.0` | The image to run. Pin a digest (§6). |
| `VOGT_VOICE_IMAGE` | no | `ghcr.io/thedancingdeveloper-org/vogt-voice:0.6.0` | The sidecar. Pin the same release as the stack. |
...
```console
docker buildx imagetools inspect ghcr.io/thedancingdeveloper-org/vogt-stack:0.6.0 \
  | grep -m 1 Digest
docker buildx imagetools inspect ghcr.io/thedancingdeveloper-org/vogt-voice:0.6.0 \
  | grep -m 1 Digest
```
