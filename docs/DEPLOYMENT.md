...
`ghcr.io/thedancingdeveloper-org/vogt:0.7.1`, is also published at every
...
```console
docker buildx imagetools inspect ghcr.io/thedancingdeveloper-org/vogt-stack:0.7.1 \
  | grep -m 1 Digest
docker buildx imagetools inspect ghcr.io/thedancingdeveloper-org/vogt-voice:0.7.1 \
  | grep -m 1 Digest
```
