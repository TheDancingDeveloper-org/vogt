...
Vogt ships as a generic product: one published stack — the `vogt-stack`
image (Python core, Rust session engine and Solid PWA behind one port) and
the `vogt-voice` sidecar, released as a pair — one Compose base
([`deploy/stack.compose.yml`](../deploy/stack.compose.yml)) that runs both
with voice on by default, and a configuration schema that decides everything
an operator is allowed to decide. It is also meant to be customised heavily —
the expected way to run it in an estate of your own is to fork this
repository and layer your estate on top of that image. This document names
the supported extension points, so that a large customisation is a documented
deployment rather than a divergent fork.
...
[Observing repositories on a host path](#observing-repositories-on-a-host-path) before pointing it
elsewhere.
...
markers enter backlog and bug views; the defaults are `TODO(vogt)` and
`FIXME(vogt)`. Every other marker is still observed, still queryable, still
counted — it just does not claim to be work. Widening this is how you drown
the ranked view.
...
The repository ships worked examples of this under
[`deploy/examples/`](../deploy/examples/): the custom-stack overlay mounts a
lifecycle hook bundle and pins a release pair, and the container-sessions
overlay adds a Docker socket and a named work volume. Each states only its
...
### Observing repositories on a host path
...
[`DEPLOYMENT.md`](DEPLOYMENT.md) has the worked command line for a second
...
The core image (`ghcr.io/thedancingdeveloper-org/vogt:0.6.0`) is a build
...
only when you are changing what the *core* carries; the contributor
workflow that builds from it is described in
...
Dockerfile, an overlay, and a lifecycle hook you can build and boot as-is. A
derivative of your own differs from it only in *which* tools and integrations
it adds, never in the mechanism. CI builds and boots the example with no
private integrations, so the contract stays honest.
...
**The loopback-core invariant survives extension.** The stack runs its own
core on loopback and the engine is the only front door. An extension adds
...
`VOGT_VOICE_IMAGE` (they are one release pair; see the upgrade section of
[`DEPLOYMENT.md`](DEPLOYMENT.md)), rebuild, redeploy. The core runs its
forward-only migrations against `vogt-data` on start. **Rollback is not
...
or restore `vogt-data` from a pre-upgrade backup. The rollback section of
[`DEPLOYMENT.md`](DEPLOYMENT.md) has the procedure.
...
A deployment of your own is the published `vogt-stack` digest, plus an
image layer that adds your integrations (a VPN client, a secrets broker,
another MCP server), plus a Compose overlay carrying your host mounts and
addresses. It is built entirely from the extension points above — image
extension, overlay, configuration — and nothing else. Neither the layer nor
the overlay belongs in this tree: a deployment tied to one operator's paths
and addresses lives in that operator's own repository.

Every deployment-specific value is an environment value, a mount, or a line
in your own Dockerfile — never a default baked into a file a stranger clones
— so a new release of the public image is a digest change in your fork and a
rebuild, not a merge. [`deploy/examples/custom-stack/`](../deploy/examples/custom-stack/)
is that shape with nothing private in it; `tests/test_public_delivery.py`
pins that it, and every other public deploy file, carries no host paths and
no maintainer integrations.
