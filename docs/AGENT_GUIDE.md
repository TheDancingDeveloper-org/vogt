...
read-only **git story**: a **phase** — `no_branch → branch_active →
...
- **Write-back is additive and forward-only.** Vogt's forge writes append —
  ...
  is **no force, ever**: Vogt does not delete branches, does not delete
...
- **Approval gates fail closed.** When you run work as a scheduled agent
