...
pasted link restores the view it names. Open panes persist per device.
...
- **A pull request collapses under the work it implements.** When a PR names a
  work item — through a `Closes #9` / `Fixes owner/repo#9` keyword in its body
  or title, or a branch named for the issue (`gh-9-…`, `wi-7/…`) — Vogt reads
...
git, it never writes the phase back onto the item. The phase is derived from
the branches and the PR edge alone; the engine's per-task run conclusion is
not an input to it.
...
**Voice works out of the box.** You do not need a paid speech provider to turn
voice on. The supported stack bundles a first-party `voice` sidecar — a local,
CPU-only, OpenAI-compatible Whisper transcriber and Piper voice with a small
default model set baked in — and wires the engine to it, so a fresh
`docker compose -f deploy/stack.compose.yml up -d --wait` transcribes the
microphone and speaks replies with nothing to configure. The first start takes a
little longer while the models load. To run without it, clear
`COMPOSE_PROFILES` in `deploy/.env`; to use a hosted provider instead, set
...
for it to exit. Its output is not in the archive's full-text index; search
reaches it through a bounded scan of the session's recent output instead, and
a hit from a running session is marked live. Its archive-only Export and
Delete actions appear once it has exited and been archived. History loads archived sessions in pages and says how many are loaded
...
If an agent prints a line beginning with the task's notify phrase (by default
`VOGT_NOTIFY:`; a task may configure its own, and exactly that phrase is
matched), two things happen: a push notification goes out, and the line is
**recorded on the run as a finding**. The push is a delivery; the finding is
the record, and it survives a phone that was switched off. Findings are listed under their run row, so the
...
- **Push** arrives for the things worth an interruption and nothing else by
  ...
  `ENGINE_FCM_SERVICE_ACCOUNT_JSON` is also accepted, but puts a private key
  in the environment and is warned about), and without it only browser
...
- [x] #4 Land the provider seam
- [ ] #5 Wire the collectors
<!-- vogt:initiative:end -->
```
