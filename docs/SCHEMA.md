...
Companion to [`ARCHITECTURE.md`](ARCHITECTURE.md) (domain model and storage).
Types are indicative; the migrations under
`src/vogt/storage/sqlite/migrations/` are authoritative.
...
- Cross-database joins happen in the application layer, not SQL `ATTACH`
  ...
  aggregate view is therefore a Python-side join, a seeded benchmark fixture
  runs in CI to keep that cost visible (§5).
...
shape. A forge PAT must be handed back to the upstream API to write as the
actor, so it has to be recoverable: Fernet ciphertext under
`forge_account_key_file`, never plaintext. `login` and `scopes` are cleartext
on purpose, so a status read ("is my account linked, and as whom?") works with
no key configured; only the write path decrypts.

`audit` is indexed on `actor_id`, `(entity_kind, entity_id)` and `at`.
`audit.list` uses the time index: it takes `since` (inclusive) and `until`
(exclusive), `offset`, so the log is readable past its newest page, and
`total`, so a reader can tell whether it is showing all of it.
...
The *order* is unindexed: the log is read newest-first by
`(revision, at, id)` and no index covers that, so every audit query sorts the
rows its filters selected. If the table gets big enough to notice, an index on
the sort key, or paging by `at` through `idx_audit_at`, would both fix it
...
| Table | Purpose | Key columns |
|---|---|---|
| `projects` | unit of the per-repo view; one explicitly registered repo or folder | `id, slug, name, root_path, repo_url, lifecycle_state, current_version, contract_version, compliance_status(compliant\|non_compliant\|not_checked), compliance_checked_at, contract_adopted_at, link_state(unlinked\|linked), exclusions(json), trust_state, created_at, updated_at` |
| `contract_exemptions` | a criterion declared unmeetable by a project, with its reason and its author | `id, project_id, rule, target, reason, declared_by, declared_at` |
| `initiatives` | cross-project epics | `id, slug, title, body, state(open\|closed), weight, created_at, updated_at` |

**There is no `project_dependencies` table, so `ref_kind = declared` is
unreachable.** Every dependency edge in the system is *observed*: `dep-refs`
reads manifests and emits `path`, `git` and `inherited` references into
`latest_dep_refs` (§3.2). The `declared` kind survives in the `RefKind` type
and the column's CHECK constraint, but nothing produces one — no operation
declares a dependency edge by hand, so an edge no manifest expresses cannot
be recorded.

There is no `contracts` table either: the contract is a versioned constant in
`core/contract.py` rather than per-instance configuration, and the evaluated
result lives in `projects.compliance_status` with its `compliance_checked_at`.
Nor is there a `packages` table — dependency edges are resolved by path and
repo URL, so published package identity is not needed to build the internal
graph.
...
The forge sync reads it from a PR's closing keywords and branch name
...
*The derived git story* joins two observed facts — the `git.branch`
observations and the `implemented_by` PR edge — into a read-only answer to
*where is this in git?* on `work.get`. It is **not a table and not a
...
contributions). The phase is derived from the branches and the PR edge alone;
the engine's per-task run conclusion is not an input to it.
...
`initiative.publish` projects an initiative onto **one forge tracking issue
per linked repo** it spans — an issue labelled `initiative:<slug>` whose body
carries a checkbox task list of the member work items (`- [ ] #<n> <title>`,
checked when the member is terminal). It is the one write-back that *edits* a
body rather than only appending, so the provider write surface has a single
bounded verb, `update_issue_body`, beside the append-only set (`comment`,
`create_issue`, `add_labels`, `set_state`). It stays additive and forward-only
by construction: Vogt rewrites only the span between two
...
`work_overlay` carries only what must never cross the forge boundary, and
joins the observed mirror (which stays the truth for title/labels/open-closed)
into the item every surface returns for a linked project. There is no `wrk_*`
row behind an upstream-truth item: the subject key is its id and its ref. On a
failed write-through nothing lands here, because the provider call runs before
the declared transaction opens. `rank` is schema for a vogt-local ordering;
the overlay upsert carries it through, and no operation sets it.

`work_overlay.branches` is a JSON array of the branch names a session started
*from Vogt* declared it would work the item on — the declared half of the
branch binding. It is additive and forward-only: recording a name never
touches git. The *observed* half — what a `git-local` sweep actually finds —
stays in the evidence store as `git.branch` observations and is never folded
in here, so the two can be compared and a disagreement surfaced as drift
rather than one overwriting the other. The column is keyed the same way the
row is — by the work-item ref, which is `WI-7` for a native item and the forge
subject for an upstream one.

`work_items.superseded_by` is the retire marker for native items migrated
upstream on `forge.link`/`forge.publish`. Set once to the new subject key,
never cleared; a superseded row is excluded from every work view (each issue
counted exactly once — the upstream item is the item) while the row itself
keeps anchoring its comments, relations, ledger rows and audit history, and
its `WI-n` ref still resolves by direct read. Retire-by-marker is chosen over
deletion for exactly that history, and no `work_links` row is written for the
new subject because the dedup reads one as "this declared row IS the item".

`work_items.ref` is the short handle (`WI-7`) allocated from a counter in
`meta`, inside the creating transaction so a rolled-back creation leaves no
...
expressed through `priority` and initiative weight, which are themselves
scored inputs. No hand-set position competes with the score.
...
`superseded_at` marks an **open** proposal whose raising condition a later
completed sweep no longer reproduces. It is not a resolution and not a
status: the row stays `open`, keeps its snapshot, and still requires a
person. It clears again if the condition returns, because a stale
"superseded" tells a reader to ignore a live proposal. Coverage-gated:
nothing is marked unless the collector that raised it completed a sweep
*after* the proposal was opened, since silence outside a completed sweep is
"not collected".

`evidence_snapshot` is the self-contained copy of the evidence as it stood at
raise time: the observation payload digest, its `subject_key`, its
`observed_at`, and enough of the payload to explain the proposal without the
observed store. `evidence_observation_id` remains the pointer to the live
row, and observations so referenced are exempt from retention pruning (§5).
A proposal must never outlive its evidence.

Drift kinds:

| Kind | Notes |
|---|---|
| `version_mismatch` | declared version vs observed tag/release |
| `unresolved_dependency` | internal-looking reference with no registered target |
| `forge_state_mismatch` | linked issue/PR state disagrees |
| `ci_red_vs_healthy` | CI red on default branch, project claims healthy |
| `vanished_upstream` | linked forge object absent within provably swept scope |
| `update_automation_gap` | a required automation toggle is off |
| `broken_path_dependency` | a path reference inside the project's own tree resolving to nothing |
| `referenced_issue_state_mismatch` | a work item's own text names a forge issue whose observed state disagrees |
| `initiative_checkbox_drift` | a box on an initiative tracking issue was ticked upstream out of step with the member's workflow state |
| `initiative_tracking_close` | the initiative is closed here; its tracking issue is still open upstream — a *proposal* to close it, never an automatic write |

Two things that look like drift are not. A contract violation is
`projects.compliance_status`, and an unregistered repository is not a
finding at all — Vogt has no notion of an unregistered path (§4); the
repositories a linked credential can see are listed by `forge.repos` for
import. Nothing compares dependency *versions* — no version is resolved
(§3.2) — and the same source living in two registered projects is reported as
the `mirrored_source` observation, not as a drift proposal.

### 2.5 Events & suppression

| Table | Purpose | Key columns |
|---|---|---|
| `events` | the single ordered notification feed | `seq INTEGER PRIMARY KEY AUTOINCREMENT, kind, entity_kind, entity_id, actor_id, audit_id, summary(json), at` |
| `suppressions` | observed subjects excluded from ranked views | `id, match_kind(exact\|pattern), subject_key_or_pattern, scope_project_id, actor_id, reason, created_at, revoked_at` |
...
Instance creation is the one audited write that emits **no** event.
`vogt init` creates the instance rather than changing anything inside one, so
it writes `meta`, the initiating actor, and an audit row at revision 0 — and
the feed therefore starts at the first change a client could act on.
Everything else, including the auto-registration of a principal seen for the
first time, lands its event in the same transaction as its entity change and
audit row.
...
| Table | Purpose | Key columns |
|---|---|---|
| `coding_sessions` | the declared link from a work item or project to a terminal the engine runs for it | `id, engine_session_id(unique), project_id, work_item_id, actor_id, cwd, template, reason, started_at, stopped_at` |
...
moment it was written, and a view that renders stale state as current is
exactly the failure the freshness rule exists to prevent. `stopped_at` is not
a counter-example: it says Vogt stopped the session, not that the process
ended.
...
freshness and trust like everything else (§3.1), which is why there is no
outcome column here. "What did we ask for, and why" and "what happened in
there" are different questions with different write disciplines.

The mechanism is the `session-outcomes` collector
...
`state: running` and `provisional: true` with no exit code and no duration —
because an outcome row with a null exit code cannot be told apart from one
whose exit code was lost, which is the confusion this store exists to
...
than to what changed. What a session left uncommitted is knowable only while
it runs, and is on the provisional row.

**Agent-task runs come back the same way.** A task in the engine may name a
Vogt project or work item; the runs of a bound task, and the findings those
runs reported, are collected as `agent_task.run` observations against that
subject. The engine never writes here, and that is the whole design: §1's
rule that nothing writes `observed.sqlite3` except collectors is what makes
an observation's freshness and coverage mean anything, so recording a run as
an observation is a *pull*.

A run cannot file an observation of its own choosing — arbitrary kind,
arbitrary subject, arbitrary payload — because that would be a second writer
of this store, with no sweep behind it and no coverage record to make its
silence readable. What a run can report is a line, through its notify phrase,
and what Vogt collects is that line against the subject the task was bound
to. An agent that wants to say something structured about a work item has the
write plane it has always had: the declared side, through an audited
operation, with an actor and a reason.

`cwd` is stored per session rather than read back through `projects.root_path`
because the record is of the path a session actually opened in: a project that
...
`depscan:project-slug`, `mirror:project/path->project`,
`contract:project-slug`, `session:<session id>`, `task-run:<run id>`,
`sync:<collector>/<owner>/<repo>` (the per-project forge-sync receipt). Same
`subject_key` + same `content_digest` in a new sweep ⇒ no new row (sweep
stats count it as unchanged), keeping growth proportional to change, not to
polling frequency.

A consequence worth stating: `observed_at` is **when a payload was first
seen, not when it was last confirmed**. `observations` rows stay immutable,
so the per-subject "last confirmed" record is a separate table —
`subject_seen` (§3.3) — touched on every sync batch including the batches
where nothing changed. A forge issue closed after its last observation is
re-observed as `closed` because the sync (`forge-issues`/`forge-prs`) reads
`state=all` incrementally, so a close is a payload change, not an absence;
and trust reads `subject_seen.last_confirmed_at` rather than `observed_at`.
The collectors that re-read whole state on every sweep, `latest_dep_refs`
among them, keep no such record: for those, `observed_at` is the only
freshness there is.
...
Marker observations carry a `promoted` flag derived from the
`marker_promotion_patterns` setting, so unpromoted markers stay queryable
without entering backlog views.

### 3.2 Derived (rebuilt, not source of truth)

| Table | Purpose |
|---|---|
| `latest_observations` | newest observation per `subject_key`, generic: kind, project, payload, `promoted`. Every typed read is a query over `kind` against this. |
| `latest_dep_refs` | one row per (project, target): `ref_kind(path\|git\|declared\|inherited)`, raw target, manifest file it was read from, resolved `to_project_id` or null |
...
state — the resolution of a reference to a registered project. Every other
typed projection a reader might expect (forge items, CI runs, contract checks,
releases, markers, update-automation posture) differs from the others in
payload shape and not in behaviour, so each is a `kind` filter over
`latest_observations` rather than a table of its own: one rebuild path is one
place for a collector and its projection to drift apart, where seven would be
seven. Update-automation posture is observation kind `posture`, subject
...
every write needs an actor and a reason. Both are mutable, unlike
`observations`.
...
- **Trust computation**: `verified` = declared row's linked subjects were
  seen in a sweep newer than `verify_horizon_hours` and agree; `stale` = last
  agreeing observation older than horizon; `unverified` = no linked
  observation ever; `disputed` = open drift proposal. Computed, never
  hand-set. The trust state is `disputed`, not `contested`: `contested` is
  reserved for the drift *resolution* status, where it names something a
  human deliberately chose.
...
- **Compliance**: `projects.compliance_status` is written directly when an
  on-demand contract check runs against a registered project (nothing
...
- **Scope**: every collector's `scope` is a set of registered project ids.
  The system has no notion of an unregistered path, so coverage
  questions are always answerable against a known finite list.

## 5. Scale envelope & retention

Single node, SQLite WAL mode: ≤ ~500 projects, ≤ ~100k work items. A seeded
fixture asserts the interactive-query target in CI — at 500 projects and
**5,000** work items rather than 100k, because seeding 100k rows per run
costs minutes and proves nothing about the query shape. It is a tripwire for
an accidental per-item query inside a ranked view; it is not evidence that
the ranked views stay interactive at the full envelope.

Retention, in precedence order — a row is pruned only if no rule protects it:

1. The **latest** observation per `subject_key` is kept indefinitely.
   Digest dedup means a stable subject's latest row can be older than the
   history window, so age alone must never prune it.
2. Any observation referenced by a `drift_proposals` row — of any status,
   including resolved — is kept.
