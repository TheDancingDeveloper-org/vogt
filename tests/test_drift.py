"""Drift proposals: raised with evidence, resolved by a person.

Includes the drift demo scenario:
...
def test_a_stale_red_is_history_not_drift(released: AppContext) -> None:
    """A fixed build must stop being a proposal.
...
def test_raising_a_proposal_is_an_audited_declared_write(
    ...
    """A drift proposal is a declared entity, so raising one is audited.
...
def test_a_raised_drift_event_carries_its_project(released: AppContext) -> None:
    # The engine's agent-task `drift-proposed` trigger scopes a fire to a
    ...
    with released.declared.read() as view:
        ...
        raised = [
            ...
            if event.kind == DRIFT_RAISED_EVENT and event.entity_id == proposal.id
        ]
...
# -- proposals raised under evidence that moved on -------------------------
...
def test_a_proposal_the_evidence_stopped_reproducing_is_marked_superseded(
    ...
    assert marked.status == "open", "marking is not resolving"
...
def test_nothing_is_superseded_without_a_sweep_that_could_say_so(
    ...
    """Coverage-gated, like every other absence claim here.
...
def test_detect_names_the_projects_it_could_not_have_raised_anything_for(
    ...
    """`detect` refuses on a wholly unswept instance, and said nothing
...
def test_accepting_applies_the_change_and_is_audited(released: AppContext) -> None:
    """Accepting is an ordinary audited write, not a silent mutation."""
    proposal = detect_drift(
        released, DriftDetectParams(auto_accept=False, reason=WHY)
    ).raised[0]
...
def test_the_low_risk_policy_accepts_state_sync_only(released: AppContext) -> None:
    """The shipped default, and what it deliberately will not touch."""
...
def test_m3_demo(instance: AppContext, tmp_path: Path) -> None:
    ...
    # The contract is opt-in, and this demo is about a project that
...
# -- the engine's drift push filter names the kind this module emits -------
...
def test_resolving_drift_is_not_worth_a_phone_interruption() -> None:
    """Push routing says "new drift", and the filter is a named set for this reason.
...
# -- reading an issue reference out of an item's own text ----------
...
@pytest.mark.parametrize(
    "text",
    [
        # WI-16's own title. The bare number is a pull request; the issue it
        # mirrors is named separately, as a URL.
...
def test_the_auto_accept_policy_is_symmetric_for_forge_state() -> None:
    """Both directions now, because closure is observed.
