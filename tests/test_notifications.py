"""GitHub notifications as observations, and the inbox over them.

Includes the inbox demo, whose acceptance criterion is:
...
def test_the_scope_is_the_registered_project_list(
    ...
    """Scoped to registered projects by construction, not by filtering.
...
def test_notifications_never_enter_the_ranked_views(
    ...
    would (ARCHITECTURE.md) — the observed-first hazard arriving through a new
    door.
    """
    sweep(instance, SweepParams(collectors=["forge-notifications"], reason=WHY))
...
def test_a_token_without_the_scope_is_partial_coverage_not_a_failed_sweep(
    ...
    """ "we could not look" is an answer, and it degrades gracefully."""
...
def test_the_collector_needs_a_network_and_says_so() -> None:
    """The forge-less layer runs exactly the offline collectors."""
...
def test_the_inbox_says_whose_it_is(instance: AppContext, forge: Forge) -> None:
    """Instance-scoped, not per-actor, and stated rather than hidden.
...
def test_an_empty_inbox_distinguishes_absence_from_ignorance(
    ...
    """Zero versus not-collected, at the view where the two look most alike."""
    result = list_notifications(instance, NotificationsParams())
...
def test_the_forge_inbox_never_leaks_into_the_events_feed(
    ...
    """The inbox demo's real assertion: events versus notifications.
