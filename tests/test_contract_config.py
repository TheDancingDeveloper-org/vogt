"""The contract is sourced from configuration.

The requirement is a contract that is declarative, carries a version
identifier, names every failing criterion, and is **sourced from
configuration**. The first three shipped early; the fourth did not. It was the
constant `DEFAULT_CONTRACT`
...
def test_a_configured_contract_is_what_gets_evaluated(tmp_path: Path) -> None:
    ...
    the case a configurable contract exists for, and it fails against the
    stock contract — so a
...
def test_a_failing_criterion_is_still_named_under_a_custom_contract(
    ...
    """The "never a bare boolean" clause does not weaken when edited."""
...
def test_editing_the_rules_without_renaming_them_changes_the_version(
    ...
    This is the hazard the two clauses create together: make the rules
...
def test_the_contract_is_still_a_value_not_a_gate(tmp_path: Path) -> None:
    """Compliance-gates-nothing does not weaken because the rules became editable.
    ...
    assert result.criteria, "a bare boolean is what the contract forbids"
...
def test_an_unreadable_path_is_not_checked_rather_than_non_compliant(
    ...
    """WI-9's shape on the contract surface.
