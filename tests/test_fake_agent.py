"""Contract tests for `scripts/fake-agent`, the synthetic agent CLI.
...
easily supply — so the gates/steer and git-story work have a stable stand-in
to build against.
"""
...
def test_edit_commit_writes_a_trailered_checkpoint(tmp_path: Path) -> None:
    ...
    result = run_fake_agent(
        ...
        cwd=tmp_path,
        env={
            "VOGT_ENGINE_AGENT_TASK_ID": "task-42",
            "VOGT_ENGINE_AGENT_TASK_RUN_ID": "run-99",
        },
    )
    assert result.returncode == 0, result.stderr
...
def test_findings_prints_notify_line_and_dumps_json(tmp_path: Path) -> None:
    ...
    result = run_fake_agent(
        ...
        env={
            ...
            "VOGT_ENGINE_AGENT_TASK_ID": "t",
            "VOGT_ENGINE_AGENT_TASK_RUN_ID": "r",
        },
    )
    assert result.returncode == 0, result.stderr
...
def test_scenario_from_prompt_marker(tmp_path: Path) -> None:
    ...
    result = run_fake_agent(
        None,
        env={
            "VOGT_ENGINE_AGENT_TASK_PROMPT_FILE": str(prompt),
