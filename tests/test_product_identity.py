...
def test_static_user_facing_surfaces_do_not_regress_to_legacy_branding() -> None:
    ...
    compatibility_literals = {
        WEB / "public" / "sw.js": ('"vogt-default"',),
        # `viewport-resize` and `terminal-*` events were renamed to `vogt:*`
        # Only `native-insets` keeps the historic name, because the
        # human-gated Android shell still dispatches it.
        WEB / "src" / "index.tsx": ('"vogt:native-insets"',),
    }
...
def test_compatibility_names_remain_explicitly_stable() -> None:
    ...
    Browser-storage keys are NOT in this set any more: the rebrand renamed them to
    ...
    Android *applicationId* is no longer in this set either: the rebrand's mobile half
    ...
    # The push-notification prefix a run prints.
