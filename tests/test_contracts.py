"""Contract checks: a status you read, never a barrier."""
...
def test_this_repository_satisfies_its_own_contract() -> None:
    """Self-compliance, checked against the real thing rather than a fixture.
...
# -- what the repository carries, not what the disk holds ----------


def test_an_untracked_file_does_not_satisfy_a_criterion(tmp_path: Path) -> None:
    """The `acme` and `arz-client` case, and it is not a corner one.
...
def test_an_empty_directory_cannot_satisfy_a_required_dir(tmp_path: Path) -> None:
    ...
    `acme` passed `docs/` on one — a `mkdir` somebody ran once — while
...
def test_a_missing_file_is_not_confused_with_an_untracked_one(tmp_path: Path) -> None:
    ...
    The contract names the specific criteria that failed rather than a bare
...
def test_a_path_with_no_repository_is_still_answerable(tmp_path: Path) -> None:
    """`contract check --path` on a plain folder is legitimate.
...
def test_checking_a_bare_path_stores_nothing(
    ...
    """Invocable against any folder, without registering it.
...
def test_checking_a_registered_project_records_the_result(
    ...
    """Recorded with failing criteria and a checked-at timestamp."""
    register_project(
        instance,
...
def test_a_plain_sweep_does_not_check_the_contract(
    ...
    """Nothing re-checks compliance on a timer."""
    register_project(
        instance,
...
# -- nothing consumes compliance as a precondition ------------------------


def test_a_non_compliant_project_refuses_nothing(
    instance: AppContext, tmp_path: Path
) -> None:
    """Compliance gates nothing, asserted rather than asserted-about.
    ...
    # Compliance gates nothing, upstream-truth or not. The write
...
def test_no_code_path_branches_on_compliance_status() -> None:
    ...
    statuses. If a service starts comparing against them, the rule has been
    ...
    assert offenders == [], (
        "compliance is a value to be read, and these modules look "
...
def test_project_create_scaffolds_something_compliant(
    ...
    """Starting compliant is easier than becoming compliant."""
