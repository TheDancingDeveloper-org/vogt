"""The Board is honest about what it is not showing.
...
Repurposed for the forge-less guarantee withdrawal: these tests
...
def test_board_reports_the_candidate_population(estate: AppContext) -> None:
    ...
    # project and is withdrawn from the surfaces — counted, not
...
def test_candidate_count_matches_the_backlog_for_the_same_scope(
    ...
    # of one — and that includes agreeing on what was excluded.
...
def test_candidate_count_honours_the_project_filter(estate: AppContext) -> None:
    ...
    # is the unlinked CTA answer: empty cells and the machine-readable marker.
    register_project(
        estate,
...
def test_a_board_with_no_observations_counts_its_exclusions(
    ...
    # Repurposed: this used to pin "no observations means candidates
