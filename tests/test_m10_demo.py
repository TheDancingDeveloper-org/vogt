"""The merge-MVP demo, as an executable acceptance test.
...
def test_the_m10_demo(
    ...
    #    is exercised by the import demo against a recording cloner; what
    #    this demo adds starts at the work item, so this registers the tree
    #    directly.)
    #    Registered with its repository and linked: the write verbs
    ...
    # The item itself is the adopted/native declared shape (link-time
    # migration owns those); `work.create` on the linked project would write
    # through
    ...
    #    records — not in the engine's workspace root.
    ...
    # 4. The work item shows the session that is running for it, and
    ...
    #    is the assertion the stage exists for.
