"""The Forgejo provider — the seam's acceptance test.
...
reads, the append-only write surface — and then re-run the open-only sync
regression shape end to end against a Forgejo host, because "the interface holds" is a
claim about behavior, not signatures.

Every test injects a transport. Nothing here touches the network, and nothing
here speaks to the estate's real `forge.example.net`.
"""
...
HOST = "forge.example.net"
REPO = f"https://{HOST}/acme/Widgets"
...
def _ref() -> RepoRef:
    return RepoRef(host=HOST, owner="acme", repo="Widgets")
...
@pytest.mark.parametrize(
    ...
    [
        (REPO, True),
        (f"{REPO}.git", True),
        (f"git@{HOST}:acme/Widgets.git", True),
        (f"ssh://git@{HOST}/acme/Widgets.git", True),
        (f"git@{HOST}/acme/Widgets.git", True),
        (f"git+https://{HOST}/acme/Widgets", True),
...
def test_forgejo_provider_satisfies_the_protocol() -> None:
    """The structural check the provider seam promised a new host would pass."""
...
def test_subject_keys_are_host_qualified_and_round_trip() -> None:
    """`forge:{host}/{owner}/{repo}#{n}` — the host-qualified scheme."""
    provider = _provider()
    key = provider.subject_key(_ref(), 42)
    assert key == f"forge:{HOST}/acme/Widgets#42"
...
def test_number_of_declines_a_key_that_names_no_number() -> None:
    provider = _provider()
    assert provider.number_of(f"posture:{HOST}/acme/Widgets") is None
...
def test_the_urls_come_from_the_provider_not_a_hardcoded_string() -> None:
    provider = _provider()
    assert provider.clone_url(_ref()) == f"https://{HOST}/acme/Widgets.git"
    assert provider.web_url(_ref()) == f"https://{HOST}/acme/Widgets"
...
def test_the_auth_header_is_forgejos_token_scheme() -> None:
    ...
    client.get("/repos/acme/Widgets/labels")
...
def test_issues_ask_for_issues_only_and_carry_since_upstream() -> None:
    ...
    assert "state=all" in urls[0], "closed history is part of the answer"
...
def test_action_tasks_become_generic_checks() -> None:
    """CI checks through Forgejo Actions: same `workflow_runs` envelope, but one
...
def test_notifications_are_repo_scoped_and_honest_about_missing_fields() -> None:
            ...
            "/notifications": [
                {
                    ...
                    "subject": {
                        ...
                        "type": "Issue",
                        "url": f"https://{HOST}/api/v1/repos/acme/Widgets/issues/9",
                    },
                }
            ]
        }
    ...
    assert notes[0].source_url == f"https://{HOST}/acme/Widgets/issues/9"
    # Forgejo's thread model has no reason and no per-thread read timestamp:
    # `None`, never a guess (not-collected is not zero, applied to fields).
...
def test_the_posture_gap_reports_itself_as_not_supported(
    ...
    """The declared `supports_posture=False` becomes a receipt, not a
...
# -- list_repos: the import picker's enumeration --------------------
...
def test_list_repos_enumerates_what_the_credential_sees() -> None:
            ...
            "/user/repos": [
                {
                    ...
                    "private": True,
                    "default_branch": "main",
                    "html_url": f"https://{HOST}/acme/Widgets",
                },
    ...
    assert [r.slug for r in repos] == ["acme/Widgets"]
    assert repos[0].visibility == "private"
    assert repos[0].web_url == f"https://{HOST}/acme/Widgets"
...
def test_comment_posts_and_reports_the_host_qualified_key() -> None:
    ...
    assert result.subject_key == f"forge:{HOST}/acme/Widgets#12"
...
def test_create_issue_appends_twice_because_labels_ride_their_own_endpoint() -> None:
    ...
    endpoint after the create. Both verbs are appends."""
    provider, recorder = _writing_provider()
...
def test_add_labels_appends_and_never_replaces() -> None:
    ...
    assert methods == {"POST"}, "POST appends; no PUT (replace), no DELETE"
...
def test_an_existing_name_is_a_typed_refusal_never_a_clobber() -> None:
    """Forgejo answers a duplicate with 409; that is `RemoteRepoExists`."""
...
# -- end to end: the open-only regression shape, against a Forgejo host ----
...
def forgejo_instance(
    ...
    register_project(
        ctx,
        RegisterProjectParams(
            name="acme", root_path=str(root), repo_url=f"{REPO}.git", reason=WHY
        ),
    )
    return ctx, fake
...
def test_forgejo_issues_and_prs_reach_the_backlog(
    ...
    assert fake.mutations == [], "and reading asked nobody anything"
...
def test_forgejo_sync_is_incremental_via_watermarks(
    ...
    with ctx.declared.read() as view:
        project = view.project_by_slug("acme")
...
def test_a_closure_upstream_leaves_the_backlog(
    ...
    """Exit criterion, re-run through the second provider: observe an
    ...
    latest = ctx.observed.latest_by_subject(f"forge:{HOST}/acme/Widgets#42")
    assert latest is not None, "and the key scheme is the provider's own"
    assert lifecycle_of(latest) == LIFECYCLE_CLOSED


def test_parse_repo_url_rejects_injection_metacharacters() -> None:
    """Owner/repo carrying `..`, `?`, `#`, `%` are unparseable.
    ...
    hosts = (HOST,)
    ok = parse_repo_url(f"https://{HOST}/acme/Widgets", hosts)
    assert ok is not None and ok.owner == "acme" and ok.repo == "Widgets"
    for bad in (
