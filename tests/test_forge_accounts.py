"""Per-actor forge account linking.
...
class RecordingForge:
    ...
    def __call__(
        ...
        if "forge.example.net/api/v1/user" in url:
...
def test_a_forgejo_account_is_validated_and_stored_for_its_host(
    ...
    forge = RecordingForge(login="acme")
...
def _attempt_comment(ctx: AppContext) -> WriteBackRecord:
    ...
    # and what this test drives is `writeback.attempt`'s identity
