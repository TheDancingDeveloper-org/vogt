"""The GitHub module: consolidation, forge drift, and write-back.

Includes the forge demo scenario:
...
def forge(
    ...
    fake = Forge(
        issues=[
            ...
            {
                # Genuinely open and carrying no labels at all — a common
                # shape upstream.
                "number": 9,
    ...
    # Linked, the upstream-truth way: this suite exercises the adopted-item
    # write-back plane, and the write verbs refuse on an unlinked
...
def test_what_the_filter_removed_is_reported_rather_than_dropped(
    ...
    """A short list and a filtered list must not look alike.
...
def test_a_closed_subject_stays_observable(instance: AppContext, forge: Forge) -> None:
    """Out of the ranked views is not out of the record.
...
def test_an_unlabelled_issue_says_its_kind_was_guessed(
    ...
    """Nothing said what it was, so nothing should claim otherwise.
...
def test_a_consolidation_is_audited_with_the_reason_it_was_given(
    ...
    """The largest read of an import leaves a row saying who asked.
...
def test_by_state_stays_declared_only_and_says_so(
    ...
    machine it has never entered (ARCHITECTURE.md).
    """
    onboard(instance, OnboardParams(project="rustnzb", reason=WHY))
    brief = brief_project(instance, ProjectBriefParams(slug="rustnzb"))
...
def forgejo(instance: AppContext, tmp_path: Path) -> tuple[AppContext, Forgejo]:
    ...
    config = instance.config.model_copy(
        update={"forge_token_files": {"forge.example.net": token}}
    )
    ...
    register_project(
        ...
        RegisterProjectParams(
            ...
            repo_url="https://forge.example.net/acme/Widgets.git",
            reason=WHY,
        ),
    )
    return ctx, fake
...
def test_the_forgejo_host_is_read_not_refused(
    ...
    """WI-9, inverted. The zeros are gone because the read is real.

    `acme` lives on Forgejo and has one open issue; `forge onboard` used
    ...
    explain, and consolidation stayed read-only on this forge too.
    """
    del forge
    ctx, fake = forgejo
    ...
    assert fake.mutations == [], "and onboarding asked nobody anything"
...
def test_an_honest_empty_and_an_unread_one_are_still_distinguishable(
    ...
    shape — that is the Forgejo read's acceptance. The distinction WI-9
    exists for has
    ...
    assert "forge.example.net" in unreadable.detail, "and what it can — both forges"
...
def test_every_write_back_is_recorded_with_its_reason(
    ...
    """Audited locally, and re-observed on the next sweep."""
    ref = _adopted(instance, forge)
    set_write_back(
        instance,
        SetWriteBackParams(project="rustnzb", policy="comment_only", reason=WHY),
...
def test_closing_upstream_raises_a_state_mismatch(
    ...
    """The forge demo's second half, minus the acceptance."""
    ref = _adopted(instance, forge)
...
def test_an_item_finished_while_the_issue_it_names_is_open_is_drift(
    ...
    # the pre-upstream-truth shape this scenario is about, still producible
    # via `work.adopt`
...
def test_a_pull_request_number_in_a_title_is_not_an_issue_reference(
    ...
    `WI-16`'s own title reads "Regression from #n" — and that `n` is a pull
...
def test_reopening_finished_work_is_automatic_once_closure_is_observed(
    ...
    """The reopen direction, now that it has positive evidence.
...
def test_a_missing_automation_toggle_names_which_one(
    ...
    """Three facts, and the gap says which of them is missing."""
