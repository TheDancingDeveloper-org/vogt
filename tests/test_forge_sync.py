"""Incremental all-state forge sync — and the open-only regression.

The defect: the collectors asked only for *open* issues, so a
...
def test_the_closure_is_recorded_as_an_observation(
    ...
    The root cause was that no closed observation was ever written; the
...
# -- receipts distinguish 0 from not-collected ----------------------------
...
def test_a_non_github_project_gets_a_not_supported_receipt(
    ...
    elsewhere = [
        r
        for r in receipts
        if r.payload.get("supported") is False and "forge.example.net" in str(r.payload)
    ]
...
# -- PRs in the backlog by default, excludable by view filter -------------
...
def test_prs_are_in_the_backlog_by_default_and_excludable(
    ...
    assert {1, 100} <= set(_backlog_numbers(instance)), (
        "issues and PRs both, by default"
    )
