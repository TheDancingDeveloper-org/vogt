"""`forge.import` — turn a picker-listed repository into a project.
...
class ForgejoImport:
    ...
    def __call__(
        ...
        if url.endswith("/user"):
            return 200, json.dumps({"login": "acme"}).encode()
                ...
                [
                    {
                        ...
                        "default_branch": "main",
                        "private": False,
                        "html_url": ("https://forge.example.net/acme/fuelwatchx"),
                    }
                ]
            ).encode()
        if "/repos/acme/fuelwatchx" in url and url.endswith("fuelwatchx"):
            return 200, json.dumps({"default_branch": "main"}).encode()
...
def test_forgejo_import_uses_configured_provider_and_host_qualified_url(
    ...
    assert result.project.repo_url == "https://forge.example.net/acme/fuelwatchx"
    assert cloner.requests[0].remote == "https://forge.example.net/acme/fuelwatchx.git"
...
def test_project_import_clones_a_public_configured_forgejo_repo_without_token(
    ...
    ctx = dataclasses.replace(
        ...
        config=ctx.config.model_copy(
            update={
                "forge_token_files": {"forge.example.net": tmp_path / "missing-token"}
            }
        ),
    )
    ...
    result = import_project(
        ctx,
        ImportProjectParams(
            repo="https://forge.example.net/acme/fuelwatchx",
            reason=WHY,
        ),
    )
    ...
    assert cloner.requests[0].remote == (
        "https://forge.example.net/acme/fuelwatchx.git"
    )
...
def test_forgejo_picker_lists_host_qualified_repositories(tmp_path: Path) -> None:
    ...
    result = list_forge_repos(ctx, ForgeReposParams(host="forge.example.net"))

    assert [(repo.owner, repo.name, repo.url) for repo in result.repos] == [
        (
            "acme",
            "fuelwatchx",
            "https://forge.example.net/acme/fuelwatchx",
        )
    ]
...
def test_forge_import_clones_registers_and_consolidates(tmp_path: Path) -> None:
    ...
    # repository's existing issue, so it does not arrive looking empty.
...
def test_forge_import_clones_under_the_actors_credential(tmp_path: Path) -> None:
    """The clone runs under the linked PAT, not the instance file token.
...
def test_forge_import_falls_back_to_the_file_token(tmp_path: Path) -> None:
    """With no linked account, the instance file token clones."""
    forge = ImportForge()
    cloner = RecordingCloner()
    ctx = _instance(
        tmp_path,
...
def test_forge_import_leaves_it_ready_for_writeback(tmp_path: Path) -> None:
    ...
    operates on the project, because it came back linked."""
    forge = ImportForge()
    cloner = RecordingCloner()
    ctx = _instance(
        tmp_path,
