...
Includes the import demo, whose acceptance criterion is:
...
def test_remote_callers_cannot_name_paths_outside_the_import_root(
    ...
    """Project register/create/import contain remote root_path.
...
def test_something_that_is_not_a_repository_is_refused(
    ...
    Nothing here looks, so it is an error rather than a search.
    """
    context, _ = importing
...
def test_import_consolidates_and_changes_nothing_upstream(
    ...
    """The import demo. Import reads history; it never writes it.

    Asserted by recording every request the adapter made and failing on any
    method that is not a GET — the same check the forge demo uses, because an
    import that
...
def test_without_the_adapter_the_import_still_works_and_says_so(
    ...
    """The core never requires the optional adapter.
...
def test_a_second_import_of_a_clean_checkout_at_parity_reuses_it(
    ...
    """Re-importing a clean checkout at parity reuses it.
...
def test_reimporting_over_a_dirty_working_tree_is_refused(
    ...
    """A dirty existing checkout is refused, not reconciled.
...
def test_reimporting_over_a_diverged_default_branch_is_refused(
    ...
    """A local default branch ahead of origin is refused.
...
def test_a_clean_reimport_never_fetches_merges_or_rebases(
    ...
    """The gate reads, and the import performs no merge logic ever.
...
def test_importing_into_a_new_folder_skips_the_parity_gate(
    ...
    """A fresh clone is never gated — there is nothing to gate.
...
def test_an_instance_without_git_says_so_instead_of_blaming_the_checkout(
    ...
    """The guard has to run before anything asks git a question.
...
def test_a_git_that_cannot_be_run_is_not_read_as_an_empty_answer(
    ...
    """The other half: `_read` tolerated far more than it meant to.
...
def test_a_checkout_with_no_origin_is_still_a_conflict(
    ...
    """The narrowed tolerance is still there for the case it was for.
...
def test_the_token_never_reaches_the_command_line_or_the_config(
    ...
    """No credential on disk, asserted at the two places one is normally left.
