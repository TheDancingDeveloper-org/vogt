...
FORGEJO = "https://forge.example.net/acme/Widgets.git"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # WI-9's host. Readable — but only when `[forge_token_files]`
...
def test_an_unconfigured_forgejo_host_names_itself_and_what_is_readable() -> None:
    ...
    A Forgejo host is readable when `[forge_token_files]` names
    ...
    assert "forge.example.net" in reason
...
def test_supported_hosts_without_config_advertises_the_constant_host_only() -> None:
    """A Forgejo host is configuration; github.com is the build's."""
...
def test_create_issue_and_add_labels_are_additive() -> None:
    ...
    # POST appends; there is no PUT (replace) and no DELETE anywhere.
    assert methods == {"POST"}
