...
- the op uses the acting actor's linked PAT when they have one, and the
...
def test_enumeration_uses_the_actors_linked_pat(tmp_path: Path) -> None:
    """A linked actor sees the repositories *their* PAT reaches."""
...
def test_enumeration_falls_back_to_the_file_token(tmp_path: Path) -> None:
    """With no linked account, the instance file token enumerates."""
