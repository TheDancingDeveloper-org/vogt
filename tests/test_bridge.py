...
That is the property these tests are really protecting — an earlier product's biggest
...
def test_the_handshake_is_answered_before_discovery_starts() -> None:
    """A slow core must not turn into "server failed to connect".
...
def test_version_skew_warns_and_never_blocks() -> None:
    """DEPLOYMENT.md: one stderr line, and startup proceeds."""
...
def test_a_banner_that_is_not_json_is_a_warning_not_a_death() -> None:
    """The one discovery failure that was not guarded is the one that hit.

    Behind the merged front door, `/connection-info` is answered by the PWA's
    `index.html` at 200. `json.loads` raised out of `main()`, so
...
def test_a_token_is_read_from_a_file_never_an_argument(tmp_path: Any) -> None:
    """A token on a command line ends up in `ps` and in history."""
    path = tmp_path / "token"
...
def test_a_session_presents_its_own_token_not_the_containers(
    ...
    . Both are present at once inside a session. If the file won,
