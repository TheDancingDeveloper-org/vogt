"""Tokens, scopes, and the two gates a write passes."""
...
def test_last_used_write_is_debounced(agent: AppContext) -> None:
    """The last-used write is skipped within the debounce window.
...
def test_prune_auth_decisions_caps_allows_and_keeps_denies_longer(
    ...
    """Allows are pruned at the horizon, denies kept longer."""
...
def test_the_writeback_scope_gates_arming_write_back(agent: AppContext) -> None:
    """In both directions, because one of them is the whole point.

    `writeback` once gated nothing: it parsed, it implied `read`, it was
...
def test_every_scope_this_instance_issues_gates_something(agent: AppContext) -> None:
    ...
    confers only `read`. `writeback` was in exactly that state for a long
    time, and nothing could have caught it: every parse test passed, because parsing
    was all it did.
    """
    del agent
...
def test_both_allow_and_deny_are_recorded(agent: AppContext) -> None:
    """The denials are the interesting half."""
    caller = authenticate(agent, bearer=_token(agent, "read"))
    authorize(
        agent,
        caller,
...
def test_the_local_path_is_unauthenticated_and_says_so(instance: AppContext) -> None:
    """DEPLOYMENT.md: loopback has no authentication by design."""
...
def test_issuing_a_token_for_another_actor_still_audits_the_caller(
    ...
    """Naming a subject is not the same as claiming to be one.
