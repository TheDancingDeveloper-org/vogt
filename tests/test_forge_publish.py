"""`forge.publish` — create the remote, push, link.
...
class PublishForge(RecordingForge):
    """The upstream-truth stand-in forge, taught the one new endpoint.
...
def test_publish_creates_the_repo_pushes_and_links(
    ...
    assert push.token not in push.remote, (
        "the token is never embedded in the remote URL"
    )

    assert result.repo == "https://github.com/acme/demo"
    assert result.branch == "main"
    assert result.project.link_state == "linked", "published means linked"
...
def test_a_published_project_is_upstream_truth_thereafter(
    ...
    assert created.item.ref == "gh:acme/demo#1", (
        "work.create on the published project writes through"
    )
