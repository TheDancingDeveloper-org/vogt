"""The MVP demo — the MVP acceptance test.
...
#: projects — the rustnzb three-forms situation that motivated `deps` and the
#: reference-level dependency model (ARCHITECTURE.md).
NZB_CRATES = ("nzb-core", "nzb-decode", "nzb-nntp", "nzb-postproc")
OTHER_PROJECTS = ("rustTorrent", "widgets-rs", "egressy", "Rust-PAR2")
...
def test_m2_demo(estate: AppContext, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Repurposed by the forge-less withdrawal: these projects are unlinked,
    # so the declared row an adoption creates is withdrawn from the ranked
    # views — before that this asserted the row replaced the marker. The
    # subject still appears exactly once (as the observed candidate, labelled
    # with its adoption), and the withdrawn row is counted, never silently
    # dropped; linking the project migrates it upstream
    # (`test_native_migration`).
    ...
    assert adopted.item.ref not in refs, (
        "the declared row is withdrawn until the project links"
    )
...
def test_the_same_estate_works_with_no_forge_at_all(estate: AppContext) -> None:
    """Unplug the network and the GitHub adapter is simply absent.
...
def test_a_second_sweep_finds_nothing_new(estate: AppContext) -> None:
    """Growth is proportional to change, not to polling."""
