"""The PR ↔ work-item edge, observed from closing keywords and branch names.

Observed-first, additive, forward-only: the `implemented_by` edge is
...
# -- parsing: branch names (coordinates with branch binding's pattern) -----
...
def test_implemented_by_cannot_be_declared_by_hand(instance: AppContext) -> None:
    """It is observed from a PR, never typed in — so it can never become
...
def test_only_depends_on_gates_completion_never_implemented_by(
    ...
    """The completion gate is `depends_on`-specific. `implemented_by`
