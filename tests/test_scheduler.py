"""The in-process collector schedule.

The requirement has two halves — "on an in-process schedule" and "triggerable
on demand" — and only the second was built. `DEPLOYMENT.md` had long
listed "collector scheduler (in-process background sweeps)" in the `serve`
process diagram, so the deployment document described something that did not
...
def test_the_scheduled_reason_says_what_it_was() -> None:
    """Every write carries a reason, including ones nobody typed."""
