...
def native_work_item(
    ...
    *,
    ...
    labels: tuple[str, ...] = (),
    reason: str = "native fixture (pre-migration shape)",
) -> WorkItem:
    """A declared work item in a project, written the audited way.

    `work.create` refuses on an unlinked project, but a *native declared row
    in a project* remains a legitimate shape the read plane must keep serving
    — `work.adopt` still produces one, and native-item migration owns their
    migration. Fixtures that need "a work item in this
...
def mark_linked(ctx: AppContext, slug: str) -> None:
    ...
    An unlinked project's native rows are withdrawn from every curated
    surface, so a test whose subject is *not* linking — Board SQL
    ...
    audited_write(
        ...
        reason="linked fixture (surface shape)",
        body=write,
    )
...
def native_comment(
    ...
    *,
    ref: str,
    body: str,
    reason: str = "native comment fixture (pre-migration shape)",
) -> object:
