...
def test_public_delivery_defaults_to_the_current_product_release() -> None:
    ...
    `stack.compose.yml` and the operator docs name (DEPLOYMENT.md). The
    ...
    # The bundled voice sidecar is versioned with the product: it is
...
# maintainer's own estate overlay is not tracked here — it is held in
...
def test_the_stack_compose_runs_the_published_aio_and_its_own_core() -> None:
    ...
    Loopback is also the containment: the engine is the only way
...
def test_the_stack_compose_wires_fcm_as_a_file_secret_never_inline() -> None:
    """The push-signing key reaches the engine as a file, never an env var.
    ...
    assert not re.search(
        ...
        "var in the shipped stack — deliver it as a file secret"
    )
...
def test_the_stack_compose_is_a_base_not_an_overlay() -> None:
    ...
    The stack is the pod plus the bundled `voice` sidecar — two
    ...
    assert services == ["vogt", "voice"], (
        f"the AIO is the pod plus its bundled voice sidecar; found services {services}"
    )
...
def test_the_voice_sidecar_is_internal_only() -> None:
    ...
    The engine is the only front door; the sidecar is reached over
...
def test_no_deploy_file_names_a_mutable_voice_image_tag() -> None:
    """Acceptance: no file under deploy/ names a mutable image tag for voice.
...
# ── The worked custom-image example ───────────────────────────────────
#
...
def test_the_custom_image_example_exists() -> None:
    assert EXAMPLE_DOCKERFILE.is_file(), "the worked example Dockerfile is missing"
    assert EXAMPLE_OVERLAY.is_file(), "the worked example overlay is missing"
    assert EXAMPLE_HOOK.is_file(), "the worked example lifecycle hook is missing"
...
def test_the_custom_image_derives_from_the_public_stack_digest() -> None:
    ...
    assert "@sha256:" in text, "the example must pin the base by digest"
...
def test_the_custom_image_overlay_mounts_a_hook_and_pins_a_compatible_pair() -> None:
    ...
    # It selects a voice image compatible with the stack it extends: a
...
def test_public_base_images_are_asserted_anonymously_pullable() -> None:
    """A from-source build pulls the org's mirror bases with no auth, so a
    ...
    assert script.exists(), "the anonymous-pullability check is missing"
