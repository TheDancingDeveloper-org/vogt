...
# -- one line per request, with a duration on it ---------------------------
...
def test_a_served_request_produces_one_line_carrying_its_duration(
    ...
    assert "query=limit=1" in line  # the evidence the slow GUI was diagnosed from
...
# -- one identifier, across two runtimes -----------------------------------
...
# -- the pathological request surfaces itself ------------------------------
...
def test_a_probe_is_quiet_until_it_is_slow(
    ...
    """`/health/ready` at 25 seconds is the line worth keeping."""
