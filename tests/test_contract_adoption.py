"""The contract is something a project opts into.
...
def test_checking_an_unadopted_project_records_nothing(
    ...
    """The contract statuses apply only to projects that adopted one."""
    slug = _handed(instance, tmp_path)
...
def test_a_project_that_declined_is_never_refused_anything(
    ...
    """Compliance still gates nothing, and opt-in does not become a new gate."""
    slug = _handed(instance, tmp_path)

    # Contract state gates nothing. The write plane's own
    # NotLinked refusal is about link state, not the
...
# -- the scaffold reaches a project Vogt was handed -----------------------
...
# -- a reported gap comes with the means to close it ----------------------
...
# -- unmet, and unmeetable, are different facts ---------------------------
