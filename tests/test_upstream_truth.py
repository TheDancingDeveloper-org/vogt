"""Upstream-truth work items and the local overlay.

The upstream-truth pivot's decisions, pinned as behaviour:
...
def test_the_other_write_verbs_refuse_on_an_unlinked_project(
    ...
    # working — the surface withdrawal is the migration's, not this refusal's.
    updated = update_work(
...
def test_a_comment_posts_upstream_and_stores_no_local_row(
    ...
    assert get_work(linked, GetWorkParams(ref=SUBJECT_1)).comments == [], (
        "the forge thread is the comment's home"
    )
...
# -- the local-only create path and the complete refusal -------------------


def test_local_only_create_succeeds_under_policy_none(
    tmp_path: Path, forge: RecordingForge
) -> None:
    """The explicit opt-in create on a linked project with policy `none`.
...
def test_default_create_on_linked_none_still_refuses(
    ...
    """Without `local_only`, the decision-9 refusal is unchanged."""
    ctx = _instance(tmp_path, forge)
    register_project(
        ctx,
        RegisterProjectParams(
...
def test_refusal_enumerates_every_missing_gate(
    ...
    """The refusal names the WHOLE chain, not just the first gate.
