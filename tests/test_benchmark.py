"""The scale tripwire, and the envelope run behind it.

The two-store split means every aggregate view is an application-layer join,
and that cost needs a tripwire rather than a late discovery. This seeds a
...
The target is meant to be asserted at the scale envelope — ~500 projects and
...
#: The scale envelope is ~500 projects and ~100k work items.
...
#: Default: the scale envelope. `tripwire` is the opt-*out*, which is
...
#: The target is interactive (< 1 s) queries. The margin absorbs a loaded
    ...
    with context.declared.write() as txn:
        ...
        for index in range(BENCHMARK_PROJECTS):
            project = Project(
                ...
                # candidates: an unlinked project's native items
...
def test_the_declared_store_stays_a_single_file(seeded: AppContext) -> None:
    """Zero external services, one volume, one file per store."""
