...
The requirement this covers did not exist for a long time, and its absence is
the reason nothing failed while the product had no client story at all. Three
separate items each recorded a piece of it as minor — "vacuously satisfied,
Vogt ships no client-setup script", the deployment document's un-built
generator, and the unpublished wheel — and no requirement owned the whole.
"""
...
def test_the_http_client_needs_no_vogt_code(configured: AppContext) -> None:
    ...
    (the version handshake exists because of it) and a second place to
    upgrade. Streamable
    HTTP has neither.
    """
...
def test_no_configuration_ever_carries_a_token_inline(
    ...
    """No credential in output, at the place a config example would leak one.
...
# -- the document DEPLOYMENT.md asked for ----------------------------
...
# -- reachable everywhere, like everything else -------------------
...
def test_connection_info_reports_the_url_unauthenticated(
    ...
    """The probe endpoint and the connect operation must not disagree.
...
def test_a_fronted_instance_reports_the_door_a_client_arrived_at(
    ...
    """The core answered with its own address, which nothing can reach.
...
def test_a_fronted_connect_renders_a_configuration_that_reaches_the_door(
    ...
    """The pasteable answer pointed at an address no client can reach.
...
def test_an_unfronted_instance_ignores_a_stated_identity(
    ...
    `/connection-info` is unauthenticated and `connect` renders a
