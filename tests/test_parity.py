...
same audit trail. Parity is *tested*, not intended (ARCHITECTURE.md).
...
SCRIPT: list[tuple[str, StepParams]] = [
    ...
    # The import picker: the same accessible-repo list must arrive
    # byte-identically on all three surfaces.
    ("forge.repos", {}),
    # The verb the picker leads to: clone a listed repository under the
    ...
    # The upstream-truth pivot: an explicit act makes the project upstream-truth.
    ("forge.link", {"project": "parity-project", "reason": WHY}),
    # The other explicit act: a local-only project is published — the
    ...
    # The explicit local-only create: on a linked project this makes a
    ...
    # The declared branch binding: a branch name lands on the linked
    ...
    # reports `not_applicable` and records nothing.
    ...
    # Runtime-pinned agent CLIs, against the same stand-in engine.
    ("agent_cli.list", {}),
    (
        "agent_cli.update",
...
def _recording_cloner(root: Path) -> Cloner:
    ...
    the internet, and parity has to drive every shared operation. The
...
class _StandInForge:
    """A deterministic forge, one per instance.
    ...
    def __call__(
        ...
        if method == "POST" and url.rstrip("/").endswith("/user/repos"):
            # `forge.publish`: mint the repository deterministically.
...
def _fresh(
    ...
    # deliberately uses the file token (sweeps have no acting person),
