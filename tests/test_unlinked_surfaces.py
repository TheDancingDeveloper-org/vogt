"""Unlinked projects have no work surface — a link/publish CTA instead.

These are the repurposed forge-less honesty tests. The suite once asserted
that an unlinked project's native backlog was *real* — items ranked, boards
drew them, `work.list` served them — because "the forge-less layer stays
real" was a guarantee. That guarantee is withdrawn: what these tests pin now
is the CTA contract that replaced it —
...
- the typed `NotLinked` write refusal stays as the write-plane half
