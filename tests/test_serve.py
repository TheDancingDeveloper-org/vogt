...
`DEPLOYMENT.md` exists because an earlier product's MCP port served only `/mcp`, so
...
# -- probes are plain HTTP, always --------------------------------


def test_health_and_version_need_no_credential(authed: TestClient) -> None:
    """The earlier failure this prevents: probes that need an MCP client."""
...
def test_connection_info_is_generated_by_the_server(authed: TestClient) -> None:
    """DEPLOYMENT.md: one document, from the running server."""
...
def test_everything_answers_on_the_one_port(authed: TestClient) -> None:
    """One process, one port, path-routed."""
...
def test_a_write_token_writes(
    ...
    # A remote root_path must be within the import root; the write still
    # succeeds when it is.
...
def test_the_write_is_attributed_to_the_token_holder(
    ...
    """Provenance comes from the credential, not from the request."""
...
def test_an_unknown_protocol_version_negotiates_down(
    ...
    """The handshake negotiates, on HTTP as well as stdio.
...
def test_ungranted_tools_are_invisible_not_erroring(
    ...
    """An agent's tool list is exactly what it can do."""
...
def test_a_listen_address_is_required(instance: AppContext) -> None:
    """No default may encode exposure."""
...
def test_new_voice_writes_over_mcp_enforce_scope_reason_and_persist_approver_audit(
    ...
    """The engine's approved payload lands at the real MCP write plane."""
