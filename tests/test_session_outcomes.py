"""Session outcomes and bound agent-task runs as evidence.
...
# -- the outcome of a session that ended -----------------------------------
...
def test_a_running_session_has_no_outcome_and_says_which(wired: AppContext) -> None:
    ...
    provisionally, with no exit code and no duration at all.
    """
...
def test_an_outcome_is_collected_and_never_declared(
    ...
    """The outcome arrives as evidence, and audits nothing."""
    start_session(wired, StartSessionParams(work_item="WI-1", reason=WHY))
    engine.finish("eng-1", exit_code=0)
...
def test_an_unchanged_outcome_does_not_grow_the_store(
    ...
    """A finished session is re-observed forever and written once."""
    start_session(wired, StartSessionParams(work_item="WI-1", reason=WHY))
    engine.finish("eng-1", exit_code=0)
    _sweep(wired)
    _sweep(wired)
...
def test_the_outcome_carries_the_sweeps_coverage(
    ...
    """Freshness is the sweep record, the same as every other kind."""
    start_session(wired, StartSessionParams(work_item="WI-1", reason=WHY))
    engine.finish("eng-1", exit_code=0)
    _sweep(wired)
...
def test_an_offline_sweep_does_not_reach_for_the_engine(wired: AppContext) -> None:
    """The forge-less layer must not need a second process running."""
