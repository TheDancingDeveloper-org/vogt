"""What the audit log can be asked.
...
def _item(ctx: AppContext, title: str, *, project: str | None = None) -> str:
    # `work.create` refuses on an unlinked project, and these
...
# -- an item's trail includes what was said about it -----------------------


def test_a_comment_appears_in_its_work_items_audit_trail(
    instance: AppContext,
) -> None:
    """The gap this was open on: a comment is audited against the comment.
...
# -- the log can be asked about a period -----------------------------------
...
# -- the project filter, answered in the query -----------------------------
...
# ── The other half of an item's history ────────────────────
#
...
def test_a_transition_event_carries_project_kind_and_labels(
    ...
    # The engine's agent-task trigger filters a transition by project,
