#!/usr/bin/env python3
"""Deterministic stand-in for the agent CLI an agent-task run drives.

An agent task spawns a CLI — `claude`, `codex`, `opencode`, usually wrapped as
`vogt-agent-auth run -- <cli>` — inside a PTY (see
`engine/server/src/agent_tasks.rs`). CI has none of those binaries and no way to
type into a PTY, so the run-orchestration behaviour (findings, and the future
gates/steer, triggers, outcomes and git story) has had no automated path.
This script is that path: it accepts the *same invocation shape* the engine
produces and plays a chosen **scenario**, with no network, no model, and no
wall-clock dependence beyond the small delays a scenario opts into.
...
* **env** — `VOGT_ENGINE_AGENT_TASK_PROMPT_FILE` (the prompt),
  `VOGT_ENGINE_AGENT_TASK_CONTEXT_FILE`, `VOGT_ENGINE_AGENT_TASK_ID`,
...
* **findings** — a run reports a finding by printing a line beginning with the
  notify phrase (`VOGT_NOTIFY:` by default); the
...
ENV_PROMPT_FILE = "VOGT_ENGINE_AGENT_TASK_PROMPT_FILE"
ENV_CONTEXT_FILE = "VOGT_ENGINE_AGENT_TASK_CONTEXT_FILE"
ENV_TASK_ID = "VOGT_ENGINE_AGENT_TASK_ID"
ENV_RUN_ID = "VOGT_ENGINE_AGENT_TASK_RUN_ID"
...
# matches. A scenario prints whichever phrase the caller asked for.
DEFAULT_NOTIFY_PHRASE = "VOGT_NOTIFY:"

# The sentinels the engine's conclusion path reads: a run that prints
...
# for these yet — the git story is future work — so this is the
...
def step_idle() -> None:
    ...
    This is the seam the future steer/gate work drives: the run pauses,
...
def step_skip() -> None:
    """Declare the run a deliberate no-op.
...
def step_schema() -> None:
    """Print a fenced ```json findings block, re-prompted until it passes.
