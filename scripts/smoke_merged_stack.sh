#!/usr/bin/env bash
# Check that a merged stack is actually merged (DEPLOYMENT.md).
#
...
# readiness probe is deliberately non-fatal, so `/readyz`'s top-level
...
if printf '%s' "$ready" | grep -q '"name":"vogt_core","ok":true'; then
    ...
    fail "  (a stack can be 'ready' with no core at all — the engine runs core-less"
    fail "   by design, and that is why this script exists rather than a healthcheck)"
fi

# 3. The two halves agree about where the workspace is.
if printf '%s' "$ready" | grep -q '"name":"workspace_agreement","ok":true'; then
    ...
    fail "  sessions and to the collectors running here"
fi

# 3a. A backup taken here would cover the engine as well as the core.
...
if printf '%s' "$ready" | grep -q '"name":"backup_agreement","ok":true'; then
    ...
    fail "  no session history, push subscriptions or VAPID keypair."
...
if [[ -n "$TOKEN" ]]; then
    if status="$(get -H "Authorization: Bearer ${TOKEN}" "${BASE}/api/vogt/status")"; then
        ...
        fail "  token and no fallback is configured"
    fi
else
...
echo
if (( failures > 0 )); then
    echo "${failures} check(s) failed — see DEPLOYMENT.md before deploying further" >&2
    exit 1
fi
