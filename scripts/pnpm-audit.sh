...
for attempt in $(seq 1 "$attempts"); do
  ...
  if printf '%s\n' "$output" | grep -qiE "$network_re"; then
    if [ "$attempt" -lt "$attempts" ]; then
      echo "::warning::pnpm audit could not reach the advisories endpoint (attempt ${attempt}/${attempts}); retrying in ${delay}s. Registry/network flake, not a dependency finding."
      sleep "$delay"
      delay=$((delay * 2))
      continue
    fi
    echo "::error::pnpm audit could not reach registry.npmjs.org after ${attempts} attempts — a registry/network failure, not a dependency advisory. Re-run once the runner's path to the registry recovers."
    exit 1
  fi
