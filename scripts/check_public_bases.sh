#!/usr/bin/env bash
# Every base image the *public* Dockerfiles reference must be anonymously
# pullable. "Build from source" (DEPLOYMENT.md) is supposed to be a clean,
...
mapfile -t refs < <(
  {
    ...
    | grep -vE 'thedancingdeveloper-org/(vogt|vogt-stack|vogt-voice):' \
    | sort -u
)
...
for ref in "${refs[@]}"; do
  ...
  for a in $(seq 1 "$attempts"); do
    ...
    case "$code" in
      ...
      401|403) echo "::error::${ref} is NOT anonymously pullable (HTTP ${code}) — a from-source build would fail at FROM. Make the package public or default the Dockerfile to an upstream image."; bad=1; break ;;
      404) echo "::error::${ref} does not exist anonymously (HTTP 404) — renamed or removed? A from-source build would fail at FROM."; bad=1; break ;;
  ...
  if [ -z "$ok" ] && [ "$bad" -eq 0 ]; then
    echo "::error::could not confirm ${ref} is anonymously pullable after ${attempts} attempts — registry/network failure, not a private-package finding."
    bad=1
  fi
done

[ "$bad" -eq 0 ] || exit 1
echo "every public base image the Dockerfiles default to is anonymously pullable"
