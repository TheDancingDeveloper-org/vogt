...
* **Additive with respect to history.** It closes, reopens, relabels and
  ...
  a commit — the same rule the product's own write-back holds.
...
def ensure_branches(api: Api, manifest: dict[str, Any], base_sha: str) -> None:
    ...
    print(f"branches: {len(manifest['branches'])} ensured (declared-branch pattern)")
...
def print_plan(manifest: dict[str, Any]) -> None:
    ...
    print(f"  branches:   {len(manifest['branches'])} (declared-branch pattern)")
