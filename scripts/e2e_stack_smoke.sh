#!/usr/bin/env bash
# End-to-end stack smoke: walk the stranger's path against a running
...
#
# This is the mechanical form of the human-on-a-phone smoke. It
...
# — the front door the engine publishes.
#
...
#     `VOGT_FIXTURE_TOKEN`. Unset, every forge step prints `SKIP (no
#     VOGT_FIXTURE_TOKEN)` and the smoke still passes on the half it could run.
#
...
FIXTURE_TOKEN="${VOGT_FIXTURE_TOKEN:-}"
...
step_token() {
    ...
    # install mode and we mint the first one. A stranger who cannot get
    ...
    if ! status="$(get "${BASE}/api/install/status")"; then
        ...
        fail "  whether the first-run wizard is open"
        return
    fi
...
step_work_create() {
    # A native work item — no project, no forge, no credential. Proves
...
step_agent_task() {
    # A synthetic agent task through the `fake-agent` preset: the run
    # prints a VOGT_NOTIFY line, which the engine records as a durable finding
    # This is the run-orchestration path a real agent CLI drives, with
    ...
    while true; do
        ...
        if [[ "$findings" =~ ^[0-9]+$ ]] && ((findings > 0)); then
            pass "the fake-agent run recorded a finding"
            return
        fi
...
# ── steps: the forge half (gated on VOGT_FIXTURE_TOKEN) ─────────────────────

FORGE_PROJECT=""

step_forge_reset() {
    # Rebuild the fixture repo to its known state. Idempotent and
    # additive; never deletes history.
    if VOGT_FIXTURE_TOKEN="$FIXTURE_TOKEN" python3 "${REPO_ROOT}/scripts/fixture_reset.py" \
...
step_forge_import() {
    ...
    # issues/PRs/labels. Uses the core's github.com file token.
    local body slug
...
step_forge_link() {
    # Make the imported project upstream-truth, so a work item created in
...
step_forge_observed_pr() {
    ...
    # observations). This is the read the forge half's acceptance turns on.
    if [[ -z "$FORGE_PROJECT" ]]; then
...
step_forge_writeback() {
    ...
    # — 'full' permits create/comment/label/close/reopen.
    if [[ -z "$FORGE_PROJECT" ]]; then
...
step_forge_upstream_work() {
    ...
    # becomes the upstream subject key. Verified against the forge
...
if [[ -n "$FIXTURE_TOKEN" ]]; then
    ...
    echo
    echo "== forge half =="
    skip "SKIP (no VOGT_FIXTURE_TOKEN) — reset/import/link/sweep/backlog/observed/upstream"
