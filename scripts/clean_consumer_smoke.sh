#!/usr/bin/env bash
# Clean-consumer smoke: prove a stranger can run the published all-in-one
...
#     VOGT_STACK_IMAGE=…/vogt-stack:0.6.0 VOGT_VOICE_IMAGE=…/vogt-voice:0.6.0 \
#         scripts/clean_consumer_smoke.sh
#
...
#      pull a stranger gets, proving the packages are public — and pull
...
#      transcript). This is the deterministic STT/TTS routing check: the audio
#      provably reached the sidecar and came back.
...
#      (the engine's 404 fallback) rather than erroring.
...
walk_core() {
    ...
    if get "${BASE}/readyz" | grep -q '"name":"vogt_core","ok":true'; then
        ...
        fail "  core at all; the AIO must carry its own"
    fi
    # The PWA at /.
...
walk_voice_off() {
    ...
    # at the speech URL and answers 404. That is the honest "unavailable"
