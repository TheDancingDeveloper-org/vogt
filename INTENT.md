...
v1 has the right core (Rust/Axum server that owns PTYs, broadcasts to clients, exposes file + git APIs) but accumulated too many half-finished surface layers — a code-server fork, a Tauri desktop, a React Native mobile app, a separate native APK wrapper, two web UIs. The foundation is salvageable but the directory is hard to navigate and the surface is hard to reason about. v2 starts clean, keeps the server primitives in spirit, and keeps the browser/PWA as the primary UI. An experimental GPUI desktop client was built and later deprecated; the shared web surface remains the supported desktop product.
