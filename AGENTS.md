...
- **`main` is the target for all development work.** Base every branch on
  `origin/main` and open every feature, fix, and docs pull request against
  it; releases are `v*` tags on `main`. A branch that has drifted behind
  `main` is rebased onto `origin/main` before its PR.
...
- Use `docs/ARCHITECTURE.md` for architecture, domain decisions, and behavior that
  exists. Do not describe aspirations as current behavior or create a
  competing design document.
...
- `deploy/` — the self-hosting base and overlays. `stack.compose.yml` runs
  ...
  its difference. Operator notes and deployment-specific values belong in
...
- Raw markers do not enter ranked views directly. Promotion, suppression, and
  exclusions gate observed-first visibility (`docs/ARCHITECTURE.md`).
...
- Documentation must describe the code in the same change. Update
  `docs/ARCHITECTURE.md` for a changed architectural/product decision,
  `docs/ENGINE.md` for an engine contract, and user/deployment docs when
  operation changes are externally visible.
- Never commit credentials, live Firebase files, tokens, private registry
  names, or operator-specific deployment values.
...
- The core image comes from the root `Dockerfile`. It contains no PWA or
  ...
  deployment target (`docs/DEPLOYMENT.md`); `deploy/vogt.compose.yml`
...
- `git diff --check`, `git status`, and the final diff show only intended
  changes. The pull request targets `main` and follows
