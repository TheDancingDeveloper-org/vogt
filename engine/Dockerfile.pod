...
# inline and none of what it builds. That split exists because the two
...
# (`@openai/codex`, `@anthropic-ai/claude-code`). They are per-commit and
# Renovate-pinned; a base image rebuilt weekly would decide their versions
# instead of the pin, so they stay in `engine/Dockerfile`. See the notes there.
#
# The three Docker Hub library bases are named through this organisation's GHCR
# mirror. `.github/workflows/mirror-base-images.yml` reads this file as
...
LABEL org.opencontainers.image.title="vogt-pod-base" \
      org.opencontainers.image.description="The development-pod toolchain base the Vogt stack image builds FROM" \
...
# always win over a same-named binary a user or a tool installs into their
# persisted $HOME. A user-managed ~/.npm-global/bin/codex once shadowed the
# system wrapper under the old ordering, silently running Codex with its nested
# sandbox/approvals still active instead of the pod-level bypass.
...
# Smallstep `step` CLI — lets in-pod agents self-issue short-lived SSH
# certificates against an operator's step-ca (`step ssh certificate ...`) for
# host-shell SSH. Installed from the official .deb (lands at /usr/bin/step).
...
# show`) in /etc/vogt/features.json so the server can expose accurate
...
#
# Consequence to keep in mind: the crate registry cache lives at
# /opt/rust/cargo/registry inside the image rather than on the persisted home
# volume, so crates re-download after an image redeploy. An sccache backend,
# where an operator configures one, still covers recompilation.
