...
#
# The dev-pod toolchain base (`ghcr.io/thedancingdeveloper-org/vogt-pod-base`).
#
# This file is everything the merged image's runtime stage used to install
# inline and none of what it builds (#184). That split exists because the two
# halves have completely different rates of change and the build was paying the
# slow one's price on every push of the fast one.
#
# Measured on the `dev` build of 2026-08-18, before the split: the merged image
# took 34 minutes, of which **1344s — 22 minutes — was this file's contents**,
# against 215s for `cargo build --release` and 178s for the core's venv.
# Nothing in here reads a line of `src/`, `engine/*/src/` or `web/`, so every
# one of those 22 minutes was spent reinstalling an unchanged toolchain because
# a Python module or a stylesheet had moved.
#
# Published as two variants, because the pod's shape is a per-stream decision
# and always was (`.github/workflows/build.yml` has passed the dev/prod split
# as build args since #23):
#
#   lean  — INSTALL_FLUTTER=false. main, and every release.
#   full  — INSTALL_FLUTTER=true.  dev, which is where the mobile work lands.
#
# **Tagged by content, not by a moving name.** `.github/workflows/pod-base.yml`
# tags each build `<variant>-<hash>-<week>`, where `<hash>` is a digest of this
# file and `<week>` is the ISO week. A change here therefore produces a new tag
# immediately, and an unchanged file still gets rebuilt once a week — which is
# not cosmetic: `rclone`, `sccache`, `gh`, the `step` CLI, `github-mcp-server`,
# Gradle and `selkies` all resolve *latest* at build time, so a purely
# content-addressed tag would freeze them at whatever the first build saw and
# call it reproducibility. The weekly component is what keeps "latest" meaning
# something while still letting an ordinary push hit a cache.
#
# The consuming build passes the resolved reference in by **digest**, as
# `POD_BASE_IMAGE` (see `engine/Dockerfile`), so what the merged image is built
# on is pinned in the same way and to the same standard as every other base here.
#
# What deliberately did *not* move into this file: the agent CLIs
# (`@openai/codex`, `@anthropic-ai/claude-code`, `theclawbay`) and the cadastre
# MCP bridge. The agent CLIs are per-commit and Renovate-pinned; a base image
# rebuilt weekly would decide their versions instead of the pin. The cadastre
# bridge is gated per stream (`INSTALL_CADASTRE_MCP`), and baking it into a
# shared base would install it into main's stack, which does not carry it. Both
# stay in `engine/Dockerfile`. See the notes there.
#
# The three Docker Hub library bases are named through this organisation's GHCR
# mirror (#33). `.github/workflows/mirror-base-images.yml` reads this file as
# well as `engine/Dockerfile` to decide what to copy, so this `FROM` line is on
# its list and Renovate's dockerfile manager tracks the pin here.
...
# Flutter is dev-only: app CI pins this exact stable release, while production
# has no reason to carry the large mobile SDK. This is what separates the two
# published variants.
...
# pod-provided tools (notably the `codex` full-access wrapper that
# `engine/Dockerfile` installs on top of this base when INSTALL_AI_CLIENTS=true)
# always win over a same-named binary a user or a tool like theclawbay installs
# into their persisted $HOME. Confirmed 2026-07-31: a theclawbay-managed
# ~/.npm-global/bin/codex shadowed the system wrapper under the old ordering,
# silently running Codex with its nested sandbox/approvals still active instead
# of the pod-level bypass.
...
# response publishes, same pattern as the step CLI and sccache installs below.
...
# Tailscale, the Infisical CLI and the Smallstep `step` CLI used to be
# installed here and are not any more.
#
# None of the three is a dev-pod tool. Tailscale joins one tailnet, Infisical
# brokers against one secrets manager, and `step` self-issues SSH certificates
# against the step-ca on one host — they are this estate's infrastructure, and
# a stranger running the published image gets three daemons' worth of surface
# for capabilities they cannot use. That was tolerable while the pod base was
# private. It is not the shape of a public artifact, and the published stack
# image is now exactly that: the generic release carries the agent CLIs and no
# estate integrations at all.
#
# So they moved one layer up, into the operator's private overlay, which is
# `FROM` the published digest and adds them back. That is the same relationship
# `engine/Dockerfile` stage 3 already states about the core — "the private path
# is the public path plus configuration ... now it is a digest" — applied to the
# whole stack. `docs/CUSTOMISATION.md` documents the mechanism publicly.
#
# Removing them is inert for a generic pod rather than a behaviour change:
# `entrypoint.sh`'s tailscale block is fully gated on `TAILSCALE_AUTH_KEY` or a
# persisted `/var/lib/tailscale/tailscaled.state`, so with neither it never
# touches the binary; the Infisical agent-auth helper is env-gated and
# auto-selects only when a secrets-manager identity is present; nothing invokes
# `step` unprompted. The estate helper scripts stay in the repo and stay tested
# — they are simply not activated in a generic image.
#
# `rclone` deliberately stays: it is a general-purpose tool with no estate in
# it, and the pod uses it for ordinary transfers.
...
# on PATH via the ANDROID_HOME entries in the ENV above. The committed Gradle
# wrapper (mobile/android/gradlew) drives builds; system gradle is only a
...
# Flutter + its bundled Dart SDK are installed only in the `full` variant. Keep
# the SDK under /opt so the runtime /home/sprooty bind mount cannot hide it. The
...
#
# The cadastre[mcp-client] bridge is deliberately NOT installed here: it is a
# private-stack, per-stream feature gated by `INSTALL_CADASTRE_MCP`, and this
# base is shared by main's stack, which does not carry it. `engine/Dockerfile`
# installs it on top of this base when the arg is set.
RUN pip3 install --break-system-packages --no-cache-dir uv ruff pytest

# Left as root on purpose. `engine/Dockerfile` continues from here and needs
# to install into /usr/local before it drops to `sprooty` at the very end; a
# base that ended as an unprivileged user would make every consumer's first
# instruction a `USER` switch.
USER root
