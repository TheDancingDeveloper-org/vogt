...
- Activity watcher: fires push to all subscriptions when any session enters `waiting-for-input`. A drift watcher does the same for newly raised drift, reading the same event bus — the vogt-core event follower republishes core changes onto it, so nothing polls the core twice. Both are off for any device that switched the kind off, and `idle_stall` / `agent_task_started` are off unless a device asks (FR-M2).
