...
# `vogt-agent-cli-install` manages under $VOGT_AGENT_CLI_ROOT. The
...
# (`DISABLE_UPDATES=1`), so `vogt-agent-cli-install` is the only writer.
#
...
quarantine_shadow() {
    ...
    if mv -f "$home" "$moved" 2>/dev/null; then
        echo "vogt-agent-clis: warning: persisted home copy $home would shadow image-managed $system; quarantined to $moved" >&2
        return 0
    fi
