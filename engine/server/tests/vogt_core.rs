//! The front door, tested against a stand-in core.
//!
...
struct Seen {
    ...
    method: String,
    /// What the door said about where clients arrive. Recorded so a
    /// test can tell "the door stated it" from "the caller did".
    public_url: Option<String>,
    api_path: Option<String>,
    /// The correlation id the door stated. Recorded so a test can
...
/// so a bare "the core saw nothing" assertion would be racing a
...
// -- an absent core costs Vogt features, never sessions -----
...
// -- what a client can learn before it asks -----------------------
...
// -- the core's changes arrive on this server's stream ------------
...
// -- the two halves agree about where the estate is ----------------
...
// ── The backup covers the engine, or says it does not ────────────
...
// ── one port, two protocols, and the front end on it ──────────────────────
...
async fn the_engine_serves_the_front_end_from_that_same_port() {
    // The one-port rule's first clause. `assets.rs` embeds `web/dist/` with rust-embed,
...
// -- the front door's identity: probes and the published URL ---------------
...
/// Every probe was once answered by the PWA catch-all, at 200, with HTML.
///
...
/// The first-run install surface: open, forwarded untouched.
///
...
/// The same failure as the probes, one namespace over.
///
...
/// The core cannot know this door's address, so the door states it.
#[tokio::test]
...
/// An exposure value carries no default, so an unconfigured door
...
// -- one request, two runtimes, one identifier ------------------------------
...
/// rather than queued. Once the burst drains, the door answers again.
#[tokio::test]
