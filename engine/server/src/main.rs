...
async fn main() -> ExitCode {
    ...
    let cli = Cli::parse();
    // The three settings the CLI parser owns resolve their env forms
    // (`ENGINE_CONFIG`/`ENGINE_BIND`/`ENGINE_TOKEN`) through config's helper
    // A `--config` flag wins over the environment; the bind and token
    // env forms are resolved inside `load`.
