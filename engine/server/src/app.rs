...
/// the PWA. Each is owned all the way to its leaves — `/mcp` by the
...
pub struct AppState {
    ...
    /// The durable assistant interaction log. Engine-local, so an
    /// absent core costs it nothing. `None` only when the store could
...
pub async fn router(cfg: Config) -> (Router, Arc<AppState>) {
    ...
    // One-shot backfill: index raw session logs that survived a hard
    ...
    // The durable assistant interaction log, opened like session
    ...
    // each change onto this server's bus as `VogtChanged`; the
    ...
    // core-state events. The follower above is what puts vogt-core's
    ...
    // schedule, so the horizon is a configured maximum rather than
    ...
    // daily schedule, so `history.db` and the raw `session-logs/` cannot
    ...
    let api_routes = Router::new()
        ...
        // Server-side speech. Both POST under `/api/assistant`, so the
        ...
        // Runtime-pinned agent CLIs: what is active, and the installer
    ...
    // answer them with index.html at 200 — which is what it once did.
    ...
    // The first-run install surface is open for the same reason the
    ...
    // The on-demand secret broker is outside the gate for the
    // same structural reason `/mcp` is: the caller is a *session*, which holds
    // no engine bearer (it is withheld on purpose) and presents its own
    ...
    let router = Router::new()
        ...
        // ones CORS refuses and the ones no route claims. It is also
...
/// schedule. Runs one sweep at startup and then every 24 hours, so a
...
/// Enforce the session-history retention horizon on a daily schedule,
