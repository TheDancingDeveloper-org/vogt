//! Provider-pluggable *workflow-engine* backend for agent tasks.
//!
...
//! The provider speaks a vendor-neutral REST + SSE contract; Vogt integrates
//! such an engine rather than re-building its execution features (sandboxes,
...
//!     "checkpoint_branch": "<branch>?", // the stage's git checkpoint branch
...
/// Present on a task means "run this in the engine"; absent means the
...
/// One event mirrored off the engine's live SSE stream.
///
...
pub struct ProviderEvent {
    ...
    /// named one. A workflow engine writes a checkpoint branch per stage
...
pub trait WorkflowProvider {
    ...
    /// Subscribe to a run's live event stream, yielding
...
struct SseEnvelope {
    ...
    #[serde(default)]
    state: Option<String>,
    /// The per-stage checkpoint branch the event reports. Accepts
...
/// [`AgentTaskRunOutcome`] and a durable [`AgentTaskRunConclusion`].
///
...
\n";
    ...
    async fn sse_events_surface_per_stage_checkpoint_branches() {
        // A stage event names the git checkpoint branch the engine wrote
