//! The front door's view of vogt-core.
//!
...
//! PTYs. If the core is down, sessions must keep running and the
//! surfaces that need it say so in as many words — so these routes
...
/// `/mcp` were minutes behind it. Shedding at the door is what keeps
...
/// How this door tells the core where clients actually arrive.
///
...
/// hand out an unreachable URL. It honours these only when configured
...
pub struct VogtCore {
    ...
    /// exposure value carries no default, and a URL this process
...
impl VogtCore {
    ...
    /// the engine as it shipped, and it is required to keep working.
...
pub async fn api(State(state): State<Arc<AppState>>, request: Request) -> Response {
    ...
    // Refuse dot-segment traversal. axum's `/api/vogt/{*path}`
...
/// The first-run install surface, passed through untouched.
///
...
/// The unauthenticated probes, at the same paths, answered by the core.
///
...
/// A caller could not tell from the status that it had been handed a
/// web page, which is why the rule is now that a probe is served or refused and
...
/// proxy-prefix escapes before the path is string-substituted and
...
/// Too many requests already open against the core. A refusal with a
...
/// `/api/config` and the absent-state views built on it.
...
// -- republishing the core's changes onto this stream -------------
        ...
        loop {
            ...
            for event in events {
                ...
                state.bus.publish(ServerEvent::VogtChanged {
                    ...
                    // can match on what changed and not only that
