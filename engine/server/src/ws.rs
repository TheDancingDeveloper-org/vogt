...
/// Bounded so one burst cannot build an unbounded frame.
    ...
    S: SinkExt<Message> + Unpin,
{
    ...
    // rather than allocating and copying each 64 KiB frame.
    let mut offset = 0;
...
/// (a warm reattach), and the cold-attach tail cap.
struct AttachAuth {
...
async fn handle_socket(
    ...
    // Stream the scrollback in zero-copy `Bytes` slices: `snapshot`
