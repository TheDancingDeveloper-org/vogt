...
fn runtime(state: &AppState) -> Result<Arc<AssistantRuntime>> {
    ...
    // A configuration that would hang refuses here instead, with the
    ...
    // API key.
    // With profiles this fires only when *no* configured route can
...
pub struct MessageReq {
    pub text: String,
    /// The raw recognised utterance behind a voice turn, before the
    /// repair pass. Sent by a voice client so the durable log can carry the raw
    /// form beside the repaired `text`. Absent for a typed turn.
    #[serde(default)]
    pub utterance: Option<String>,
    /// Which configured provider profile to run this turn against.
...
///
/// Cancellation-safe by construction: the composer's Stop button aborts
...
/// The durable interaction log, newest first.
///
...
/// route requires the `assistant` token capability, enforced in
/// `auth::required_capability`. Entries carry external content still delimited
/// as untrusted data, exactly as it was fed to the model.
pub async fn log(
    State(state): State<Arc<AppState>>,
