...
pub async fn subscribe(
    ...
    // network. Require https and refuse a non-routable host at the
    // subscribe boundary.
...
/// Hostnames are allowed (real push services use them); only an IP
...
pub fn spawn_vogt_drift_watcher(state: Arc<AppState>) {
    ...
    tokio::spawn(async move {
        loop {
            ...
            // more: the change event carries kind, entity kind, id and seq by
...
/// Is this bus event drift that is worth waking someone for?
...
mod tests {
    ...
    /// The requirement's "and for nothing else by default", at the filter that keeps it.
    #[test]
