//! The Vogt half of the assistant's toolbox.
//!
...
//! reaching them, so the capability matrix in docs/ENGINE.md defines the slice and intersected with
...
//! entirely, means the Vogt tools are not offered this turn. It does
...
/// exhaustive registry coverage, classification and these exact sets.
...
const TARGET_KEYS: &[&str] = &[
    ...
    "kind",
    "template",
    // Model selection meets the gate: a spoken "using GPT 5.6 medium" is half of what was
...
/// data, and that a write waits for a human.
...
/// Wrap Vogt-derived text as untrusted data.
///
...
pub fn delimit(operation: &str, text: &str) -> String {
    ...
    // the delimiter: work-item bodies are typed by people, imported
...
/// or re-open a delimiter wrapper. Case-insensitive — `</VOGT-DATA>`
