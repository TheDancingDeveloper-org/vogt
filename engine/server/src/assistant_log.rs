//! Durable, attributable log of every assistant interaction.
//!
...
//! turn and wrong for the question a durable log answers: what did or did not happen, who
//! drove it, and does it survive the process. The core's audit log records the *write* an
...
//! durable state, and the one that keeps it independent of the core: the log is the *engine's*,
...
//!
//! No audio: text and structure only. The
//! recognised utterance, its repaired form where the repair pass changed it, the
...
//! `<vogt-data>` / `<terminal-output>` / `<tool-error>` delimiters.
...
/// "both directions" property a fact a reader can select on rather
/// than infer.
#[derive(Debug, Clone)]
pub enum LogEvent {
    /// A recognised voice utterance. `repaired` is set only when the
    ...
    /// loop produced it, so external content keeps its delimiters.
    ToolResult { name: String, content: String },
    /// A pending action and its outcome. Logged once when proposed
