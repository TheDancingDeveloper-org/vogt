//! Server-side speech: the engine as a thin proxy in front of an
...
//!
//! The whole point of this module is that speech is configured *independently* of
...
//! on-device speech or, failing that, typed input. The two halves are
...
//! interaction log stays text-only (its own module says so, and this one
...
impl AssistantSpeech {
    ...
    /// in which case both routes 404 and the client falls back. A half
    /// is enabled when its base-URL list is non-empty; the config loader has
    /// already refused a key set against an empty list.
...
/// The audio is forwarded and never stored.
pub async fn stt(
...
/// unconfigured or every entry fails, so the client falls back. The
