...
/// The prefix a run prints to report what it cost: the remainder is
...
/// The default re-prompt budget for `output_schema` validation: enough
...
pub struct AgentTask {
    ...
    /// Event triggers this task listens on, each independently enabled.
    ...
    /// How many runs of this task may be in flight at once. A trigger
    ...
    /// The Vogt subject this task's runs are about.
    ///
    ...
    /// Approval points this task's runs hold at. Each is a first-class
    ...
    /// The one audited bypass: with it set, a run answers its own gates
    ...
    /// An optional JSON Schema the run's findings block must satisfy.
    ...
    /// recorded `partially-succeeded`. Only meaningful with
    ...
    /// The branch this task's runs are bound to and work on. Names the
    ...
    /// instead of a PTY: the engine owns the agent loop, and Vogt tracks
...
/// The default per-task concurrency cap: a task runs one run at a time
...
/// One event trigger a task listens on.
///
...
/// the trigger recorded on it so the conclusion path is unchanged and a
...
/// The five trigger kinds, tagged by `kind` so the wire spelling is the
...
/// their `summary` (`to`, `ref`, and — via the additive core enrichment —
/// `project`, `kind`, `labels`). `observation-new` and
...
pub enum TriggerSpec {
    ...
    /// A PR linked to a work item reached a terminal check status. When
...
/// What a matched trigger decided about a run: which trigger fired, the
...
pub struct AgentTaskRun {
    ...
    /// Why this run started, when it was a trigger: the trigger kind and
    ...
    /// carrying its own terminal state. This is the audit trail: an
    ...
    /// The typed verdict, once the run has one. `None` while running.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub outcome: Option<AgentTaskRunOutcome>,
    /// The durable conclusion record, written the moment the run ends.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub conclusion: Option<AgentTaskRunConclusion>,
    /// How many times the CLI was re-prompted to fix its findings against the
    /// task's `output_schema`. 0 when no schema was set or it validated
    /// first try.
    #[serde(default)]
    pub retries: u32,
    ...
    /// task's declared `branch`. The bound branch the conclusion
    ...
    /// observations off its live event stream. Each carries its own
...
pub enum AgentTaskRunTrigger {
    Manual,
    Scheduled,
    /// A core-state event trigger fired the run. Which trigger and which
    /// event are on the run's `trigger_detail`.
    Event,
    /// An explicit programmatic fire through the API.
    Api,
}

/// The audit of why a run started: which trigger fired it and, when a
...
/// The typed verdict of a finished run, a strict superset of the
...
/// * `Blocked` — the run stopped at an approval gate that failed closed;
...
/// The durable record a run leaves behind: the one place a reader, a
...
pub struct AgentTaskRunConclusion {
    ...
    #[serde(default)]
    pub retries: u32,
    /// The bound branch the run worked on (seen from the engine as the
    /// branch checked out in the run's workspace, or the task's
...
/// Why a run is being started, and what it should be bound to.
///
...
/// The live controls for one in-flight run: the handle its steer and
...
impl AgentTaskRegistry {
    ...
    /// `VogtChanged` (`vogt_core::spawn_event_follower`). This watcher
    ...
    /// start a run for each task that matched.
    ///
    ...
    /// Run a task now, recording it as an explicit `api` fire rather than
    ...
    /// What a start that cannot proceed *now* does, by trigger. A
    ...
    async fn start_run_inner(
        ...
        // The per-task concurrency cap. Counting live controls as well
        ...
        // A trigger fire binds the run to the item the event named, for
        ...
        // A task with a workflow-engine backend takes an entirely
        ...
        env.push(("VOGT_ENGINE_AGENT_TASK_ID".to_string(), task.id.to_string()));
        env.push((
            "VOGT_ENGINE_AGENT_TASK_RUN_ID".to_string(),
            run_id.to_string(),
        ));
        env.push((
        ...
        env.push((
            "VOGT_ENGINE_AGENT_TASK_CONTEXT_FILE".to_string(),
        ...
        // (unlike a session), so an agent that wants to write to
        ...
        // stats against where the run began. A workspace that is
        ...
        // so a client — and `why` — can see the run and the trigger
        ...
        // steer queue between rounds. Spawned for every run, not only
        ...
        // findings block the CLI writes and re-prompts it on a mismatch.
    ...
    /// The workflow-engine path: hand a run to an external engine and
    ...
    async fn start_workflow_engine_run(
        ...
        // Resolve the engine token from its file, if configured. A set
        ...
        match provider.create_run(&goal, repo_ref.as_deref()).await {
            Ok(provider_run) => {
                ...
                self.record_workflow_run(id, trigger, &origin.detail, &run, now)?;
                // Track the run off its live SSE event stream,
                ...
                // stream or poll fallback reports as a run observation.
    ...
    /// Write a workflow run's terminal verdict back: set its status,
    ...
    /// Record a workflow run as errored with a written reason. Builds a
    ...
    /// the number the concurrency cap is compared against.
    ...
    /// `output_schema`: how many re-prompts it took and whether it ever
    ...
    /// Record something a run reported about itself.
    ///
    ...
    /// prompt boundary. `interrupt` sends the CLI's cancel first.
    ///
    ...
    fn reconcile_run_history(&self) -> Result<()> {
        ...
        for task in tasks.iter_mut() {
            ...
            for run in &mut task.runs {
                ...
                // restart as "approved" would be the exact failure fail-closed
                // gates exist against.
...
/// an explicit programmatic fire — allowed only when the task has an
...
/// Match one enabled trigger against a core event.
///
...
/// Validate and normalise a task's declared triggers: trim the string
...
/// Normalise a submitted [`WorkflowEngineConfig`]: a config with a blank
...
/// tracking has exhausted its bounded retry budget.
...
/// stream and degrading to polling. A live stream sends progress or
...
/// Conclude a run whose live stream reported a terminal state. The
...
/// Poll a workflow run to a terminal state and record its conclusion.
///
...
/// notify phrase: exactly that phrase and nothing else.
...
fn default_command() -> Vec<String> {
    vec![
        "/bin/sh".to_string(),
        "-lc".to_string(),
        "printf 'Vogt scheduled agent task\\n\\n'; cat \"$VOGT_ENGINE_AGENT_TASK_PROMPT_FILE\"; printf '\\nPrompt file: %s\\nContext file: %s\\n' \"$VOGT_ENGINE_AGENT_TASK_PROMPT_FILE\" \"$VOGT_ENGINE_AGENT_TASK_CONTEXT_FILE\"; exec \"${SHELL:-/bin/bash}\"".to_string(),
    ]
}
...
/// Drive one run's gates and steer queue for its whole life.
///
...
///
/// Takes the registry rather than only the push manager: the matched
...
// --- outcomes, conclusion, and schema validation --------------------
...
/// `failed` wordings are kept verbatim from before typed outcomes existed so existing clients and
...
mod tests {
    ...
    #[test]
    fn a_default_notify_task_is_matched_on_the_default_prefix() {
        assert_eq!(
            accepted_notify_phrases(DEFAULT_NOTIFY_PHRASE),
            vec![DEFAULT_NOTIFY_PHRASE.to_string()]
        );
    ...
    #[test]
    fn find_notify_match_recognises_the_default_prefix_and_reads_the_message() {
        let phrases = accepted_notify_phrases(DEFAULT_NOTIFY_PHRASE);
        let line = "noise\nVOGT_NOTIFY: the sky is falling\nmore";
        let (idx, matched) = find_notify_match(line, &phrases).expect("the default is recognised");
        assert_eq!(matched, "VOGT_NOTIFY:");
    ...
    // --- outcomes, conclusion inputs, schema validation ---------------------
    ...
    // --- event triggers, matching, validation, audit -----------------------
    ...
    /// The non-fatal contract: a task whose workflow engine cannot be
    ...
    /// A workflow run's per-stage checkpoint branches are collected as run
    ...
    /// Non-fatal absence: recording a checkpoint against a run the registry
