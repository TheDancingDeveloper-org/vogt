...
/// the shape to forbid: absence must read as absence.
///
/// Empty is therefore the honest default, and it is the chat key's rule applied to the
...
pub struct AssistantProfile {
    ...
    /// Per-profile, because the hang this refuses is a property of one
...
pub struct Config {
    ...
    /// attach is still accepted. Off by default: a token in the query
    ...
    /// (loopback/RFC-1918/link-local) endpoint. Off by default: the
    ...
    /// Defaults to $HOME/.local/share/vogt-engine.
    ...
    /// agent session. The inline form is read once and removed from
    ...
    /// agent-auth helper so forge and CI credentials are
    ...
    /// enforces. Empty when nothing is declared, which turns the
    ...
    /// Named backends beside the one the flat keys above describe.
    ...
    /// Retention horizon for the durable assistant interaction log, in
    ...
    /// Server-side speech, configured **independently of the chat
    ...
    /// the client falls back to on-device or typed input.
    ///
    ...
    /// error, the same destination rule the chat key follows.
    ///
    ...
    /// the product rather than the core, and it is a second place
    ...
    /// The URL clients reach *this* server at.
    ///
    /// Configured, never inferred, and no default: the URL is an **exposure**
    /// value, not an allocation. This process binds a container port and is
    ...
    ///
    /// That argument was first made for the core, when the core was the door.
    /// It holds one hop out, because it did not weaken on the way: a fronted
...
pub fn load(
    ...
    // `ENGINE_BIND`/`ENGINE_TOKEN` are the env names; the CLI flag
    // still wins. Precedence: CLI flag > env > config file.
    ...
    // Server-side speech. Read independently of the chat keys above:
    ...
    // is refused, matching the chat key's destination rule.
    ...
    let agent_auth_helper = engine_env("ENGINE_AGENT_AUTH_HELPER")
        ...
        .unwrap_or_else(|| std::path::PathBuf::from("/usr/local/bin/vogt-agent-auth"));
    ...
    Ok(Config {
        ...
        state_dir: from_file
            .state_dir
            .map(std::path::PathBuf::from)
            .or_else(|| dirs_home().map(|h| h.join(".local/share/vogt-engine")))
            .unwrap_or_else(|| std::path::PathBuf::from("/var/lib/vogt-engine")),
        ...
        // sync forever, and getting it wrong is silent.
...
/// the one `engine_env` lookup. A `--config` flag on the command line still
/// wins over this in `main`.
...
///
/// The file form is the one the container uses: `vogt-agent-auth`
/// brokers secrets-manager secrets into files under a private temporary directory
...
/// value is the dangerous one: it is a private key in a place every
...
fn fcm_service_account(
    ...
    if inline.is_some() {
        ...
        unsafe { std::env::remove_var("ENGINE_FCM_SERVICE_ACCOUNT_JSON") };
        tracing::warn!(
            "ENGINE_FCM_SERVICE_ACCOUNT_JSON carries a private key in the process \
             ...
             (a path to the document) and rotate the key"
        );
    }
    Ok(inline)
}
...
/// Read an engine setting from the environment.
///
/// The prefix is `ENGINE_`. Several of these are extension points a
/// customiser has to type — `PUBLIC_URL`, `ASSISTANT_BASE_URL`,
/// `ALLOWED_ORIGINS`, `BIND` — so the name is kept short and predictable.
///
...
/// Mixing the two in one namespace would be a worse public surface, not a
/// better one.
///
/// One choke point rather than scattered `std::env::var` calls, so every
/// engine setting is read the same way.
...
/// Refuse at startup rather than at the first request.
///
...
/// One half of the server-side speech pipeline is enabled when its
/// ordered base-URL list is non-empty. A key set against an *empty* list is
/// refused at startup, the same destination rule the chat key follows: a
...
mod tests {
    ...
    fn a_speech_key_with_an_empty_url_list_is_refused_and_names_the_setting() {
        ...
        // provider — the same failure the chat key's rule refuses.
    ...
    fn the_url_list_is_off_unless_configured_and_parses_voicemodes_when_given() {
        ...
        // to forbid, so absence stays absence (see DEFAULT_STT_BASE_URLS).
...
mod prefix_tests {
    ...
    /// The document comes from a file when one is named, the inline
    ...
    /// the config path — are resolved by `load` through `engine_env`, so
    /// `ENGINE_TOKEN`/`ENGINE_BIND` in the environment are honoured.
    ///
    /// Kept in one test because these env names are process-global: two tests
    /// setting `ENGINE_TOKEN` in parallel would race. No other test in this
    /// crate touches these names or calls `load`.
    #[test]
    fn load_honours_engine_token_and_bind() {
        ...
        // `ENGINE_TOKEN` / `ENGINE_BIND` are honoured.
        unsafe {
    ...
    /// `ENGINE_CONFIG` resolves the config path, so `main` can prefer a
    /// `--config` flag and fall back to the env name.
    #[test]
    fn config_path_from_env_reads_engine_config() {
        let clear = || unsafe { std::env::remove_var("ENGINE_CONFIG") };
        clear();
