...
pub struct ScopedTokenConfig {
    ...
    /// and the form a deployment should use.
    ///
    /// A file rather than a value because the front-door token beside it
    /// commonly arrives through `ENGINE_EXTRA_TOKENS_JSON`, which is an
    ...
    /// `docker inspect`, which is exactly what `VOGT_CORE_TOKEN_FILE`
    /// exists to avoid. One brokered file per credential is already the
...
pub struct AuthorizedToken<'a> {
    ...
    /// its own. `None` means "no pairing", not "no core token": what
...
pub struct AuthorizedIdentity {
    ...
    /// The vogt-core token paired with it, or `None` if it has none.
    pub vogt_core_token: Option<String>,
}
