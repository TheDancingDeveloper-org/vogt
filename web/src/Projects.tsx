...
//
//   1. **Evidence comes before the act.** The drift requirement is unusually specific: a
...
//      submit without one. Nothing here composes a reason,
...
/**
 * How old the evidence behind one aggregate is.
 *
...
/**
 * Trust, wherever a thing carries one.
 *
 ...
 *
 * A proposal with no snapshot cannot show both sides, and the drift rule makes
...
const DriftCard: Component<{
  ...
  return (
    <article class={`vogt-projects-drift vogt-projects-drift--${props.proposal.status}`}>
      ...
      {/* Superseded is a reading aid, not a resolution. The proposal
      ...
      {/* The evidence, rendered open. Not a disclosure: the drift rule makes seeing
...
// -- the URL, which is where a view lives --------------------------
...
const Projects: Component<Props> = (props) => {
  ...
  // The registry is a place to find a project, so it filters and sorts.
  ...
  // -- drift arrives rather than being asked for -------------------
  //
  ...
  // somebody's sentence to a background refresh is exactly the reason the
  // write rule makes them type.
  ...
  // -- the repo picker ------------------------------------------------------
  // Enumerates what the acting credential (a linked PAT, else the file
        ...
        title={(
          <>
            {/* The route says where it is as a heading and not only as a
                ...
                secondary route needs the title. */}
                ...
                <section class="vogt-projects-project">
                  ...
                  <div class="vogt-projects-panels">
                    ...
                    <section class="vogt-projects-panel">
                      ...
                      {/* The contract is opt-in, so this status is
      ...
      <Show when={place().view === "import"}>
        <section class="vogt-projects-import" aria-label="Import a repository">
          ...
          {/* Name the repository, or pick from the ones your linked credential
              can see. The picker is an
              *enumeration* of what your token reaches, not a candidate
              crawl: the token is the scope, and you still confirm
              what is imported. */}
          <p class="vogt-projects-note">
            ...
            forge — it changes nothing upstream.
          </p>
