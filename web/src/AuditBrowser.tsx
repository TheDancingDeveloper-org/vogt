// The audit browser and the notification inbox.
//
...
//      later. A row that showed the operation and dropped the
...
//   4. **A query is a place.** The whole filter set lives in the URL,
//      which is what makes the link-in from a work item work: a work item's detail
...
//      the server's own reason, never as an empty audit log. Of every
 ...
 * the backlog's windowing arithmetic needs.
 */
...
/** The collector behind the notification inbox. Its absence from a sweep is a real answer. */
...
// escalates. So every fetcher returns its failure as a value, and
...
/**
 * What the sweep behind the inbox says about itself.
 *
...
// -- the URL, which is where a query lives -------------------------
...
const AuditBrowser: Component<Props> = (props) => {
  ...
  //
  // The audit requirement's second clause is a link from a work item into this browser. The
   ...
   * this surface is arrived at. Dropping the bound and querying
  ...
  // -- live, and honest about the difference -----------------------
  //
  ...
  return (
    <div class="vogt-surface vab">
      {/* The shared working header: this route had no title
          ...
          <div class="vab-filter-grid">
            ...
            <label class="vab-field">
              ...
              {/* A list rather than a select: `audit.list` matches the operation
                  ...
                  product is the kind of thing the scale requirement is about. */}
