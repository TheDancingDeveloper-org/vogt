...
const DiffView: Component<{
  ...
  // diff renders inline instead, and flips back as the host grows.
...
const GitTab: Component<Props> = (props) => {
  ...
  // from spinning the other's button.
  const runGitOp = async (
    ...
    try {
      ...
      // renders (and its Git markers) — reflect it there too.
  ...
  // An error stays until the next action replaces it.
  createEffect(() => {
  ...
  // left pending for whichever repo does.
  createEffect(() => {
  ...
  // unstaged — for the per-row controls.
