...
const CommandPalette: Component<Props> = (props) => {
  ...
  const baseCommands = (): Command[] => {
    const commands: Command[] = [
      // -- Vogt --------------------------------------------------------
      //
      // Every entry here *navigates*. None of them writes: the palette
      // reaches a mutating verb by opening the view that collects
      // its reason, and never by executing it — a palette entry cannot type
      // a reason any more than a button can, and the write rule is about where
      ...
      // The palette's mutating verbs. Every one of these *opens the view that
      ...
      // too, so the keyboard is never a poorer map than the chrome. Each
  ...
  // Compose an existing session into the active terminal workspace.
  ...
  const allCommands = (limit?: number): Command[] => {
    ...
    return [
      ...
      // ahead of a work item that merely happens to tie.
      ...sessionCommands(),
      ...splitCommands(),
      // A registered project, opened on its own page. The palette
      ...
      // reaches and what a shared link reaches are the same place.
