...
const TreeNodeView: Component<NodeProps> = (props) => {
  ...
  // survives a tab switch and a workspace remount instead of collapsing.
  ...
  // exactly it — the rest of the tree keeps its loaded, expanded state.
  createEffect(() => {
  ...
  const toggle = () => {
    ...
    // open over a now-hidden folder is orphaned on screen. The children
...
const FileTree: Component<Props> = (props) => {
  ...
  // of the tree keeps its expanded state. The root level is served by
  ...
  // and visibility into one debounced wake.
  const revalidate = () => {
    void refetch();
    void refetchGitStatus();
  };
