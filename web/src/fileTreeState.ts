...
// switch and a workspace remount. Before this, folder expansion and the
 ...
 *  editor keeps the full viewport width. */
 ...
 *  collapsed on a narrow (phone) viewport and open on a roomy one. */
...
function readSidebarCollapsed(): boolean {
  try {
    return defaultSidebarCollapsed(localStorage.getItem(SIDEBAR_KEY), isNarrowViewport());
  } catch {
...
// the tree keeps its loaded, expanded state instead of collapsing. "" is
// the root level.
