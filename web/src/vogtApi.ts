// The Vogt half of the product, as the PWA sees it.
//
...
// token paired with this client's front-door token. The browser
...
//      the same rule the legacy GUI has always kept.
...
//      have collected a reason the *user* typed (the write rule: a mutating
//      operation appears only through a view that collects one). `call()`
...
/** What the front door says when vogt-core is not there. */
...
async function call<T>(
  ...
  const read = async (
    ...
    if (!res.ok) {
      ...
      // stranded panel per Vogt surface. Everything else, a 403 most of
...
export interface BoardListResult {
  ...
  /**
   * Set for a project scope: "unlinked" is the machine-readable
  ...
  /**
   * Declared rows excluded because their project is unlinked (the
   * link-or-publish withdrawal); on an unlinked project scope, the open native items a
...
export interface RankedView {
   ...
   * On an unlinked project scope, the count a link or publish
...
export interface SessionSummary {
  ...
  /** What the session was *asked* to run — not what the agent inside
 ...
 *  and observed are kept separate, never merged, so the surface shows
...
/** One contradiction between a work item and its git evidence. Derived
 *  and read-only: Vogt reports the disagreement, it does not reconcile
 *  it. */
 ...
 *  `task_conclusion_available` records that the task-run conclusion is an
...
export interface WorkDetail {
  ...
  /** Branches this item is worked on, declared and observed side by
 ...
 *  what a reader is required to see before resolving one. */
...
export interface DriftProposal {
   ...
   *  condition that raised it. Not a resolution: the proposal is
...
/** One recorded reference between projects.
 *
 *  Records *which projects reference which* and stops there: no lockfile is
 *  parsed and no package version is resolved (by design), so there is no
...
/** One thing a collector found, as the observed store holds it.
 *
...
export const notifications = (params: Record<string, unknown> = {}) =>
  ...
  // different answer from "nothing to say".
...
/** The ordered feed, from a cursor and optionally about one entity.
 *
 *  The cursor feed behind view freshness: a surface that polls this and reports how
 ...
 *  picker is an *enumeration* of what the acting credential can see, not a
 *  candidate crawl: the token is
...
/** Enumerate the repositories the linked credential can see, so a
 *  person can pick which to import. Lists by credential, never by
...
/** Whether the acting actor has linked a forge account, and as whom. */
...
/** Link the acting actor's own forge PAT. Validated against the forge,
...
/** Register an existing folder or repository as a project. */
...
/** Run the collectors over the registered projects. */
