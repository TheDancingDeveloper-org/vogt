 ...
 * A 403 says the credential is good and the capability is missing; a
 ...
 * conflation of "offline" with "unauthorized" that this path exists to
 * keep apart.
 */
 ...
 * dead error and a shell that still believes it is signed in.
 *
 ...
 * The base is prefixed onto every request URL with the bearer token
...
export interface HistorySearchResult {
  ...
  // the archived index. Older engines omit it; treated as false.
  live?: boolean;
}
...
export interface PushPreferences {
  ...
  // New drift raised in vogt-core. Absent from any subscription
...
/** The kebab-case kinds an event trigger can be (engine `TriggerSpec`). */
 ...
 * Tagged by `kind`; the per-kind filter fields sit alongside it, and
...
/** Why a run started, when a trigger fired it (engine `RunTriggerDetail`). */
...
/** One thing a run reported about itself (engine `AgentTaskFinding`). */
...
/** One choice a gate offers (engine `gates::GateOption`). */
...
/** A declared approval point on a task (engine `gates::GateSpec`). */
...
/**
 * A gate instance on a run (engine `gates::GateRecord`). `state` is the
...
/** The typed verdict of a finished run (engine `AgentTaskRunOutcome`). */
...
/** Files/insertions/deletions a run left on its bound branch. */
...
/** What a run cost, when the CLI reported usage. */
 ...
 * the typed outcome plus how long it ran, what it exited with, how many
...
export interface AgentTaskRun {
  ...
  // Why the run started, when a trigger fired it (event/api runs only).
  ...
  findings: AgentTaskFinding[];
  gates?: AgentTaskGate[];
  // The typed verdict and durable conclusion, present once the run ends.
  outcome?: AgentTaskRunOutcome;
  conclusion?: AgentTaskRunConclusion;
  // Schema-validation bookkeeping: re-prompts spent and whether the
...
export interface AgentTask {
  ...
  // Event triggers this task listens on, and how many runs may be in
  ...
  // An optional JSON Schema the findings block must satisfy, where the
...
export const api = {
  ...
  getSession: (id: string, signal?: AbortSignal, tailBytes?: number) =>
    req<SessionDetail>(
      ...
      // tail_bytes avoids fetching and base64-decoding up to 4 MiB.
      tailBytes
  ...
  /** The runtime-pinned agent CLIs; `upstream` also asks npm's latest. */
  ...
  // `profile` names which configured backend runs this turn. Omitted
  ...
  /**
   * Server-side STT: upload captured audio, get text back. Used by a
  ...
  /**
   * Server-side TTS: send reply text, get an audio blob to play. Used
   * by a client with no on-device synthesis. Throws `ApiError` with
   * `status: 404` when unconfigured, so the caller falls back.
   */
...
export interface PublicConfig {
  ...
  /**
   * Whether the server-side speech routes are configured. A client
  ...
  /**
   * The provider profiles a request may name. A name and the model
...
/** One runtime-pinned agent CLI as the engine reports it. */
...
export function subscribeEvents(
  ...
  (async () => {
    try {
      ...
      // dead credential until the backoff gives up.
      if (!res.ok || !res.body) {
      ...
      while (!cancelled) {
        ...
        // looking live. That is exactly the liveness rule's "a lost stream shall be
