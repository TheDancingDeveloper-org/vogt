...
export class DemoStore {
  ...
  private async vogt(path: string, method: string, query: URLSearchParams, body: Record<string, unknown>): Promise<Response> {
    ...
    // The demo has no session behind it: signing out revokes nothing, and
    // says so the way the core would on a surface with no token.
    if (path === "/auth/logout" && method === "POST") return json({ revoked: false, token: null });
