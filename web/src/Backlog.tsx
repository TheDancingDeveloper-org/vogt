...
//   2. **A filtered view is a place.** Filters live in the URL, so
...
// Neither has a default, a placeholder that doubles as a
...
//      unreachable Vogt that says so with the server's own reason.
//
...
/** Saved filters are per-client state in v2 (`REQUIREMENTS.md` §3). */
 ...
 * DOM, so the row count stops driving layout cost.
 */
 ...
 *
 *  `q` is the free-text search; the `not_*` keys carry the exclusion
 *  set. An older build has none of them in its own `URL_KEYS`, so it
...
/** The Board's refresh cadences, offered here too so a ranked view left
...
/** The six required filters, plus the view they apply to, plus the free
 *  text and exclusions the triage grammar gained. */
interface Filter {
  view: ViewName;
  project: string;
  kinds: string[];
...
/**
 * How old the evidence behind this answer is.
 *
...
// -- the URL, which is where a filtered view lives -----------------
...
/** Free-text match for the backlog search: the ranked row's title and
...
// escalates. So the fetchers return the failure as a value.
...
const Backlog: Component<Props> = (props) => {
  ...
  // Which saved lens (by name) is the default. Persisted alongside the
  ...
  // The refresh cadence, a per-device preference like the Board's. It
  ...
  // the failure the freshness rule is actually about.
  //
  ...
  const viewAge = createViewAge(() => {
    ...
    return {
      ...
      // Off (the default) is a view with no poll, which the view-age badge renders as
  ...
  // The link-or-publish CTA: a project scope whose server answer says
  ...
  /**
   * The workflow-state filter, applied here rather than in the query.
   *
  ...
  const visible = createMemo<RankedEntry[]>(() => {
    ...
    // Search and exclusion join the page-only state filter over
   ...
   *  the loaded page — so the count line can say "N of M loaded". */
  const pageNarrowed = () => {
    const active = filter();
  ...
  // A genuinely new page — a refetch, a different query — is a different
  ...
  // The whole of quick-create's refusal, in one place: no title, no reason, no
  ...
  const beginRowAct = (ref: string, kind: RowActKind) => {
    ...
    // justify a different write on a different row.
    setRowReason("");
    // "Start a session…" is reachable from the collapsed row now; the
  ...
  const rowActions = (entry: RankedEntry) => (
    <div class="vogt-backlog-row-actions">
      ...
      >
        ...
        {/* "Start a session…" lives in the collapsed facts row on a desk;
   ...
   * transition is reported with the server's own reason (which names the rule
  ...
  // -- bulk label ---------------------------------------------------
  //
  // The backlog requirement says "bulk transition/label". Bulk transition was built and bulk
  ...
  // while anything is selected. Above the list, the bar scrolled out of
    ...
    <Show when={selected().length > 0 || bulkOutcomes().length > 0}>
      ...
      <div class="vogt-backlog-bulk">
        <Show when={selected().length > 0}>
          ...
          {/* Bulk label, on the same batch and under the same rule. */}
          <form
            class="vogt-backlog-bulk-row"
  ...
  // -- what is filtering this view, said as chips ---------------------------
  //
  ...
  const activeFilterChips = createMemo(() => {
    ...
    const ex = active.exclude;
    // Exclusions read as negations, so a reader never mistakes one for
  ...
  // -- the exclusion builder: one facet, one value, over the loaded page.
  ...
  return (
    <div class="vogt-surface vogt-backlog">
      <SurfaceHeader
        ...
        // belongs to the ranked work rather than to the chrome over it (the
        // same first-viewport treatment the Board has).
        collapseControls
        collapseHonesty
      ...
      <ProgressiveFilters
        ...
        lenses={(
          /* A combination worth keeping is a lens. */
          <SavedLenses
            prefix="vogt-backlog"
      ...
      >
        <label class="vogt-backlog-field vogt-backlog-field-wide">
          <span>Search</span>
          {/* Free text over the loaded page's titles (and adopted item
            ...
            <span>
              ...
              >
                {/* State, search and exclusion are page-only, so the count says
                    so out loud: N of the M rows this page loaded. */}
            ...
            <Show
              ...
              fallback={
                <div class="vogt-backlog-empty">
                  ...
                  >
                    {/* An empty ranking that offers a way forward: it
                    ...
                    {(entry, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* The title wraps to what it says, and the
