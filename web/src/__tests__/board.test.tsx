// The board's interaction contract.
//
...
describe("the columns are the workflow's states, never hard-coded", () => {
...
describe("optimistic, then whatever the server said", () => {
  it("moves the card on the drop, before anything is written", async () => {
    ...
    // reason has not been typed yet, so there is nothing to write.
  ...
  it("never persists a state the server refused", async () => {
    ...
    // "Never persist a refused move", checked where a client would break it: the only
...
describe("the same move, from the keyboard", () => {
  ...
  it("opens the item on Space and on a single click of the card body", async () => {
    fakeVogt();
    const space = board();
...
describe("Stage 9 — the phone board is one workflow state at a time", () => {
  ...
  it("fades the state-pill row's right edge when it overflows", async () => {
    const desktop = onAPhone();
    try {
      fakeVogt(twoStates);
      const { container } = board();
...
describe("a card with more to say expands where it stands", () => {
...
describe("trust on every card, and never blank", () => {
  ...
  it("says how old the view is, and never calls a polled view current", async () => {
    ...
    // The freshness half, on the board: the requirement asks for trust
    ...
    // The "How this view stays current" note was collapsible chrome over
    // the cards, so it was removed from the surface. The freshness itself — the
    // half actually required — is still stated on the badge above, never
...
describe("quick-create on the board, which is the half that was missing", () => {
...
describe("quick-create has a binding, now that there is one to bind", () => {
  it("opens on `n` (the legend now lives in the `?` help, not over the cards)", async () => {
    fakeVogt();
    const { container } = board();
    await waitFor(() => card(container, "WI-1"));

    // The always-visible keyboard legend was moved off the surface, so
  ...
  it("closes the quick-create form on Escape", async () => {
    fakeVogt();
    const { container } = board();
    await waitFor(() => card(container, "WI-1"));
...
describe("the filter set is the URL", () => {
  it("restores every one of the six filters from a pasted link", async () => {
    ...
    // The requirement names six: project, workflow state, type, label, initiative,
  ...
  it('reads a hide-finished selection as one "Finished hidden" chip', async () => {
    fakeVogt();
    const view = board();
    await waitFor(() => expect(columnNames(view.container).length).toBeGreaterThan(0));
...
describe("a combined filter is nameable and recalled", () => {
  ...
  it("survives a reload, because it is per-client state and says so", async () => {
    ...
    // thing carried across, which is the "per-client in v2" rule.
    const second = board();
    await waitFor(() =>
  ...
  it("confirms before overwriting a lens whose name is already taken", async () => {
    fakeVogt();
    // The overwrite prompt is the shared accessible confirm, threaded in
  ...
  it("keeps a forgotten lens recoverable for a short window", async () => {
    fakeVogt();
    const view = board();
    await waitFor(() => expect(columnNames(view.container).length).toBeGreaterThan(0));
...
describe("an outage is not an empty board", () => {
...
// -- swimlanes, which were asserted as a filter value -----------------------
//
...
// round-trips through the URL in the filter-set block above, and a board that
...
describe("a swimlane is a grouping, not a filter value", () => {
...
describe("the per-column WIP count is what is in the column", () => {
...
describe("lanes and columns collapse, and the layout is this client's", () => {
...
// -- a card can be moved without a drag ------------------------------------
//
...
// -- a same-column, other-lane drop is explained, not swallowed ------------
//
