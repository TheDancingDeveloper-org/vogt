...
describe("migrateStorageKeys (#271 identity rename)", () => {
  ...
  it("migrates the keys the app-theme work (#299) did not: expanded tree and backlog poll", () => {
    localStorage.setItem("mydevenv2.fileTree.expanded.v1", '{"src":true}');
    localStorage.setItem("mydevenv2.backlog.poll.v1", "off");

    migrateStorageKeys();

    expect(localStorage.getItem("vogt.fileTree.expanded.v1")).toBe('{"src":true}');
    expect(localStorage.getItem("vogt.backlog.poll.v1")).toBe("off");
    expect(localStorage.getItem("mydevenv2.fileTree.expanded.v1")).toBeNull();
    expect(localStorage.getItem("mydevenv2.backlog.poll.v1")).toBeNull();
  });
  ...
  it("leaves intentionally-kept historic keys alone", () => {
    // Legacy tab data, the #299 legacy-fallback keys, notification-channel-style
    // ids and the like are not in the rename map and must survive untouched.
    localStorage.setItem("mydevenv2.tabs.v1", "legacy-tabs");
    localStorage.setItem("mydevenv2.appTheme.v1", "legacy-theme");
    localStorage.setItem("mydevenv2.commandPalette.recent.v1", "[]");
    localStorage.setItem("mydevenv2.fileTree.sidebarCollapsed.v1", "1");

    migrateStorageKeys();

    expect(localStorage.getItem("mydevenv2.tabs.v1")).toBe("legacy-tabs");
    expect(localStorage.getItem("vogt.tabs.v1")).toBeNull();
    // The #299 keys have their own inline fallback; this pass does not touch
    // their historic values.
    expect(localStorage.getItem("mydevenv2.appTheme.v1")).toBe("legacy-theme");
    expect(localStorage.getItem("mydevenv2.commandPalette.recent.v1")).toBe("[]");
    expect(localStorage.getItem("mydevenv2.fileTree.sidebarCollapsed.v1")).toBe("1");
  });
