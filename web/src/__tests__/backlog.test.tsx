...
describe("bulk label, which was the half of the clause nothing called", () => {
  it("writes `work.update` per item with the reason typed for the batch", async () => {
    ...
    // shape the bulk transition has, which is what the requirement asks for.
...
describe("bulk transition, which was built and asserted by nothing", () => {
...
describe("quick-create refuses to submit without a typed reason", () => {
...
describe("trust on every ranked row, and never blank", () => {
...
describe("the filter set is the URL, and can be named", () => {
...
describe("a ranked row is content-sized and expands where it stands", () => {
  const twoRows = {
    "GET /backlog": {
      body: {
        items: [
...
describe("an unreachable backlog is not an empty one", () => {
...
// -- the ranking's explainable half -----------------------------------------
//
...
describe("the `why` panel is the contributions, not a second copy of the score", () => {
