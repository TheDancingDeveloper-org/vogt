// The server-side speech pipeline (VOICE_POC §3.5).
//
...
describe("the server-side speech pipeline", () => {
  ...
  // The take is meant to auto-submit hands-free — ending it must call
  // `send` exactly once, with the transcript put back through the domain
  // repair pass first, and no manual "Send" tap in between.
  ...
  // An empty transcript is a take that heard nothing — it must send
  ...
  // Abandoning a take — leaving the surface mid-recording — must drop
  ...
  it("offers no microphone and stays silent when server speech is unconfigured", async () => {
    ...
    // with no on-device speech degrades to typed input with no error.
