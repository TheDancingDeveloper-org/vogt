// Scale on the board, both halves: the columns window rather than truncate,
...
describe("a long column windows rather than truncating", () => {
  ...
  it("moves focus to a card the window has not drawn", async () => {
    const total = 300;
    fakeVogt({
      "GET /workflows": { body: { workflows: [chain("feature", ["open", "done"])] } },
      "GET /work": { body: { items: column(total), total } },
  ...
  it("rolls a refused move back into a windowed column and says why there", async () => {
    ...
    // Optimistic placement and the designed absent state together, on the column shape most likely to lose
 ...
 * was walked, times its size, which is exactly the quantity the scale
 * requirement's second clause is about.
 *
...
describe("the filter and drag paths do not degrade with backlog size", () => {
  ...
  it("re-projects nothing while a reason is being typed into the composer", async () => {
    ...
    // that had to survive the optimisation.
