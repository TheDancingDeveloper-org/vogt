// The repair pass between a recognizer and the composer.
//
// These are the utterances from the voice POC script, in the forms a
// recognizer actually returns them. They are the half of the promised
...
const PROJECTS = ["rustnzbd", "rusttorrent", "vogt", "calendar", "sandbox"];
...
describe("project slugs", () => {
  it("rejoins a slug the recognizer split into words", () => {
    // U2, and the reason voice was adopted unproven: `rustnzbd` is not a
  ...
  it("repairs a slug that was heard slightly wrong", () => {
    ...
    expect(repairUtterance("what about calender", PROJECTS).text).toBe(
      "what about calendar",
    );
  });
