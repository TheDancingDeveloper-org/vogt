// The drift inbox's ordering rule.
//
...
// clause of the drift requirement and defeat the whole of it — so the assertions below are
// about what is on screen and where it is relative to the controls, not about
// whether a panel exists somewhere on the page.
//
// `projectPage.test.tsx` mounts this inbox for the project page and asserts that the
...
describe("both sides, with provenance and age, before anything can be done", () => {
  ...
  it("refuses to offer an act at all when there is no evidence to weigh", async () => {
    ...
    // Not a disabled button — no form. The drift rule makes seeing both sides the
...
describe("one proposal, one typed reason, one act", () => {
...
describe("superseded is a reading aid, not a resolution", () => {
  it("says a later sweep stopped reproducing it, and still offers the act", async () => {
    ...
    // were cleared by a person, and supersession changes what they can see, not who
    // decides.
...
describe("a reference read from an item's own text", () => {
