// One work item, and the inline edit the requirements name and nothing implemented.
//
...
// something — an edit, an outage, an answer to a waiting session. The item page requirement is a
...
describe("one item, one page, and everything about it on it", () => {
  ...
  it("shows the collected evidence behind its ranking, per input", async () => {
    // The ranking's shape: the contributions, not a single score. A total on its
  ...
  it("offers the control that starts a session on this item", async () => {
    // The form is collapsed by default, behind a button that names it.
  ...
  it("keeps all six on one page, at once", async () => {
    ...
    // product the requirement was written against.
    fullItem();
    const { container } = detail();
...
// -- branches, declared and observed, kept separate -------------------------
//
...
// as drift rather than being merged away. The panel's job is to keep
...
// -- the derived git story — branch / PR / phase + drift ---------------------
//
// With branches and the PR edge observed, the item answers "where
...
// -- the item's "state history" ---------------------------------------------
//
// §6.2 recorded this as the one clause of the item page the page did not answer: the
...
describe("the item's own state history, on the item page", () => {
  ...
  it("says an item that has never moved has never moved", async () => {
    // The absent-state rule on this panel: the absence is designed. A creation and no
...
describe("an inline edit renders optimistically and the server decides", () => {
  ...
  it("does not re-derive a refused value when the editor is reopened", async () => {
    ...
    // Close and reopen: "never persist, cache, or re-derive".
...
// -- the Move-to transition, from the detail page ---------------------------
//
...
// -- the editor now changes assignee, effort, labels and body ---------------

describe("the editor changes assignee, effort, labels and body", () => {
  it("opens the actor picker as a keyboard-operable select on the item's assignee", async () => {
    ...
    const { container } = detail();
    // The chip shows the actor's display name, and keeps the raw
    ...
    // own — the a11y property the editor needs from the picker.
...
// -- the page leads with the item, not its machinery ------------------------

describe("the item page collapses its chrome and leads with comments", () => {
...
// The trust rule's second clause, which §6.2 recorded as newly checkable and not met:
...
describe("observed evidence says whether it has settled", () => {
...
describe("the observed panel tells an outage from no evidence", () => {
...
describe("the item page tells an outage from an empty item", () => {
...
//
// The phone demo ends "open it, unblock it", and until now unblocking
...
describe("a session waiting for input can be answered from the item", () => {
...
// -- the item, its session's liveness, and the way to the terminal ----------
//
// §6.2a said this needed "a fixture nobody has written" — an engine stub
// beside the Vogt one — which the waiting-session work above now provides. The badge
// and the control are the forward leg; the terminal's link back is the
// return leg and is asserted in `terminalLink.test.tsx`.

describe("a work item shows what its session is doing, and how to reach it", () => {
