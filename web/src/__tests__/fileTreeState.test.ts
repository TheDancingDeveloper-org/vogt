...
describe("fileTreeState — persistent tree view state", () => {
  it("remembers folder expansion and survives a remount (re-read from storage)", () => {
    ...
    // expansion the reader had must come back rather than collapsing.
    resetFileTreeState();
    expect(isExpanded("src")).toBe(true);
    expect(isExpanded("src/components")).toBe(true);
  ...
  it("invalidates only the affected folder, not its siblings", () => {
    createRoot((dispose) => {
...
describe("sidebar drawer default — phone overlay", () => {
...
describe("workspaceVersion — cross-surface invalidation", () => {
