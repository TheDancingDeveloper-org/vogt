// The shell, and what a pasted link does to it.
//
...
// half found broken for *every* surface, and the half §6.2a records as
// "mounted by nothing". So this file mounts the shell, behind the same route
// table `index.tsx` routes with, and asserts the property addressability actually
...
describe("a pasted link opens the surface it names", () => {
  ...
  it("opens an audit query, narrowed to what the link carried", async () => {
    // The link the item page hands out: `#/audit?ref=WI-1`. Addressability names the
  ...
  it("orders the rail's Running sessions by attention, like Sessions", async () => {
...
describe("shared live steering", () => {
  it("labels desktop and phone counts, then refreshes them from the live stream", async () => {
    ...
    await waitFor(() => {
      ...
      // running sessions as rows, so the waiting session is drawn once
...
describe("the assistant is not there to be reached when it is not provisioned", () => {
...
describe("Sessions owns its machine workspace", () => {
  it("keeps the machine workspace and internal pane bar around a deep-linked tool", async () => {
    ...
    await surface(container, ".history-view");
    // The Live Sessions sub-panel was removed; Sessions still owns the
