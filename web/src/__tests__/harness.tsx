  ...
  return async (call) => {
    ...
    return {
      body: {
        ...
        // The honesty pair. A legacy `/work` fixture describes declared
        ...
        // The link-or-publish marker rides through unchanged, so a
...
export interface FakeVogt {
  ...
  /** The engine's SSE stream, which view liveness is about. */
  stream: FakeStream;
...
// table above — it is the *other* half of this product, and the half the
// liveness rule names. `store.ts` reads it with `fetch` and a `ReadableStream` rather than
...
/** One open drift proposal, carrying the evidence the drift rule makes acting on it
...
export function fakeVogt(routes: Routes = {}, engine: Routes = {}): FakeVogt {
  ...
  const stub = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (!url.pathname.startsWith(VOGT_PREFIX)) {
      ...
      // answering 404, which is what makes the "the engine is away"
...
export interface Mounted {
  /** The current URL, exactly as the router holds it (addressability's subject). */
  url(): string;
