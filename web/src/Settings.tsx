...
// which is where the push rule's "and for nothing else by default" is actually kept.
...
const Settings: Component<Props> = (props) => {
  ...
  // Runtime-pinned agent CLIs: the engine's report, and one move at a time.
  ...
  const refreshAgentClis = async () => {
    try {
      ...
      // An engine without the agent-CLIs route answers 404 here; that is "no runtime
  ...
  const clearAuth = async () => {
    ...
    // refused, so it travels the same way: `signOut` clears both token
