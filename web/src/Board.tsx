// The board, with the interaction contract it is only honest with:
// live-ish, addressable, optimistic and server-authoritative,
// and as much of the swimlane requirement as columns and lanes need to be usable.
//
...
//
//  2. **The drop collects the reason, at the drop site.** The write rule
//     says a mutating operation appears only through a view
...
//     re-derived. The only thing this file
...
//     engine's SSE stream as `vogt-changed`, so a transition
...
//     once.** Two halves of the scale requirement, and they are the same decision.
//
...
// -- filters, which are the URL ------------------------------------

/**
 * The exclusion (negative) half of the filter set.
 *
...
interface Filters {
  ...
  assignee: string;
  lanes: LaneMode;
  /** Free-text search over the loaded cards' title and body. */
  q: string;
  /** The negative filter set. */
 ...
 *
 *  The `not_*` keys carry the exclusion set; an older build has none of
...
// so. The backlog and bugs views have had this for some time; the board —
// which the saved-filter requirement names first, and whose filter set is the
// more elaborate of the two — had none.
//
...
/** Free-text match for the board search: title first, then body/ref, so
...
/** How many exclusion values are set across every facet. */
 ...
 * an aggregated view of exactly the kind the freshness rule names, so a card that showed
 ...
 *  all one whose move the server refused. */
...
// also the part the scale requirement's second clause is about — "the board's filter and
 ...
 * server still says it is in (the optimistic half). Only the ref and the
...
/**
 * A move Vogt accepted, said where the reader is working. Like a
...
const Board: Component<Props> = (props) => {
  ...
  // The default lens is applied only on a bare load: a non-empty query
  ...
  // is dropped once the reader reaches the end.
  ...
  // The honesty pair: how many things the Backlog would consider for
  ...
  // The link-or-publish CTA: "unlinked" on a project scope means the
   ...
   * state. `beginMove` refuses a same-state move, so the drop would
   ...
   *  not merely true ("roll the item back visibly"). */
  ...
  // broken without it, both of them the addressability rule's actual subject:
  //
  ...
  // unchanged items across a refetch. JSON is adequate here: both sides
  ...
  const mergeItems = (fresh: WorkItem[]): WorkItem[] => {
    ...
    return fresh.map((item) => {
      ...
      // Compared by value, not by updated_at: two writes in the same
  ...
  // republishes onto the stream this client already has open, so a
  ...
  /**
   * The cards left after the two client-side filters.
   *
   ...
   * per-column "Nothing here". Not while a read is in flight (that is
  ...
  const boardEmpty = createMemo(() => {
    ...
    // Client-side search/exclusion can empty the loaded set even
  ...
  /**
   * The honesty gap: the Board draws declared cards, but the estate's
  ...
  /**
   * Open the move composer from the touch "Move…" control.
   *
  ...
  const commitMove = async () => {
    ...
    if (!reason) return; // No reason the user typed, no write.
    ...
    try {
      ...
      // The server's answer is the answer. The optimistic position is
  ...
  // -- quick-create ------------------------------------------------
  //
  // The quick-create requirement names the board *first* and the backlog
  // second; the backlog had it and the board did not, which made half of a
  // must-have clause an absence. The rule is the backlog's, unchanged, because
  // it is one rule and it does not get a second dialect: title, type, project, and a reason
  ...
  /** The whole of quick-create's refusal, in one place. `disabled` says it on
  ...
  const submitCreate = async (event: Event) => {
    ...
    if (!title || !reason || creating()) return; // No reason, no write — again for the keyboard.
    setCreating(true);
    setCreateError(null);
    try {
   ...
   * reader would have no way to know why — which is the failure the keyboard
   * and scale requirements have between them, and the reason the cap was easier.
   *
  ...
  const onCardKeyDown = (event: KeyboardEvent, item: WorkItem, lane: Lane) => {
    ...
    // The card is a button, so it opens on Enter *and* Space — the two
  ...
  /** The keyboard requirement's last clause: quick-create has a binding, now that there is a
  ...
  // The whole-board escape. Clears every filter — search and exclusions
  ...
  // Which saved lens (by name) is applied on a bare load. Held here so
  ...
  const saveCurrent = async (name: string) => {
    // A name already in use is an overwrite, and overwriting a saved view
  ...
  // Forget was immediate and unrecoverable. It still removes the lens at
  ...
  // "hide finished columns" is a state filter set to exactly the board's
  ...
  const activeFilterChips = createMemo(() => {
    ...
    const ex = active.exclude;
    // Exclusions read as negations — "Not project: vogt" — so a reader
  ...
  return (
    ...
    >
      ...
      <ProgressiveFilters
        ...
        lenses={(
          /* A combined filter is a named lens. */
          <SavedLenses
            prefix="board"
      ...
      >
        <label class="board-field board-field--wide">
          <span>Search</span>
          {/* Free text over the loaded cards' title and body. A filter
        ...
        <div class="board-field board-field--wide board-exclude">
          <span>Exclude</span>
          {/* A negation is a facet plus a value. It lands as a "Not …"
                      ...
                      <For each={columns()}>
                        {(column) => {
                          ...
                          const slice = createMemo(() => {
                            ...
                            let end = range.end;
                            // Keyboard navigation through a windowed column: an element
                          ...
                          // This cell is the same state as the card being
                          ...
                          return (
                            <div
                              class={`board-cell${collapsedColumn(column.state) ? " collapsed" : ""}${
                                // A same-state, other-lane cell is not a
                              ...
                              // so the state a cell belongs to has
                              ...
                              onDrop={(event) => {
                                ...
                                if (!item) return;
                                // A drop that changes only the lane is a
