...
// (the tail cap, sent by `openAttach` when no `resume_from` is present), writes
...
//      once (the outbound coalesce cap), and the gate is re-checked
...
// `Terminal` park/connect state machine, so it carries no risk to the
...
export function warmAttachOnce(
  ...
  return new Promise<boolean>((resolve) => {
    // Cold attach: no `resume_from`, so `openAttach` sends the tail cap
    ...
    const finish = (ok: boolean) => {
      ...
      // never leaves a second live socket on a session (the no-socket-leak invariant).
      try {
