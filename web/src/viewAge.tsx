// How old a view is, and the nudge that keeps it from getting old.
//
// The liveness requirement is three clauses in one sentence: server-announced state updates
...
export interface AgeInput {
  ...
  now: number;
  /** The server's own words when Vogt cannot be asked at all. */
  outage?: string | null;
...
/**
 * Re-read when vogt-core says something changed.
 *
 ...
 */
/** The nudge coalescing window: long enough to fold a write burst into
...
export function onVogtLive(handler: () => void, options: LiveOptions = {}): void {
  ...
  onMount(() => {
    if (options.onNudge !== false) {
      // Coalesce the nudge fan-out. Every audited write emits an event,
    ...
    // answer is furthest from current — this is the liveness rule's "reconcile on
