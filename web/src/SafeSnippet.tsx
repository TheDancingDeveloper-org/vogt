...
function parts(text: string, query: string): SnippetPart[] {
  ...
  // U+0001/U+0002 are the backend's non-HTML match delimiters. Keep support
  // for literal <mark> wrappers from older engine responses during rollout.
  for (const chunk of text.replace(/\u0001/g, "<mark>").replace(/\u0002/g, "</mark>").split(/(<\/?mark>)/gi)) {
    ...
    for (const match of chunk.matchAll(pattern)) {
      ...
      if (index > cursor) output.push({ text: chunk.slice(cursor, index), highlighted: false });
      output.push({ text: match[0], highlighted: true });
      cursor = index + match[0].length;
    }
    if (cursor < chunk.length) output.push({ text: chunk.slice(cursor), highlighted: false });
  }
  return output;
}
