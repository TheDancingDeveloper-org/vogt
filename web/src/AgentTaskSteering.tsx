...
interface Props {
  run: AgentTaskRun;
  /** Deliver a steer to the run's PTY at its next prompt boundary. */
  onSteer: (text: string, interrupt: boolean) => void | Promise<void>;
  /** Answer an open gate by the index of the chosen option. */
  onAnswerGate: (
    gateId: string,
    optionIndex: number,
  ) => void | Promise<void>;
...
/**
 * The steer bar and approval-gate controls for one agent-task run.
 *
