// One work item, in full, with the sessions running for it.
//
...
//      running* is marked provisional rather than fresh. That rule
...
//      it is "we do not know", and it renders that way.
 ...
 *
 * Every surface owes a designed absent state, which means a
...
/** One branch's status line: "active 2h ago", how far it has diverged,
...
/** The derived git phase, in words, shown beside the workflow state.
...
/**
 * A session's activity, which is its liveness indicator.
 *
...
// -- answering a session that is waiting for input ----------
//
// The phone demo is one sentence: *receive a push that a session is
// waiting for input, open it, unblock it.* The first two parts existed —
// the push routes the notification and the item page opens the terminal — and the third
...
// reason: the trust rule's second clause is a rule about *evidence*, and until this
 ...
 *
 * *a claim backed by a still-running session is marked provisional,
 ...
 * the reason has been typed — the same rule quick-create keeps.
 */
/**
...
const ReasonForm: Component<{
  ...
  busyLabel: string;
  placeholder: string;
  /** Non-null when the write cannot be offered, and why. */
...
const WorkItemDetail: Component<Props> = (props) => {
  ...
  // -- inline edit -------------------------------------------------
  //
  // The optimistic-write rule's subject is "a drag **or inline edit**", and there was no inline
  ...
  //      the rule's "never persist, cache, or re-derive".
  //
  ...
  // and the fact chip should read the project's name.
  ...
  /** Branches this item is worked on: declared on the overlay and
  ...
  /** The derived git story: branch/PR summary, a phase shown beside the
   *  workflow state, and the contradictions between them as drift. Read-only —
   *  derived from the same observations the branch and PR collectors already gathered, never
  ...
  const freshnessTone = createMemo(() => {
    // A claim backed by a still-running session is provisional, not
  ...
  // -- the observed store, for this item ---------------------------
  ...
  /**
   * The panel's tone, by the trust rule rather than by the clock.
   *
   ...
   *  already in. The server still decides what is legal; this only
  ...
  /** Show the item body as its Markdown source rather than rendered. */
  ...
  const submitEdit = async (reason: string) => {
    ...
    try {
      ...
      // The optimistic value is discarded here and remembered
  ...
  /** Whether the start-a-session form is open. Closed by default: the
        ...
        {(current) => (
          <>
            <div class="wid-facts">
              ...
              {/* The derived git phase, shown *beside* the workflow
              ...
              <Show when={gitStory()}>
                {(story) => (
                  <span
                    ...
                    title="Derived from the branches and the pull request Vogt observed. Shown beside the workflow state, never written onto it."
                  >
            ...
            <div class="wid-columns">
              <div class="wid-main">
                ...
                {/* Comments sit directly under the Description: the
                ...
                <section class="wid-panel">
                  ...
                  {/* Move to: the rail above says where the
                  ...
                  {/* How it got here (the "state history"). The rail above
                ...
                {/* The derived git story: where this item is in git,
                    read from the branches and the PR edge. The
                            ...
                            {(finding) => (
                              ...
                              >
                                <span
                                  ...
                                  title="Vogt reports this disagreement between the item and its git evidence — it does not reconcile it."
                                >
                                  drift
                                </span>{" "}
                                {finding.message}
                          ...
                          >
                            <div class="wid-branch-head">
                              ...
                              <Show when={branch.drift}>
                                <span
                                  class="wid-branch-drift"
                                  title="Declared and observed disagree about this branch — Vogt reports it, it does not reconcile it."
                                >
                                  drift
                                </span>
                              </Show>
                ...
                <section class="wid-panel">
                          ...
                          return (
                            <li class="wid-session">
                              ...
                              <div class="wid-session-meta">
                                ...
                                {/* What it was asked to run. Shown
                              ...
                              {/* The phone's "session start/approve", and
                                  the phone demo: a session waiting for input
                  ...
                  {/* Collapsed by default: the start form is 300px of
                ...
                {/* The observed store itself. The panel above
