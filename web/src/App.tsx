 ...
 * filter set, the thing a link is required to restore. Held in memory
...
const App: Component = () => {
  ...
  // First-run install mode: true only when the instance said so, and
  ...
  /**
   * The one place a refused credential becomes a signed-out shell.
   *
   ...
   * signing the reader out over either is the forbidden collapse of "offline"
   * into "unauthorized".
   */
  ...
  onMount(() => {
    ...
    void (async () => {
      ...
      if (!token) {
        ...
        // no one has minted yet. `null` — unreachable, or a deployment
  ...
  createEffect(() => {
    ...
    if (path === "/") {
      // A fresh operator arriving from the identity wizard is taken
      ...
      // is configured — the assistant does not exist unless it is
    ...
    if (
      ...
      // surface leave one chip, not two that read the same.
  ...
  // Cross-links from a session to its files and its Git. A session
  ...
  // current pane. Creates no session; if no terminal tab is active there
  ...
  const onOpenBesideCurrent = (s: SessionSummary) => {
    ...
    // Duplicate guard, touch + a11y path: a session already in the
  ...
  const onRestoreWorkspaceLayout = async (layoutId: string) => {
    ...
    // bindings, not shells: restore re-attaches to the sessions that are
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (shortcut.id === "editor-save") {
      ...
      // otherwise pop its Save-Page dialog.
    ...
    >
      ...
      >
        ...
        <main class="main">
          ...
          {/* A place arrives over the network now. A chunk that fails
            ...
            >
              <div class="tab-view">
                ...
                {/* Mount the editor workspace once and keep it in the DOM
                    ...
                    search) never actually retained. */}
                <Show when={isIDEMode}>
                  <div
      ...
      <nav class={`phone-bottom-nav${terminalScreenActive() ? " phone-bottom-nav--terminal" : ""}`} aria-label="Primary navigation">
        ...
        {/* The bar reaches four of eleven places; this fifth slot opens a sheet
            ...
            account actions are within two taps from any surface. */}
        <button
          type="button"
      ...
      {/* A lazy component that is always mounted fetches its chunk at boot,
          ...
          nobody opened into a failure of the whole shell (the first chunk split
          shipped exactly that: `Settings-*.js` failed to load on a phone and the top
