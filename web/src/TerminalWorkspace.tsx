...
interface LayoutNodeProps {
  ...
  activePaneId: string;
  parked: boolean;
  /** Whether to draw a per-pane header with its session dropdown. Only
  ...
  /** This node's share of its parent split's main axis. Undefined at the
...
const PaneView: Component<Omit<LayoutNodeProps, "node"> & { node: PaneNode }> = (
  ...
  // pane's session or the session list changes.
  createEffect(() => {
 ...
 * on the node object, as this used to, tore every pane down on any edit (the
 * "all panes reload" symptom).
 */
const LayoutNodeView: Component<LayoutNodeProps> = (props) => {
  ...
  // drives — updates without any pane remounting.
...
const TerminalWorkspace: Component<Props> = (props) => {
  ...
  // others. Only meaningful with a split; a single pane already fills.
  const [soloed, setSoloed] = createSignal(false);
  const [busy, setBusy] = createSignal<SplitDirection | "close" | null>(null);
  // When set, the "Split with…" picker is open for this direction: the
  ...
  // Find bar: opened by Ctrl/Cmd+Shift+F or the toolbar button, drives
  ...
  // Toolbar overflow: on a phone, nine text buttons forced a hidden
  ...
  // header dropdown offers and the pool the split picker draws from.
  ...
  // no session is spawned and none is ever shown in two panes at once.
  ...
  // No PTY is created — the session already exists; the server's attach
  ...
  // a session already on screen swaps the two. No PTY is created.
  ...
  // ride along into the saved layout via the persistence effect.
  const resizeSplitBy = (
    splitId: string,
    dividerIndex: number,
    delta: number,
  ...
  // below, a separate, confirmed act.
  ...
  // *this* workspace while it is the active tab.
  createEffect(() => {
