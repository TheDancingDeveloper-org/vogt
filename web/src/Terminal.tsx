...
const TerminalView: Component<Props> = (props) => {
  ...
  // Touch copy chip: on a coarse pointer there is no right-click and iOS
  ...
  // Foreground-replay gate for background pre-warm. While a live
  ...
  const scheduleCachePersist = () => {
    ...
    // 5s, not 1s: a reconnect cache does not need per-second freshness,
  ...
  const fitAndResize = () => {
    ...
    try {
      ...
      // pane no longer looks frozen until it is clicked.
  ...
  onMount(() => {
    ...
    // while a stale 2004 mode is left enabled (an old token bug)
    ...
    // without a click, live output included.
  ...
  function connect() {
    ...
    // top of the live socket's and every line doubles or triples. Drop
  ...
  return (
    <>
      <div class="terminal-shell">
        {/*
          The work-item link's return leg. A terminal opened for a work item links back to
