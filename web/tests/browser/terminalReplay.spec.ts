...
//   the two operator symptoms directly: switching away and back does not reset
//   or flood (F1/F4), and a reload does not slow the active pane (F3/F5), with
//   no spurious `[disconnected]` (F2). See docs/local/TERMINAL_ATTACH_BUDGET_PLAN.md.
...
test.describe("terminal replay budget (live acceptance)", () => {
  ...
  test("symptom 1: switch away past the ring and back — no reset, no flood, no disconnect", async ({
    page,
  }) => {
    ...
    // The reattach replayed at most the budget, and (with F4) did not reset.
