 ...
 * notification "End" action. Runs on an emulator/device via
 * connectedAndroidTest — the deterministic tier; long-form screen-off
...
public class VoiceServiceLifecycleTest {
    ...
    public void starting_without_record_audio_degrades_without_crashing() {
        ...
        revoke(RECORD_AUDIO);
        try (ActivityScenario<MainActivity> scenario = ActivityScenario.launch(MainActivity.class)) {
            // Before the fix, a microphone-typed FGS started without
